...
queue
    .add(() => Promise.reject(new Error("error")))
    .catch(() => {
...
for await (const result of pMapIterable(items, (item) =>
    queue.add(() => fetchItem(item), { priority: item.priority })
)) {
...
setInterval(() => {
    const now = Date.now();
    const stuckTasks = queue.runningTasks.filter(
        (task) => now - task.startTime > 30000 // Running for over 30 seconds
    );
