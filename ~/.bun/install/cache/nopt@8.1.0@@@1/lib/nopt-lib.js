...
function clean(
    data,
    { types = {}, typeDefs = {}, dynamicTypes, invalidHandler, typeDefault } = {}
) {
        ...
        val = val
            .map((v) => {
                ...
                if (typeof v === "string") {
                    ...
                    v = v.trim();
                    if (
                        (v === "null" && ~type.indexOf(null)) ||
                        (v === "true" && (~type.indexOf(true) || hasTypeDef(type, BooleanType))) ||
                        (v === "false" && (~type.indexOf(false) || hasTypeDef(type, BooleanType)))
                    ) {
                        v = JSON.parse(v);
                        debug("jsonable %j", v);
...
function parse(
    ...
    data,
    remain,
    { types = {}, typeDefs = {}, shorthands = {}, dynamicTypes, unknownHandler, abbrevHandler } = {}
) {
    const StringType = typeDefs.String?.type;
    const NumberType = typeDefs.Number?.type;
    const ArrayType = typeDefs.Array?.type;
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg.charAt(0) === "-" && arg.length > 1) {
            ...
            let isArray =
                isTypeDef(argType, ArrayType) || (isTypeArray && hasTypeDef(argType, ArrayType));
            ...
            if (typeof argType === "undefined") {
                ...
                const hangingLa =
                    !hadEq && la && !la?.startsWith("-") && !["true", "false"].includes(la);
                if (unknownHandler) {
                    if (hangingLa) {
