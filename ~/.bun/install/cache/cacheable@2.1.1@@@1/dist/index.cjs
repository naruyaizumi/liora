...
var Cacheable = class extends import_hookified2.Hookified {
    ...
    hash(object, algorithm = import_utils.HashAlgorithm.SHA256) {
        const validAlgorithm = Object.values(import_utils.HashAlgorithm).includes(algorithm)
            ? algorithm
            : import_utils.HashAlgorithm.SHA256;
        ...
        for await (const [i, key] of keys.entries()) {
            if (!result[i]) {
                ...
                if (secondaryResult && secondaryResult.value !== void 0) {
                    ...
                    const cascadeTtl = (0, import_utils.getCascadingTtl)(
                        this._ttl,
                        this._primary.ttl
                    );
