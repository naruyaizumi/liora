// src/index.ts
import { getOrSet, wrap } from "@cacheable/memoize";
...
// src/index.ts
import { getOrSet as getOrSet2, wrap as wrap2, wrapSync } from "@cacheable/memoize";
import { CacheableMemory, createKeyv as createKeyv2, KeyvCacheableMemory } from "@cacheable/memory";
import {
...
var Cacheable = class extends Hookified2 {
    ...
    async processSecondaryForGetRawNonBlocking(primary, secondary, key) {
        ...
        if (secondaryResult?.value) {
            ...
            const ttl = calculateTtlFromExpiration(cascadeTtl, expires);
            const setItem = { key, value: secondaryResult.value, ttl };
            this.hook("BEFORE_SECONDARY_SETS_PRIMARY" /* BEFORE_SECONDARY_SETS_PRIMARY */, setItem)
                .then(async () => {
                    await primary.set(setItem.key, setItem.value, setItem.ttl);
                })
                .catch((error) => {
