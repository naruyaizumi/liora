  ...
  opt.filter =
    filter ?
      (file, entry) =>
        filter(file, entry) &&
        mapHas(
          (0, strip_trailing_slashes_js_1.stripTrailingSlashes)(file),
        )
    : file =>
        mapHas(
          (0, strip_trailing_slashes_js_1.stripTrailingSlashes)(file),
...
const listFileSync = opt => {
  ...
  try {
    ...
    if (stat.size < readSize) {
      ...
      const read = node_fs_1.default.readSync(
        fd,
        buf,
        0,
        stat.size,
...
exports.list = (0, make_command_js_1.makeCommand)(
  listFileSync,
  listFile,
  opt => new parse_js_1.Parser(opt),
  opt => new parse_js_1.Parser(opt),
  (opt, files) => {
    if (files?.length) (0, exports.filesFilter)(opt, files)
