...
const customRequestSerializer = (req: SerializedRequest) => {
    const { headers, id, method, raw, remoteAddress, remotePort, url, query, params } = req;
    return {
        myOwnRequest: {
...
const fakeErrorWithCause = new Error("A fake error for testing with cause", {
    cause: new Error("An inner fake error"),
});
