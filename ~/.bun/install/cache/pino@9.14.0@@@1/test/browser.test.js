...
test("opts.browser.asObjectBindingsOnly passes the bindings but keep the message unformatted", ({
    end,
    ok,
    is,
    deepEqual,
...
test("opts.browser.serialize and opts.browser.transmit only serializes log data once", ({
    end,
    ok,
    is,
}) => {
...
test("opts.browser.serialize and opts.asObject only serializes log data once", ({
    end,
    ok,
    is,
}) => {
...
test("opts.browser.serialize, opts.asObject and opts.browser.transmit only serializes log data once", ({
    end,
    ok,
    is,
}) => {
...
test("opts.browser.write obj falls back to console where a method is not supplied", ({
    end,
    ok,
    is,
}) => {
...
function absentConsoleMethodTest(method, fallback) {
    test(
        "in absence of console." + method + ", console." + fallback + " is used",
        ({ end, is }) => {
