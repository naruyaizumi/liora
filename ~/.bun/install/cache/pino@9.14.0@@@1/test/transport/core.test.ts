...
test("pino.transport with file URL", async ({ same, teardown }) => {
    const destination = join(os.tmpdir(), "_" + Math.random().toString(36).substr(2, 9));
    const transport = pino.transport({
        target: url.pathToFileURL(join(__dirname, "..", "fixtures", "ts", "to-file-transport.ts"))
            .href,
        options: { destination },
    });
    teardown(transport.end.bind(transport));
...
test("stdout in worker", async ({ not }) => {
    ...
    const child = execa(process.argv[0], [
        "-r",
        "ts-node/register",
        join(__dirname, "..", "fixtures", "ts", "transport-main.ts"),
    ]);
...
test("log and exit on ready", async ({ not }) => {
    ...
    const child = execa(process.argv[0], [
        "-r",
        "ts-node/register",
        join(__dirname, "..", "fixtures", "ts", "transport-exit-on-ready.ts"),
    ]);
...
test("log and exit before ready", async ({ not }) => {
    ...
    const child = execa(process.argv[0], [
        "-r",
        "ts-node/register",
        join(__dirname, "..", "fixtures", "ts", "transport-exit-immediately.ts"),
    ]);
...
test("log and exit before ready with async dest", async ({ not }) => {
    ...
    const child = execa(process.argv[0], [
        "-r",
        "ts-node/register",
        join(__dirname, "..", "fixtures", "ts", "transport-exit-immediately-with-async-dest.ts"),
        destination,
    ]);
...
test("string integer destination", async ({ not }) => {
    ...
    const child = execa(process.argv[0], [
        "-r",
        "ts-node/register",
        join(__dirname, "..", "fixtures", "ts", "transport-string-stdout.ts"),
    ]);
