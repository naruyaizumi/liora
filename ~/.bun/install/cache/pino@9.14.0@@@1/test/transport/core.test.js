...
test("pino.transport errors if transport worker module does not export a function", ({
    plan,
    equal,
}) => {
...
test("pino.transport with an array including a pino-pretty destination", async ({
    same,
    match,
    teardown,
}) => {
...
test("pino.transport with target pino/file and append option", async ({ same, teardown }) => {
    ...
    await writeFile(
        destination,
        JSON.stringify({ pid, hostname, time: Date.now(), level: 30, msg: "hello" })
    );
    const transport = pino.transport({
        target: "pino/file",
...
test("sets worker data informing the transport that pino will send its config", ({
    match,
    plan,
    teardown,
}) => {
...
test("sets worker data informing the transport that pino will send its config (frozen file)", ({
    match,
    plan,
    teardown,
}) => {
...
test("log and exit on ready", async ({ not }) => {
    let actual = "";
    const child = execa(process.argv[0], [
        join(__dirname, "..", "fixtures", "transport-exit-on-ready.js"),
    ]);
...
test("log and exit before ready", async ({ not }) => {
    let actual = "";
    const child = execa(process.argv[0], [
        join(__dirname, "..", "fixtures", "transport-exit-immediately.js"),
    ]);
...
test("log and exit before ready with async dest", async ({ not }) => {
    const destination = file();
    const child = execa(process.argv[0], [
        join(__dirname, "..", "fixtures", "transport-exit-immediately-with-async-dest.js"),
        destination,
    ]);
...
test("string integer destination", async ({ not }) => {
    let actual = "";
    const child = execa(process.argv[0], [
        join(__dirname, "..", "fixtures", "transport-string-stdout.js"),
    ]);
