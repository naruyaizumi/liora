...
method itself, `level` is the log level itself. This hook **_must_** invoke the
...
log.fatal(
    "this will be written to /tmp/debug.stream.out, /tmp/info.stream.out and /tmp/fatal.stream.out"
);
```
