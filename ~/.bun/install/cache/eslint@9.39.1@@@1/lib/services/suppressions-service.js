...
class SuppressionsService {
    ...
    applySuppressions(results, suppressions) {
        ...
        for (const result of filtered) {
            ...
            for (const ruleId in violationsByRule) {
                ...
                const suppressionsCount = suppressions[relativeFilePath][ruleId].count;
            ...
            for (const ruleId in suppressions[relativeFilePath]) {
                ...
                unused[relativeFilePath] ??= {};
                unused[relativeFilePath][ruleId] = suppressions[relativeFilePath][ruleId];
            }
     ...
     */
    save(suppressions) {
        return fs.promises.writeFile(this.filePath, stringify(suppressions, { space: 2 }));
    }
    ...
    getRelativeFilePath(filePath) {
        return path.relative(this.cwd, filePath).split(path.sep).join(path.posix.sep);
    }
