    ...
    create(context) {
        ...
        function verify(target) {
            ...
            function verifyAssignmentIsUsed(targetAssignment, allAssignments) {
                ...
                const readReferences = targetAssignment.variable.references.filter((reference) =>
                    reference.isRead()
                );
                    ...
                    if (
                        ...
                        (assignment.segments.length &&
                            assignment.segments.every(
                                (segment) => !targetAssignment.segments.includes(segment)
                            ))
                    ) {
                        return false;
                    }
                    ...
                    if (
                        ...
                        (isIdentifierUsedBetweenAssignedAndEqualSign(
                            ...
                            targetAssignment.segments.some((segment) =>
                                isIdentifierUsedInSegment(segment, reference.identifier)
                            ))
                    ) {
                        if (
                    ...
                    if (variable.scope.type === "module") {
                        if (
                            variable.defs.some(
                                (def) =>
                                    (def.type === "Variable" &&
                                        def.parent.parent.type === "ExportNamedDeclaration") ||
