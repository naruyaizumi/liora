...
function shouldReportImportExport(node, previousNodes, allowSeparateTypeImports) {
    ...
    while (i < previousNodes.length) {
        ...
        if (allowSeparateTypeImports) {
            const isTypeNode = node.importKind === "type" || node.exportKind === "type";
            const isTypePrevious =
                previousNode.importKind === "type" || previousNode.exportKind === "type";
...
function checkAndReport(
    ...
    if (modules.has(module)) {
        ...
        if (declarationType === "import") {
            if (shouldReportImportExport(node, importNodes, allowSeparateTypeImports)) {
                messagesIds.push("import");
            }
            if (includeExports) {
                if (shouldReportImportExport(node, exportNodes, allowSeparateTypeImports)) {
