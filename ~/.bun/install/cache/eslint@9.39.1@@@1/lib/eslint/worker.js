...
(async () => {
    ...
    warningService.emitEmptyConfigWarning = warningService.emitInactiveFlagWarning = () => {};
    ...
    const lintResultCache = createLintResultCache(processedESLintOptions, cacheFilePath);
    ...
    debug("Linting started %t after dependencies loaded", lintingStartTime - depsLoadedTime);
    ...
    for (;;) {
        ...
        debug('Config array for file "%s" loaded in %t', filePath, loadConfigDuration);
        ...
        debug('File "%s" processed in %t', filePath, fileLintingEndTime - fileLintingStartTime);
    }
