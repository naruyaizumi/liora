...
function calculateAutoWorkerCount(eslint, filePaths, maxWorkers) {
    ...
    for (const filePath of filePaths) {
        ...
        if (countAllMatched || needsReprocessing(lintResultCache, fix, filePath, config)) {
            processableFileCount++;
            lowWorkerCount = getWorkerCountFor(processableFileCount, maxWorkers);
        }
    }
    debug("%d file(s) to process counted in %t", processableFileCount, hrtimeBigint() - startTime);
 ...
 */
function calculateWorkerCount(eslint, filePaths, { availableParallelism } = os) {
...
const disableCloneabilityCheck = Symbol("Do not check for uncloneable options.");
 ...
 */
async function runWorkers(filePaths, workerCount, eslintOptionsOrURL, warnOnLowNetLintingRatio) {
    ...
    function workerExecutor(resolve_, reject) {
        ...
        worker.once("message", (/** @type {WorkerLintResults} */ indexedResults) => {
            ...
            const netLintingRatio =
                Number(indexedResults.netLintingDuration) / Number(workerDuration);
...
class ESLint {
    ...
    static async outputFixes(results) {
        ...
        await Promise.all(
            results
                ...
                .map((r) => retrier.retry(() => fs.writeFile(r.filePath, r.output)))
        );
    }
    ...
    async lintFiles(patterns) {
        ...
        const { concurrency, cwd, globInputPaths, errorOnUnmatchedPattern } = eslintOptions;
        ...
        debug("%d file(s) found in %t", filePaths.length, hrtimeBigint() - startTime);
    ...
    async loadFormatter(name = "stylish") {
        ...
        if (!namespace && normalizedFormatName.includes("/")) {
            ...
            try {
                const npmFormat = normalizePackageName(normalizedFormatName, "eslint-formatter");
    ...
    async findConfigFile(filePath) {
        ...
        return this.#configLoader.findConfigFileForPath(filePath ?? fakeCwd).catch(() => void 0);
    }
