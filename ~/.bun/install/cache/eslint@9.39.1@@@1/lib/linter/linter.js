    ...
    for (const ruleId of directiveRules) {
        ...
        if (ruleId === null || !!ruleMapper(ruleId)) {
            if (type === "disable-next-line") {
                const { line, column } = updateLocationInformation(loc.end, language);
                ...
                const { line, column } = updateLocationInformation(loc.start, language);
 ...
 */
function getDirectiveComments(sourceCode, ruleMapper, warnInlineConfig, config, report) {
...
class Linter {
    ...
    #eslintrcVerifyWithoutProcessors(file, providedConfig, providedOptions) {
        ...
        const envInFile =
            options.allowInlineConfig && !options.warnInlineConfig ? findEslintEnv(file.body) : {};
    ...
    verify(textOrSourceCode, config, filenameOrOptions) {
        ...
        if (options.preprocess || options.postprocess) {
            return this._distinguishSuppressedMessages(
                this._verifyWithProcessor(textOrSourceCode, configToUse, options)
            );
        }
        return this._distinguishSuppressedMessages(
    ...
    _verifyWithProcessor(textOrSourceCode, config, options, configForRecursive) {
        ...
        const filterCodeBlock =
            options.filterCodeBlock || ((blockFilePath) => blockFilePath.endsWith(".js"));
        const originalExtname = path.extname(filename);
        ...
        const messageLists = files.map((block) => {
            ...
            if (
                ...
                return this._verifyWithConfigArray(block.rawBody, configForRecursive, {
                    ...options,
                    filename: block.path,
                    physicalFilename: block.physicalPath,
                });
