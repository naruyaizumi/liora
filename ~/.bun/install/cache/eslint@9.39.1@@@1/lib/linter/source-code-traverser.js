...
class ESQueryHelper {
    ...
    calculateSelectors(node, ancestry, isExit) {
        ...
        const selectorsByNodeType =
            (isExit ? this.exitSelectorsByNodeType : this.enterSelectorsByNodeType).get(
                node[nodeTypeKey]
            ) || [];
            ...
            switch (step.kind) {
                case STEP_KIND_VISIT: {
                    try {
                        if (step.phase === 1) {
                            esquery
                                .calculateSelectors(step.target, currentAncestry, false)
                                .forEach((selector) => {
                            ...
                            esquery
                                .calculateSelectors(step.target, currentAncestry, true)
                                .forEach((selector) => {
                                    visitor.callSync(selector, ...(step.args ?? [step.target]));
                                });
                        }
                ...
                default:
                    throw new Error(`Invalid traversal step found: "${step.kind}".`);
            }
        }
    }
}
