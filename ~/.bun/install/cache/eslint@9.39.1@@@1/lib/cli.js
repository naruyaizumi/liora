...
function createOptionsModule(options) {
    ...
    return new URL(`data:text/javascript;base64,${Buffer.from(optionsSrc).toString("base64")}`);
}
...
function validateConcurrency(concurrency) {
    if (concurrency === void 0 || concurrency === "auto" || concurrency === "off") {
        return;
    }
    ...
    async execute(args, text, allowFlatConfig = true) {
        ...
        if (options.inspectConfig) {
            ...
            try {
                ...
                spawn.sync("npx", ["@eslint/config-inspector@latest", ...flags], {
                    encoding: "utf8",
                    stdio: "inherit",
                });
            } catch (error) {
        ...
        if (
            useStdin &&
            (options.suppressAll || options.suppressRule || options.pruneSuppressions)
        ) {
            log.error(
        ...
        if (
            ...
            if (!options.passOnUnprunedSuppressions) {
                const unusedSuppressionsCount = Object.keys(unusedSuppressions).length;
