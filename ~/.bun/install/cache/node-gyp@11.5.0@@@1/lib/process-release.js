...
function processRelease(argv, gyp, defaultVersion, defaultRelease) {
    ...
    if (defaultRelease && defaultRelease.headersUrl && !overrideDistUrl) {
        ...
        libUrl32 = resolveLibUrl(
            name,
            defaultRelease.libUrl || baseUrl || distBaseUrl,
            "x86",
            versionSemver.major
        );
        libUrl64 = resolveLibUrl(
            name,
            defaultRelease.libUrl || baseUrl || distBaseUrl,
            "x64",
            versionSemver.major
        );
        libUrlArm64 = resolveLibUrl(
            name,
            defaultRelease.libUrl || baseUrl || distBaseUrl,
            "arm64",
            versionSemver.major
        ...
        tarballUrl = url.resolve(
            baseUrl,
            name + "-v" + version + (canGetHeaders ? "-headers" : "") + ".tar.gz"
        );
    }
    ...
    return {
        ...
        ia32: {
            ...
            libPath: normalizePath(
                path.relative(url.parse(baseUrl).path, url.parse(libUrl32).path)
            ),
        },
        x64: {
            libUrl: libUrl64,
            libPath: normalizePath(
                path.relative(url.parse(baseUrl).path, url.parse(libUrl64).path)
            ),
        },
        arm64: {
            libUrl: libUrlArm64,
