...
async function install(gyp, argv) {
    ...
    if (release.semver.prerelease[0] === "pre") {
        ...
        if (!gyp.opts.nodedir) {
            throw new Error(
                '"pre" versions of node cannot be installed, use the --nodedir flag instead'
            );
        }
    ...
    if (gyp.opts.ensure) {
        ...
        try {
            ...
            if (err.code === "ENOENT") {
                log.verbose(
                    "install",
                    "version not already installed, continuing with install",
                    release.version
                );
                try {
                    return await go();
        ...
        if (win) {
            ...
            try {
                ...
                if (err.code === "ENOENT") {
                    log.verbose(
                        "install",
                        `version not already installed for ${arch}, continuing with install`,
                        release.version
                    );
                    try {
    ...
    async function go() {
        ...
        try {
            if (shouldDownloadTarball) {
                if (tarPath) {
                    ...
                    try {
                        ...
                        if (res.status !== 200) {
                            throw new Error(
                                `${res.status} response downloading ${release.tarballUrl}`
                            );
                        }
                        ...
                        if (err.code === "ENOTFOUND") {
                            throw new Error(
                                "This is most likely not a problem with node-gyp or the package itself and\n" +
            ...
            for (const k in contentShasums) {
                log.verbose(
                    "validating download checksum for " + k,
                    "(%s == %s)",
                    contentShasums[k],
                    expectShasums[k]
                );
    ...
    async function eaccesFallback(err) {
        ...
        log.warn(
            "EACCES",
            'current user%s does not have permission to access the dev dir "%s"',
            userString,
            devDir
        );
        ...
        if (process.cwd() === tmpdir) {
            log.verbose(
                "tmpdir == cwd",
                "automatically will remove dev files after to save disk space"
            );
