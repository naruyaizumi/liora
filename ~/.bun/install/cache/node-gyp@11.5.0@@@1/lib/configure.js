...
async function configure(gyp, argv) {
    ...
    async function getNodeDir() {
        ...
        if (!gyp.opts.nodedir && process.config.variables.use_prefix_to_find_headers) {
            ...
            let availVersion;
            try {
                const nodeVersionH = readFileSync(
                    path.join(prefix, "include", "node", "node_version.h"),
                    { encoding: "utf8" }
                );
            ...
            if (availVersion === release.version) {
                ...
                log.verbose(
                    "using local node headers based on prefix",
        ...
        if (gyp.opts.nodedir) {
            ...
            log.verbose(
                "get node dir",
                "compiling against specified --nodedir dev files: %s",
                nodeDir
            );
        } else {
            ...
            if ("v" + release.version !== process.version) {
                ...
                log.verbose(
                    "get node dir",
                    "compiling against --target node version: %s",
                    release.version
                );
            } else {
                ...
                log.verbose(
                    "get node dir",
                    "no --target version specified, falling back to host node version: %s",
                    release.version
                );
            }
        ...
        if (process.platform === "os390") {
            ...
            if (zoslibIncPath) {
                ...
                if (zoslibIncPath === undefined) {
                    msg = msgFormat(
                        "Could not find zos-base.h file in the directory set " +
                            "in ZOSLIB_INCLUDES environment variable: %s; set it " +
                            "to the correct path, or unset it to search %s",
                        process.env.ZOSLIB_INCLUDES,
                        nodeRootDir
                    );
                }
            } else {
                ...
                if (zoslibIncPath === undefined) {
                    msg = msgFormat(
                        "Could not find any of %s in directory %s; set " +
                            "environmant variable ZOSLIB_INCLUDES to the path " +
                            "that contains zos-base.h",
                        candidates.toString(),
                        nodeRootDir
                    );
                }
...
module.exports.usage =
    "Generates " + (win ? "MSVC project files" : "a Makefile") + " for the current module";
