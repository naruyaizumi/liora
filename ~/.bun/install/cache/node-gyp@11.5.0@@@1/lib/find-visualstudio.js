...
class VisualStudioFinder {
    ...
    async findVSFromSpecifiedLocation(supportedYears) {
        ...
        if (sdkVersionMatched) {
            info.packages.push(
                `Microsoft.VisualStudio.Component.Windows10SDK.${sdkVersionMatched[3]}.Desktop`
            );
        }
    ...
    async findNewVSUsingSetupModule(supportedYears) {
        ...
        const filterArg =
            vcInstallDir !== undefined
                ? `| where {$_.InstallationPath -eq '${vcInstallDir}' }`
                : "";
        const psArgs = [
            "-NoProfile",
            "-Command",
    ...
    processData(vsInfo, supportedYears) {
        ...
        vsInfo = vsInfo.filter((info) => {
            ...
            this.addLog(
                `${info.versionYear ? "unsupported" : "unknown"} version "${info.version}" found at "${info.path}"`
            );
            return false;
        });
    ...
    getMSBuild(info, versionYear) {
        ...
        const msbuildPathArm64 = path.join(
            info.path,
            "MSBuild",
            "Current",
            "Bin",
