...
test("three consecutive wildcards: *.*.*.password (4 levels deep)", () => {
    ...
    assert.strictEqual(
        parsed.simple.password,
        "secret-2-levels",
        "2-level password should NOT be redacted"
    );
    assert.strictEqual(
        parsed.user.auth.password,
        "secret-3-levels",
        "3-level password should NOT be redacted"
    );
    assert.strictEqual(
        parsed.nested.deep.auth.password,
        "[REDACTED]",
        "4-level password SHOULD be redacted"
    );
});
...
test("four consecutive wildcards: *.*.*.*.password (5 levels deep)", () => {
    ...
    assert.strictEqual(
        parsed.nested.deep.auth.password,
        "secret-4-levels",
        "4-level password should NOT be redacted"
    );
    assert.strictEqual(
        parsed.config.user.auth.settings.password,
        "[REDACTED]",
        "5-level password SHOULD be redacted"
    );
});
...
test("five consecutive wildcards: *.*.*.*.*.password (6 levels deep)", () => {
    ...
    assert.strictEqual(
        parsed.config.user.auth.settings.password,
        "secret-5-levels",
        "5-level password should NOT be redacted"
    );
    assert.strictEqual(
        parsed.data.reqConfig.data.credentials.settings.password,
        "[REDACTED]",
        "6-level password SHOULD be redacted"
    );
});
...
test("three wildcards with censor function receives correct values", () => {
    ...
    assert.strictEqual(
        censorCalls[0].value,
        "secret-value",
        "censor should receive the actual value"
    );
    assert.deepStrictEqual(
        censorCalls[0].path,
        ["nested", "deep", "auth", "password"],
        "censor should receive correct path"
    );
...
test("three wildcards with remove option", () => {
    ...
    assert.strictEqual(
        "password" in parsed.nested.deep.auth,
        false,
        "password key should be removed"
    );
...
test("mixed: two and three wildcards in same redactor", () => {
    ...
    assert.strictEqual(
        parsed.user.auth.password,
        "[REDACTED]",
        "3-level should be redacted by *.*.password"
    );
    assert.strictEqual(
        parsed.config.deep.auth.password,
        "[REDACTED]",
        "4-level should be redacted by *.*.*.password"
    );
});
...
test("three wildcards with arrays", () => {
    const obj = {
        users: [{ auth: { password: "secret1" } }, { auth: { password: "secret2" } }],
    };
