...
function _parse_opts(args, descriptor) {
    ...
    if (Object.keys(kwargs).length) {
        throw new TypeError(
            sub("%s() got an unexpected keyword argument %r", get_name(), Object.keys(kwargs)[0])
        );
    }
...
const ArgumentError = _callable(
    class ArgumentError extends Error {
        ...
        str() {
            ...
            return sub(format, { message: this._message, argument_name: this._argument_name });
        }
    }
);
...
const BooleanOptionalAction = _camelcase_alias(
    _callable(
        class BooleanOptionalAction extends Action {
            constructor() {
                let [option_strings, dest, default_value, type, choices, required, help, metavar] =
                    _parse_opts(arguments, {
                        option_strings: no_default,
                        dest: no_default,
                        default: undefined,
...
const _SubParsersAction = _camelcase_alias(
    _callable(
        class _SubParsersAction extends Action {
            constructor() {
                let [option_strings, prog, parser_class, dest, required, help, metavar] =
                    _parse_opts(arguments, {
                        option_strings: no_default,
                        prog: no_default,
...
const FileType = _callable(
    class FileType extends Function {
        ...
        constructor() {
            let [flags, encoding, mode, autoClose, emitClose, start, end, highWaterMark, fs] =
                _parse_opts(arguments, {
                    flags: "r",
                    encoding: undefined,
...
const _ActionsContainer = _camelcase_alias(
    _callable(
        class _ActionsContainer {
            constructor() {
                let [description, prefix_chars, argument_default, conflict_handler] = _parse_opts(
                    arguments,
                    {
            ...
            _get_optional_kwargs() {
                ...
                for (option_string of args) {
                    ...
                    if (!this.prefix_chars.includes(option_string[0])) {
                        let args = { option: option_string, prefix_chars: this.prefix_chars };
                        let msg =
        ...
        class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
            ...
            _match_argument(action, arg_strings_pattern) {
                ...
                if (match === null) {
                    ...
                    if (msg === undefined) {
                        msg = sub(
                            action.nargs === 1 ? "expected %s argument" : "expected %s arguments",
                            action.nargs
                        );
                    }
            ...
            _parse_optional(arg_string) {
                ...
                if (option_tuples.length > 1) {
                    let options = option_tuples
                        .map(([, /*action*/ option_string /*, explicit_arg*/]) => option_string)
                        .join(", ");
            ...
            parse_known_intermixed_args(args = undefined, namespace = undefined) {
                ...
                if (a.length) {
                    throw new TypeError(
                        sub("parse_intermixed_args: positional arg" + " with nargs=%s", a[0].nargs)
                    );
                }
