...
test("Create error with 2 parameters", (t) => {
    ...
    t.assert.deepStrictEqual(
        warnItem.format("alice", "attitude"),
        "hey alice, I like your attitude"
    );
    t.assert.deepStrictEqual(warnItem.code, "CODE");
});
...
test("Create error with 3 parameters", (t) => {
    ...
    t.assert.deepStrictEqual(warnItem.name, "TestWarning");
    t.assert.deepStrictEqual(
        warnItem.format("alice", "attitude", "see you"),
        "hey alice, I like your attitude see you"
    );
    t.assert.deepStrictEqual(warnItem.code, "CODE");
});
...
test("Should throw when error has no code", (t) => {
    t.plan(1);
    t.assert.throws(
        () => createWarning({ name: "name" }),
        new Error("Warning code must not be empty")
    );
});
