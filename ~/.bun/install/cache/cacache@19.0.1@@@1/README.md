...
function _only_ returns `data` itself, without any wrapper.
...
```javascript
fetch("https://registry.npmjs.org/cacache/-/cacache-1.0.0.tgz")
    .then((data) => {
...
```javascript
request
    .get("https://registry.npmjs.org/cacache/-/cacache-1.0.0.tgz")
    .pipe(
        cacache.put
...
_Streaming only_ If present, uses the provided event emitter as a source of
...
const integrity =
    hashAlgorithm + "-" + crypto.createHash(hashAlgorithm).update(data).digest("base64");
```
