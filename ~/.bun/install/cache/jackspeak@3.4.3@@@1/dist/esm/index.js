...
const toEnvVal = (value, delim = '\n') => {
  const str =
    ...
    : Array.isArray(value) ? value.map(v => toEnvVal(v)).join(delim)
...
export const isConfigOption = (o, type, multi) =>
  !!o &&
  typeof o === 'object' &&
  isConfigType(o.type) &&
  o.type === type &&
  ...
  usageMarkdown() {
    ...
    for (const row of rows) {
      if (row.left) {
        ...
        headingLevel = level
        out.push(
          `${'#'.repeat(headingLevel)} ${normalizeOneLine(row.text, row.pre)}`,
        )
      } else {
