...
### uniqueFilename(_dir_, _fileprefix_, _uniqstr_) → String

Returns the full path of a unique filename that looks like:
`dir/prefix-7ddd44c0`
or `dir/7ddd44c0`

_dir_ – The path you want the filename in. `os.tmpdir()` is a good choice for this.

_fileprefix_ – A string to append prior to the unique part of the filename.
The parameter is required if _uniqstr_ is also passed in but is otherwise
...
_uniqstr_ – Optional, if not passed the unique part of the resulting
