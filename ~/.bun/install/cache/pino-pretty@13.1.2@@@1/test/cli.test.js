...
describe("cli", () => {
        ...
        test(`flips epoch and level via ${optionName}`, async (t) => {
            ...
            const endPromise = once(child.stdout, "data", (data) => {
                t.assert.strictEqual(
                    data.toString(),
                    `INFO [${formattedEpoch}] (42): hello world\n`
                );
            });
            child.stdin.write(logLine);
            await endPromise;
    ...
    for (const optionName of ["--translateTime", "-t"]) {
        test(`translates time to default format via ${optionName}`, async (t) => {
            ...
            const endPromise = once(child.stdout, "data", (data) => {
                t.assert.strictEqual(
                    data.toString(),
                    `[${formattedEpoch}] INFO (42): hello world\n`
                );
            });
            child.stdin.write(logLine);
            await endPromise;
    ...
    for (const optionName of ["--customLevels", "-x"]) {
        test(`customize levels via ${optionName}`, async (t) => {
            t.plan(1);
            const logLine =
                '{"level":1,"time":1522431328992,"msg":"hello world","pid":42,"hostname":"foo"}\n';
        ...
        test(`customize levels via ${optionName} with minimumLevel`, async (t) => {
            ...
            const child = spawn(
                process.argv[0],
                [bin, "--minimumLevel", "err", optionName, "err:99,info:1"],
                { env }
            );
            child.on("error", t.assert.fail);
            const endPromise = once(child.stdout, "data", (data) => {
                t.assert.strictEqual(
                    data.toString(),
                    `[${formattedEpoch}] ERR (42): hello world\n`
                );
            });
            child.stdin.write(
                '{"level":1,"time":1522431328992,"msg":"hello world","pid":42,"hostname":"foo"}\n'
            ...
            child.stdin.write(
                '{"level":99,"time":1522431328992,"msg":"hello world","pid":42,"hostname":"foo"}\n'
            );
            await endPromise;
            t.after(() => child.kill());
        });
        ...
        test(`customize levels via ${optionName} with minimumLevel, customLevels and useOnlyCustomProps false`, async (t) => {
            ...
            const endPromise = once(child.stdout, "data", (data) => {
                t.assert.strictEqual(
                    data.toString(),
                    `[${formattedEpoch}] CUSTOM (42): hello world\n`
                );
            });
            child.stdin.write(
                '{"level":1,"time":1522431328992,"msg":"hello world","pid":42,"hostname":"foo"}\n'
    ...
    for (const optionName of ["--customColors", "-X"]) {
        test(`customize levels via ${optionName}`, async (t) => {
            t.plan(1);
            const child = spawn(process.argv[0], [bin, optionName, "info:blue,message:red"], {
                env,
            });
            child.on("error", t.assert.fail);
            const endPromise = once(child.stdout, "data", (data) => {
        ...
        test(`customize levels via ${optionName} with customLevels`, async (t) => {
            ...
            const child = spawn(
                process.argv[0],
                [bin, "--customLevels", "err:99,info", optionName, "info:blue,message:red"],
                { env }
            );
            child.on("error", t.assert.fail);
            const endPromise = once(child.stdout, "data", (data) => {
    ...
    for (const optionName of ["--useOnlyCustomProps", "-U"]) {
        test(`customize levels via ${optionName} false and customColors`, async (t) => {
            ...
            const child = spawn(
                process.argv[0],
                [bin, "--customColors", "err:blue,info:red", optionName, "false"],
                { env }
            );
            child.on("error", t.assert.fail);
            const endPromise = once(child.stdout, "data", (data) => {
        ...
        test(`customize levels via ${optionName} true and customColors`, async (t) => {
            ...
            const child = spawn(
                process.argv[0],
                [bin, "--customColors", "err:blue,info:red", optionName, "true"],
                { env }
            );
            child.on("error", t.assert.fail);
            const endPromise = once(child.stdout, "data", (data) => {
        ...
        test(`customize levels via ${optionName} true and customLevels`, async (t) => {
            ...
            const child = spawn(
                process.argv[0],
                [bin, "--customLevels", "err:99,custom:30", optionName, "true"],
                { env }
            );
            child.on("error", t.assert.fail);
            const endPromise = once(child.stdout, "data", (data) => {
        ...
        test(`customize levels via ${optionName} false and customLevels`, async (t) => {
            ...
            const child = spawn(
                process.argv[0],
                [bin, "--customLevels", "err:99,custom:25", optionName, "false"],
                { env }
            );
            child.on("error", t.assert.fail);
            const endPromise = once(child.stdout, "data", (data) => {
    ...
    test("does ignore escaped keys", async (t) => {
        ...
        const logLine =
            '{"level":30,"time":1522431328992,"msg":"hello world","log.domain.corp/foo":"bar"}\n';
        child.stdin.write(logLine);
        await endPromise;
        t.after(() => child.kill());
    });
    ...
    for (const optionName of ["--singleLine", "-S"]) {
        test(`singleLine=true via ${optionName}`, async (t) => {
            t.plan(1);
            const logLineWithExtra =
                JSON.stringify(
                    Object.assign(JSON.parse(logLine), {
                        extra: {
                            foo: "bar",
                            number: 42,
                        },
            ...
            const endPromise = once(child.stdout, "data", (data) => {
                t.assert.strictEqual(
                    data.toString(),
                    `[${formattedEpoch}] INFO (42): hello world {"extra":{"foo":"bar","number":42}}\n`
                );
            });
    ...
    test("does ignore nested keys", async (t) => {
        ...
        const child = spawn(
            process.argv[0],
            [bin, "-S", "-i", "extra.foo,extra.nested,extra.nested.miss"],
            { env }
        );
        child.on("error", t.assert.fail);
        const endPromise = once(child.stdout, "data", (data) => {
            t.assert.strictEqual(
                data.toString(),
                `[${formattedEpoch}] INFO (42 on foo): hello world {"extra":{"number":42}}\n`
            );
        });
