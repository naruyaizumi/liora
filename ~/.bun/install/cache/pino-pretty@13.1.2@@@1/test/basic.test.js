...
describe("basic prettifier tests", () => {
    ...
    test("can print message key value when its a string", (t) => {
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    const formatted = pretty(chunk.toString());
                    t.assert.strictEqual(formatted, `[${formattedEpoch}] INFO (${pid}): baz\n`);
                    cb();
                },
            })
        );
    ...
    test("can print message key value when its a number", (t) => {
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    const formatted = pretty(chunk.toString());
                    t.assert.strictEqual(formatted, `[${formattedEpoch}] INFO (${pid}): 42\n`);
                    cb();
                },
            })
        );
    ...
    test("can print message key value when its a Number(0)", (t) => {
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    const formatted = pretty(chunk.toString());
                    t.assert.strictEqual(formatted, `[${formattedEpoch}] INFO (${pid}): 0\n`);
                    cb();
                },
            })
        );
    ...
    test("can print message key value when its a boolean", (t) => {
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    const formatted = pretty(chunk.toString());
                    t.assert.strictEqual(formatted, `[${formattedEpoch}] INFO (${pid}): true\n`);
                    cb();
                },
            })
        );
    ...
    test("can use different level keys", (t) => {
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    const formatted = pretty(chunk.toString());
                    t.assert.strictEqual(formatted, `[${formattedEpoch}] WARN (${pid}): foo\n`);
                    cb();
                },
            })
        );
    ...
    test("can use a customPrettifier on default time output", (t) => {
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    const formatted = pretty(chunk.toString());
                    t.assert.strictEqual(formatted, `TIME: ${formattedEpoch} INFO (${pid}): foo\n`);
                    cb();
                },
            })
        );
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    ...
                    t.assert.strictEqual(
                        formatted,
                        `[${formattedEpoch}] INFO (matteo/${pid}): hello world\n`
                    );
                    cb();
                },
            })
    ...
    test("filter some lines based on minimumLevel", (t) => {
        ...
        const expected = [undefined, undefined, `[${formattedEpoch}] INFO (${pid}): baz\n`];
        const log = pino(
            {},
            new Writable({
                write(chunk, enc, cb) {
                    const formatted = pretty(chunk.toString());
                    t.assert.strictEqual(formatted, expected.shift());
                    cb();
                },
            })
        );
    ...
    test("ignores multiple keys", (t) => {
        ...
        const arst = pretty(
            `{"msg":"hello world", "pid":"${pid}", "hostname":"${hostname}", "time":${epoch}, "level":30}`
        );
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    ...
                    t.assert.strictEqual(
                        formatted,
                        `[${formattedEpoch}] INFO (${pid}): hello world\n`
                    );
                    cb();
                },
            })
    ...
    test("Prints extra objects on one line with singleLine=true", (t) => {
        ...
        const log = pino(
            ...
            new Writable({
                write(chunk, enc, cb) {
                    ...
                    t.assert.strictEqual(
                        formatted,
                        `[${formattedEpoch}] INFO (${pid}): message {"extra":{"foo":"bar","number":42},"upper":"FOOBAR"}\n`
                    );
        ...
        log.info({
            msg: "message",
            extra: { foo: "bar", number: 42 },
            upper: "foobar",
            undef: "this will not show up",
        });
    });
    ...
    test("sync option", async (t) => {
        ...
        t.assert.strictEqual(
            formatted,
            `[${formattedEpoch}] INFO (${pid}): message {"extra":{"foo":"bar","number":43},"upper":"foobar"}\n`
        );
    });
