...
describe("cli", () => {
    ...
    for (const optionName of ["--messageKey", "-m"]) {
      ...
      `.trim()
            ...
            const env = { TERM: " dumb", TZ: "UTC" };
            const child = spawn(process.argv[0], [bin, optionName, "new_msg"], {
                env,
                cwd: tmpDir,
            });
    ...
    test("throws on missing config file", async (t) => {
        ...
        const endPromise2 = once(child, "close", () => {
            match(
                data.toString(),
                "Error: Failed to load runtime configuration file: pino-pretty.config.missing.json",
                t
            );
        });
    ...
    test("throws on invalid custom config file", async (t) => {
        ...
        const endPromise2 = once(child, "close", () => {
            match(
                data,
                "Error: Invalid runtime configuration file: pino-pretty.config.invalid.js",
                t
            );
        });
        await Promise.all([endPromise1, endPromise2]);
