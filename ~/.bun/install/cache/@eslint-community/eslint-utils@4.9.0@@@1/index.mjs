...
function getInnermostScope(initialScope, node) {
    ...
    do {
        ...
        for (const childScope of scope.childScopes) {
            const range = /** @type {[number, number]} */ (childScope.block.range);
...
function getOpeningParenOfParams(node, sourceCode) {
    return node.id
        ? /** @type {Token} */ (sourceCode.getTokenAfter(node.id, isOpeningParenToken))
        : /** @type {Token} */ (sourceCode.getFirstToken(node, isOpeningParenToken));
}
...
const callAllowed = new Set(
    [
        ...
        .../** @type {(keyof typeof Math)[]} */ (Object.getOwnPropertyNames(Math))
            .filter((k) => k !== "random")
            .map((k) => Math[k])
...
const operations = Object.freeze({
    ...
    BinaryExpression(node, initialScope) {
        ...
        if (left != null && right != null) {
            switch (node.operator) {
                ...
                case "<":
                    return {
                        value: /** @type {any} */ (left.value) < /** @type {any} */ (right.value),
                    };
                case "<=":
                    return {
                        value: /** @type {any} */ (left.value) <= /** @type {any} */ (right.value),
            ...
            if (calleeNode.type === "MemberExpression") {
                ...
                if (object != null) {
                    ...
                    if (property != null) {
                        ...
                        const methodName = /** @type {PropertyKey} */ (property.value);
                ...
                if (callee != null) {
                    ...
                    const func = /** @type {(...args: any[]) => any} */ (callee.value);
        ...
        if (object != null) {
            ...
            if (property != null) {
                if (
                    ...
                    return {
                        value: /** @type {Record<PropertyKey, unknown>} */ (object.value)[
                            /** @type {PropertyKey} */ (property.value)
                        ],
                    };
                }
    ...
    NewExpression(node, initialScope) {
        ...
        if (callee != null && args != null) {
            const Func = /** @type {new (...args: any[]) => any} */ (callee.value);
            if (callAllowed.has(Func)) {
 ...
 */
function hasId(node) {
    return Boolean(/** @type {Partial<FunctionDeclaration | FunctionExpression>} */ (node).id);
}
    ...
    Object.assign(Object.create(null), {
        ...
        $visit(node, options, visitorKeys) {
            ...
            if (typeof (/** @type {any} */ (this)[type]) === "function") {
                return /** @type {any} */ (this)[type](node, options, visitorKeys);
            }
        ...
        $visitChildren(node, options, visitorKeys) {
            ...
            for (const key of /** @type {(keyof Node)[]} */ (visitorKeys[type] || getKeys(node))) {
            ...
            if (parent.arguments.length === 1 && parent.arguments[0] === node) {
                return sourceCode.getTokenAfter(
                    ...
                    parent.typeArguments ||
                        /** @type {RuleNode} */ (
                            /** @type {unknown} */ (
                                /** @type {TSNewExpression | TSCallExpression} */ (parent)
                                    .typeParameters
                            )
                        ) ||
                        parent.callee,
...
function replaceF(matcher, str, replace) {
    ...
    for (const match of matcher.execAll(str)) {
        ...
        chunks.push(
            String(
                replace(
                    .../** @type {[string, ...string[]]} */ (/** @type {string[]} */ (match)),
                    match.index,
                    match.input
                )
            )
...
class PatternMatcher {
     ...
     */
    *execAll(str) {
        const { pattern, escaped } = /** @type {{pattern:RegExp,escaped:boolean}} */ (
            internal.get(this)
        );
        let match = null;
        let lastIndex = 0;
...
function isHasSource(node) {
    return (
        ...
        /** @type {ImportDeclaration|ExportAllDeclaration|ExportNamedDeclaration} */ (node)
            .source != null
    );
}
const has =
...
class ReferenceTracker {
    ...
    constructor(globalScope, options = {}) {
        const { mode = "strict", globalObjectNames = ["global", "globalThis", "self", "window"] } =
            options;
    ...
    *iterateCjsReferences(traceMap) {
        for (const { node } of this.iterateGlobalReferences(requireCall)) {
            const key = getStringIfConstant(/** @type {CallExpression} */ (node).arguments[0]);
    ...
    *_iterateVariableReferences(variable, path, traceMap, shouldReport) {
        ...
        try {
            for (const reference of variable.references) {
                ...
                const node = /** @type {RuleNode & Identifier} */ (reference.identifier);
    ...
    *_iterateLhsReferences(patternNode, path, traceMap) {
        ...
        if (patternNode.type === "ObjectPattern") {
            for (const property of patternNode.properties) {
                const key = getPropertyName(/** @type {AssignmentProperty} */ (property));
    ...
    *_iterateImportReferences(specifierNode, path, traceMap) {
        ...
        if (type === "ImportSpecifier" || type === "ImportDefaultSpecifier") {
            ...
            yield* this._iterateVariableReferences(
                /** @type {Variable} */ (findVariable(this.globalScope, specifierNode.local)),
                path,
                nextTraceMap,
                false
            );
