        ...
        req.on("response", (res) => {
            ...
            if (codings === "gzip" || codings === "x-gzip") {
                ...
                response = new Response(
                    ...
                    // but just in case we proxy it to the stream in use.
                    body
                        .on("error", /* istanbul ignore next */ (er) => unzip.emit("error", er))
                        .pipe(unzip),
                    responseOptions
                );
                resolve(response);
            ...
            if (codings === "deflate" || codings === "x-deflate") {
                ...
                res.once("data", (chunk) => {
                    ...
                    const decoder =
                        (chunk[0] & 0x0f) === 0x08 ? new zlib.Inflate() : new zlib.InflateRaw();
                    // exceedingly rare that the stream would have an error,
                    // but just in case we proxy it to the stream in use.
                    body.on(
                        "error",
            ...
            if (codings === "br") {
                ...
                // exceedingly rare that the stream would have an error,
                // but just in case we proxy it to the stream in use.
                body.on("error", /* istanbul ignore next */ (er) => decoder.emit("error", er)).pipe(
                    decoder
                );
                response = new Response(decoder, responseOptions);
                resolve(response);
