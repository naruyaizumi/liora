...
test("level-change event", async (t) => {
    ...
    instance.once("level-change", (lvl, val, prevLvl, prevVal, logger) =>
        plan.equal(logger, instance)
    );
    instance.level = "info";
    const child = instance.child({});
    instance.once("level-change", (lvl, val, prevLvl, prevVal, logger) =>
        plan.equal(logger, child)
    );
    child.level = "trace";
...
test("exposed labels", async () => {
    assert.deepEqual(Object.keys(pino.levels.labels), ["10", "20", "30", "40", "50", "60"]);
});
...
test("fatal method should call async when sync-flushing fails", async (t) => {
    ...
    stream.flush = () => {
        throw Error("flush should be called");
    };
...
describe("changing level respects level comparison set to", () => {
    ...
    test("ASC in parent logger", async () => {
        ...
        const stream = sink();
        const logger = pino(
            { levelComparison, customLevels, useOnlyCustomLevels: true, level: "info" },
            stream
        );
