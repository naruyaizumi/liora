...
test("sends to multiple streams using number levels", async () => {
    ...
    const streams = [{ stream }, { level: 20, stream }, { level: 60, stream }];
    const log = pino(
        {
            level: "debug",
        },
...
test("supports grandchildren", async (t) => {
    ...
    const streams = [{ stream }, { level: "debug", stream }];
    const log = pino(
        {
            level: "debug",
        },
...
test("levelVal overrides level", async () => {
    ...
    const streams = [{ stream }, { level: "blabla", levelVal: 15, stream }, { level: 60, stream }];
    const log = pino(
        {
            level: "debug",
        },
