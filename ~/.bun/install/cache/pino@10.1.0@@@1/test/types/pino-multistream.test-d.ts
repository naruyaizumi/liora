...
expectType<pino.MultiStreamRes<"error">>(
    pino.multistream({ level: "error" as const, stream: process.stderr })
);
expectType<pino.MultiStreamRes<"fatal">>(
    pino.multistream([{ level: "fatal" as const, stream: createWriteStream("") }])
);

expectType<pino.MultiStreamRes<"error" | "fatal">>(pino.multistream(streams));
expectType<pino.MultiStreamRes<"error" | "fatal">>(pino.multistream(streams, {}));
expectType<pino.MultiStreamRes<"error" | "fatal">>(
    pino.multistream(streams, { levels: { info: 30 } })
);
