...
test("redact.censor option – can be a function that accepts value and path arguments", async () => {
    const stream = sink();
    const instance = pino(
        { redact: { paths: ["topLevel"], censor: (value, path) => value + " " + path.join(".") } },
        stream
    );
    instance.info({
        topLevel: "test",
...
test("redact.censor option – can be a function that accepts value and path arguments (nested path)", async () => {
    ...
    assert.equal(
        req.headers.cookie,
        "SESSID=298zf09hf012fh2; csrftoken=u32t4o3tb3gg43; _gat=1; req.headers.cookie"
    );
});
...
test("censor function should not be called for non-existent nested paths (issue #2313)", async () => {
    ...
    assert.equal(
        censorCalls.length,
        0,
        "censor should not be called when req.authorization does not exist"
    );
    ...
    assert.equal(
        censorCalls.length,
        0,
        "censor should not be called when neither a.b.c nor req.authorization exist"
    );
