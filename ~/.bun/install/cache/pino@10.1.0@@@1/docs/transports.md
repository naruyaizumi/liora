...
const pino = require("pino");
const transport = pino.transport({
    targets: [{ target: "/absolute/path/to/my-transport.mjs", level: "error" }],
    dedupe: true,
    sync: true,
});
pino(transport);
...
**NOTE: there is no "default" destination for a pipeline but
a terminating target, i.e. a `Writable` stream.**
