    ...
    constructor(definitions) {
        ...
        for (const key of Object.keys(definitions)) {
            ...
            if (typeof definitions[key].merge === "string") {
                definitions[key] = {
                    ...definitions[key],
                    merge: MergeStrategy[/** @type {string} */ (definitions[key].merge)],
                };
            }
            ...
            if (typeof definitions[key].validate === "string") {
                definitions[key] = {
                    ...definitions[key],
                    validate: ValidationStrategy[/** @type {string} */ (definitions[key].validate)],
                };
            }
            ...
            for (const [key, strategy] of this.#definitions) {
                try {
                    if (key in result || key in object) {
                        ...
                        const value = merge.call(this, result[key], object[key]);
                        if (value !== undefined) {
                            result[key] = value;
                        }
                    }
        ...
        for (const key of Object.keys(object)) {
            ...
            if (Array.isArray(definition.requires)) {
                if (!definition.requires.every((otherKey) => otherKey in object)) {
                    throw new MissingDependentKeysError(key, definition.requires);
                }
            }
