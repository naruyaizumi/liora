...
class ConfigCommentParser {
    ...
    parseStringConfig(string) {
        ...
        const trimmedString = string.trim().replace(/(?<!\s)\s*([:,])\s*/gu, "$1");
    ...
    parseJSONLikeConfig(string) {
        ...
        try {
            const items = /** @type {RulesConfig} */ (levn.parse("Object", string)) || {};
    ...
    parseDirective(string) {
        ...
        return new DirectiveComment(directiveText, directiveValue.trim(), justificationPart);
    }
}
...
class TextSourceCodeBase {
    ...
    getLoc(nodeOrToken) {
        ...
        throw new Error("Custom getLoc() method must be implemented in the subclass.");
    }
    ...
    getRange(nodeOrToken) {
        ...
        if (hasPosStyleRange(nodeOrToken)) {
            return [nodeOrToken.position.start.offset, nodeOrToken.position.end.offset];
        }

        throw new Error("Custom getRange() method must be implemented in the subclass.");
    }
    ...
    getAncestors(node) {
        ...
        for (let ancestor = this.getParent(node); ancestor; ancestor = this.getParent(ancestor)) {
