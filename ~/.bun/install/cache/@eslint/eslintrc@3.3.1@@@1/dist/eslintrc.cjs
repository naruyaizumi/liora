...
function createContext(
    ...
    const matchBasePath =
        ...
        cwd;
    const name =
        providedName || (filePath && path__default["default"].relative(cwd, filePath)) || "";
    const pluginBasePath =
        resolvePluginsRelativeTo || (filePath && path__default["default"].dirname(filePath)) || cwd;
...
function translateESLintRC(
    eslintrcConfig,
    { resolveConfigRelativeTo, resolvePluginsRelativeTo, pluginEnvironments, pluginProcessors }
) {
    const flatConfig = {};
    const configs = [];
