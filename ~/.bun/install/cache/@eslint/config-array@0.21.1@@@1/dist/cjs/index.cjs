...
function normalizeConfigPatterns(config, namespacedBasePath, path) {
    ...
    if (hasBasePath) {
        if (path.isAbsolute(config.basePath)) {
            ...
            newConfig.basePath = path.resolve(namespacedBasePath, config.basePath);
        }
    }
...
class ConfigArray extends Array {
    ...
    constructor(
        configs,
        { basePath = "/", normalized = false, schema: customSchema, extraConfigTypes = [] } = {}
    ) {
        super();
    ...
    async normalize(context = {}) {
        if (!this.isNormalized()) {
            ...
            this.length = 0;
            this.push(
                ...normalizedConfigs.map(this[ConfigArraySymbol.preprocessConfig].bind(this))
            );
        ...
        this.forEach((config, index) => {
            ...
            const universalFiles = config.files.filter((element) => {
                if (Array.isArray(element)) {
                     ...
                     */
                    if (element.every((pattern) => universalPattern.test(pattern))) {
                        return true;
                    }
