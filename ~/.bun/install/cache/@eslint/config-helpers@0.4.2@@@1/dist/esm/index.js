...
function normalizePluginConfig(userNamespace, plugin, config) {
    ...
    if (typeof result.processor === "string") {
        const { namespace: processorNamespace, name: processorName } = getPluginMember(
            result.processor
        );
    ...
    if (typeof result.language === "string") {
        const { namespace: languageNamespace, name: languageName } = getPluginMember(
            result.language
        );
...
function findPluginConfig(config, pluginConfigName) {
    ...
    if (flatConfig && (Array.isArray(flatConfig) || !isLegacyConfig(flatConfig))) {
        return deepNormalizePluginConfig(userPluginNamespace, plugin, flatConfig, pluginConfigName);
    }
...
function processExtends(config, configNames) {
        ...
        if (typeof extendsElement === "string") {
            ...
            if (Array.isArray(pluginConfig)) {
                pluginConfig.forEach((pluginConfigElement, index) => {
                    extensionNames.set(pluginConfigElement, `${extendsElement}[${index}]`);
                });
            } else {
    ...
    for (const { indexPath, value: extendsElement } of flatTraverse(objectExtends)) {
        ...
        const extensionName =
            extensionNames.get(extendsElement) ?? getExtensionName(extendsElement, indexPath);

        result.push(extendConfig(configObject, baseConfigName, extension, extensionName));
    }
