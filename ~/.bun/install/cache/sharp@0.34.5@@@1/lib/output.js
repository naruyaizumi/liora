...
function toFile(fileOut, callback) {
    ...
    if (!is.string(fileOut)) {
        ...
        is.string(this.options.input.file) &&
        path.resolve(this.options.input.file) === path.resolve(fileOut)
    ) {
...
function toFormat(format, options) {
    const actualFormat = formats.get(
        (is.object(format) && is.string(format.id) ? format.id : format).toLowerCase()
    );
    if (!actualFormat) {
        throw is.invalidParameterError(
            "format",
...
function jpeg(options) {
    if (is.object(options)) {
        ...
        if (is.defined(options.chromaSubsampling)) {
            if (
                is.string(options.chromaSubsampling) &&
                is.inArray(options.chromaSubsampling, ["4:2:0", "4:4:4"])
            ) {
        ...
        const optimiseCoding = is.bool(options.optimizeCoding)
        ...
        const trellisQuantisation = is.bool(options.trellisQuantization)
        ...
        const optimiseScans = is.bool(options.optimizeScans)
        ...
        const quantisationTable = is.number(options.quantizationTable)
...
function png(options) {
    if (is.object(options)) {
        ...
        if (is.defined(options.compressionLevel)) {
            if (
                is.integer(options.compressionLevel) &&
                is.inRange(options.compressionLevel, 0, 9)
            ) {
...
function webp(options) {
    if (is.object(options)) {
        ...
        if (is.defined(options.preset)) {
            if (
                is.string(options.preset) &&
                is.inArray(options.preset, [
                    "default",
                    "photo",
                    "picture",
                    "drawing",
...
function gif(options) {
    if (is.object(options)) {
        ...
        if (is.defined(options.interFrameMaxError)) {
            if (
                is.number(options.interFrameMaxError) &&
                is.inRange(options.interFrameMaxError, 0, 32)
            ) {
        ...
        if (is.defined(options.interPaletteMaxError)) {
            if (
                is.number(options.interPaletteMaxError) &&
                is.inRange(options.interPaletteMaxError, 0, 256)
            ) {
...
function tiff(options) {
    if (is.object(options)) {
        ...
        if (is.defined(options.compression)) {
            if (
                is.string(options.compression) &&
                is.inArray(options.compression, [
                    "none",
                    "jpeg",
                    "deflate",
                    "packbits",
        ...
        if (is.defined(options.predictor)) {
            if (
                is.string(options.predictor) &&
                is.inArray(options.predictor, ["none", "horizontal", "float"])
            ) {
        ...
        if (is.defined(options.resolutionUnit)) {
            if (
                is.string(options.resolutionUnit) &&
                is.inArray(options.resolutionUnit, ["inch", "cm"])
            ) {
            ...
            if (is.integer(options.quality) && is.inRange(options.quality, 1, 100)) {
                ...
                this.options.jxlDistance =
                    options.quality >= 30
                        ...
                        : (53 / 3000) * options.quality * options.quality -
                          (23 / 20) * options.quality +
                          25;
            } else {
                throw is.invalidParameterError(
                    "quality",
...
function raw(options) {
    if (is.object(options)) {
        if (is.defined(options.depth)) {
            if (
                is.string(options.depth) &&
                is.inArray(options.depth, [
                    "char",
                    "uchar",
                    "short",
                    "ushort",
...
function tile(options) {
    if (is.object(options)) {
        ...
        if (is.defined(options.layout)) {
            if (
                is.string(options.layout) &&
                is.inArray(options.layout, ["dz", "google", "iiif", "iiif3", "zoomify"])
            ) {
        ...
        if (is.defined(options.depth)) {
            if (
                is.string(options.depth) &&
                is.inArray(options.depth, ["onepixel", "onetile", "one"])
            ) {
