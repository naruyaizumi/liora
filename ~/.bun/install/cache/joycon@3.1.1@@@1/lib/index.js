...
class JoyCon {
    constructor({ files, cwd = process.cwd(), stopDir, packageKey, parseJSON = JSON.parse } = {}) {
        this.options = {
            files,
            cwd,
            stopDir,
    ...
    async recusivelyResolve(options) {
        if (
            ...
            _path.default.basename(options.cwd) === "node_modules"
        ) {
            return null;
        }
    ...
    normalizeOptions(args) {
        ...
        options.stopDir = options.stopDir
            ? _path.default.resolve(options.stopDir)
            : _path.default.parse(options.cwd).root;
