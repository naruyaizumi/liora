...
class TrackedFinalizer extends RefTracker {
    static create(envObject, finalize_callback, finalize_data, finalize_hint) {
        const finalizer = new TrackedFinalizer(
            envObject,
            finalize_callback,
            finalize_data,
            finalize_hint
...
class NotSupportWeakRefError extends EmnapiError {
    constructor(api, message) {
        super(
            `${api}: The current runtime does not support "FinalizationRegistry" and "WeakRef".${message ? ` ${message}` : ""}`
        );
    }
}
...
class NotSupportBufferError extends EmnapiError {
    constructor(api, message) {
        super(
            `${api}: The current runtime does not support "Buffer". Consider using buffer polyfill to make sure \`globalThis.Buffer\` is defined.${message ? ` ${message}` : ""}`
        );
    }
}
...
class CleanupQueue {
    ...
    add(envObject, fn, arg) {
        if (
            this._cleanupHooks.filter(
                (hook) => hook.envObject === envObject && hook.fn === fn && hook.arg === arg
            ).length > 0
        ) {
