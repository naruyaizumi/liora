    ...
    readData(uint8Array, type, includeCovers) {
        ...
        switch (type !== "TXXX" && type[0] === "T" ? "T*" : type) {
            ...
            case "PCST": {
                ...
                try {
                    text = util
                        .decodeString(uint8Array.subarray(1), encoding)
                        .replace(/\x00+$/, "");
                } catch (error) {
            ...
            case "TXXX": {
                ...
                const textTag = {
                    ...
                    text: this.splitValue(
                        type,
                        util.decodeString(idAndData.data, encoding).replace(/\x00+$/, "")
                    ),
                };
                output = textTag;
                break;
                    ...
                    switch (this.major) {
                        case 2:
                            pic.format = util.decodeString(
                                uint8Array.subarray(offset, offset + 3),
                                "latin1"
                        ...
                        case 4:
                            ...
                            pic.format = util.decodeString(
                                uint8Array.subarray(offset, fzero),
                                defaultEnc
                            );
                            offset = fzero + 1;
                            break;
            ...
            case "POPM": {
                ...
                output = {
                    ...
                    counter:
                        valueLen > 0
                            ? util.decodeUintBE(uint8Array.subarray(offset + 1))
                            : undefined,
                };
                break;
            }
            case "GEOB": {
                ...
                const mimeType = util.decodeString(
                    uint8Array.subarray(offset + 1, fzero),
                    defaultEnc
                );
                offset = fzero + 1;
                fzero = util.findZero(uint8Array, offset, length, encoding);
            ...
            case "WFED":
                output = util.decodeString(
                    uint8Array.subarray(
                        offset + 1,
                        util.findZero(uint8Array, offset + 1, length, encoding)
                    ),
                    encoding
                );
                break;
