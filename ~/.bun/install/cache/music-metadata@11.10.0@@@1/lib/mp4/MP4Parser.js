...
export class MP4Parser extends BasicParser {
        ...
        this.atomParsers = {
            ...
            mdat: async (len) => {
                ...
                if (this.options.includeChapters) {
                    const trackWithChapters = [...this.tracks.values()].filter(
    ...
    static read_BE_Integer(array, signed) {
        const integerType =
            (signed ? "INT" : "UINT") + array.length * 8 + (array.length > 1 ? "_BE" : "");
        ...
        if (audioTracks.length >= 1) {
            ...
            if (audioTrack.media.header && audioTrack.media.header.timeScale > 0) {
                if (audioTrack.media.header.duration > 0) {
                    ...
                    const duration =
                        audioTrack.media.header.duration / audioTrack.media.header.timeScale; // calculate duration in seconds
                    ...
                    for (const fragment of audioTrack.fragments) {
                        ...
                        for (const sample of fragment.trackRun.samples) {
                            ...
                            if (dur == null) {
                                throw new Error(
                                    "Missing sampleDuration and no default_sample_duration in tfhd"
                                );
                            }
                            totalTimeUnits += dur;
                        }
                    ...
                    this.metadata.setFormat(
                        "duration",
                        totalTimeUnits / audioTrack.media.header.timeScale
                    );
                }
            }
                ...
                switch (child.header.name) {
                    ...
                    default: {
                        const uint8Array = await this.tokenizer.readToken(
                            new Token.Uint8ArrayType(payLoadLength)
                        );
                        this.addWarning(
    ...
    async parseValueAtom(tagKey, metaAtom) {
        const dataAtom = await this.tokenizer.readToken(
            new AtomToken.DataAtom(Number(metaAtom.header.length) - AtomToken.Header.len)
        );
    ...
    async parseTrackBox(trakBox) {
        ...
        await trakBox.readAtoms(
            ...
            async (child, remaining) => {
                ...
                switch (child.header.name) {
                    case "chap": {
                        const chap = await this.tokenizer.readToken(
                            new ChapterTrackReferenceBox(remaining)
                        );
                        track.chapterList = chap;
                        break;
                    }
                    case "tkhd": // TrackHeaderBox
                        track.header = await this.tokenizer.readToken(
                            new AtomToken.TrackHeaderAtom(payLoadLength)
                        );
                        break;
                    case "hdlr": // TrackHeaderBox
                        track.handler = await this.tokenizer.readToken(
                            new AtomToken.HandlerBox(payLoadLength)
                        );
                    ...
                    case "mdhd": {
                        ...
                        const mdhd_data = await this.tokenizer.readToken(
                            new AtomToken.MdhdAtom(payLoadLength)
                        );
                        track.media.header = mdhd_data;
                        break;
                    }
                    case "stco": {
                        const stco = await this.tokenizer.readToken(
                    ...
                    case "stsc": {
                        ...
                        const stsc = await this.tokenizer.readToken(
                            new AtomToken.StscAtom(payLoadLength)
                        );
                    ...
                    case "stsd": {
                        ...
                        const stsd = await this.tokenizer.readToken(
                            new AtomToken.StsdAtom(payLoadLength)
                        );
                        track.soundSampleDescription = stsd.table.map((dfEntry) =>
                            this.parseSoundSampleDescription(dfEntry)
                        );
                        break;
                    }
                    case "stts": {
                        ...
                        const stts = await this.tokenizer.readToken(
                            new AtomToken.SttsAtom(payLoadLength)
                        );
                        track.timeToSampleTable = stts.entries;
                        break;
                    }
                    case "stsz": {
                        const stsz = await this.tokenizer.readToken(
                            new AtomToken.StszAtom(payLoadLength)
                        );
            ...
            async (child, remaining) => {
                ...
                switch (child.header.name) {
                    case "tfhd": {
                        ...
                        const fragmentHeaderBox = new AtomToken.TrackFragmentHeaderBox(
                            child.getPayloadLength(remaining)
                        );
    ...
    async parseChapterTrack(chapterTrack, track, len) {
        ...
        for (let i = 0; i < chapterTrack.chunkOffsetTable.length && len > 0; ++i) {
            ...
            debug(
                `Chapter title=${chapter.title}, offset=${chapter.sampleOffset}/${track.header.duration}`
            ); // ToDo, use media duration if required!!!
