    ...
    get: (uint8array, off) => {
        return {
            ...
            originator: stripNulls(
                new Token.StringType(32, "ascii").get(uint8array, off + 256)
            ).trim(),
            originatorReference: stripNulls(
                new Token.StringType(32, "ascii").get(uint8array, off + 288)
            ).trim(),
            originationDate: stripNulls(
                new Token.StringType(10, "ascii").get(uint8array, off + 320)
            ).trim(),
            originationTime: stripNulls(
                new Token.StringType(8, "ascii").get(uint8array, off + 330)
            ).trim(),
