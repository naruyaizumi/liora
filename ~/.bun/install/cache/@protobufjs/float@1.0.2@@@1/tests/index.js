...
function runTest(float, test) {
    var common = [0, -0, Infinity, -Infinity, 0.125, 1024.5, -4096.5, NaN];

    test.test(test.name + " - using 32 bits", function (test) {
        common
            .concat([3.4028234663852886e38, 1.1754943508222875e-38, 1.1754946310819804e-39])
    ...
    test.test(test.name + " - using 64 bits", function (test) {
        common
            .concat([1.7976931348623157e308, 2.2250738585072014e-308, 2.2250738585072014e-309])
            .forEach(function (value) {
                var strval = value === 0 && 1 / value < 0 ? "-0" : value.toString();
                test.ok(
                    checkValue(
...
function checkValue(value, size, read, write, write_comp) {
    ...
    var strval = value === 0 && 1 / value < 0 ? "-0" : value.toString();
    if (value !== value) {
        if (value_comp === value_comp) return false;
    } else if (value_comp !== value) return false;
