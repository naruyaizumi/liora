...
tape.test("eventemitter", function (test) {
    ...
    test.equal(
        ...
        ee,
        "should return itself when registering events"
    );
    ee.emit("a", 1);
    ...
    test.same(
        ee._listeners,
        { a: [] },
        "should remove all listeners of the respective event when calling off(evt)"
    );
    ...
    test.same(
        ee._listeners,
        { a: [] },
        "should remove the exact listener when calling off(evt, fn)"
    );
