...
var require_ci_info = __commonJS({
    "node_modules/ci-info/index.js"(exports) {
        ...
        exports.isCI = !!(
            env3.CI !== "false" && // Bypass all checks if CI env is explicitly set to 'false'
...
// src/cli/options/get-context-options.js
var detailedCliOptions = normalizeOptionSettings(cli_options_evaluate_default).map((option) =>
...
function parseArgv(rawArguments, detailedOptions, logger, keys2) {
    ...
    if (keys2) {
        detailedOptions = detailedOptions.filter((option) => keys2.includes(option.name));
...
async function logFileInfoOrDie(context) {
    ...
    const result = await format((0, import_fast_json_stable_stringify.default)(fileInfo), {
        parser: "json",
    });
...
var DirectoryIgnorer = class {
    ...
    shouldIgnore(absolutePathOrPattern) {
        ...
        return directoryNames.some((directoryName) =>
            __privateGet(this, _directories).has(directoryName)
        );
    }
};
...
function findUpSync(name, { cwd: cwd3 = process2.cwd(), type = "file", stopAt } = {}) {
    ...
    while (directory) {
        ...
        try {
            ...
            if (
                (type === "file" && (stats == null ? void 0 : stats.isFile())) ||
                (type === "directory" && (stats == null ? void 0 : stats.isDirectory()))
            ) {
                return filePath;
            }
        } catch {}
...
function getNodeModuleDirectory(directory) {
    ...
    if (
        ...
        (fs3.existsSync(nodeModules) || !isWritable(path8.join(directory)))
    ) {
        return;
    }
    return nodeModules;
...
function findCacheDirectory(options = {}) {
    ...
    if (files) {
        ...
        directory = (0, import_common_path_prefix.default)(
            files.map((file) => path8.resolve(directory, file))
        );
    }
...
var n = class {
    ...
    constructor(e) {
        ((this._eventListeners = /* @__PURE__ */ new Map()),
            ...
            (this._logger = e == null ? void 0 : e.logger));
    }
    once(e, t) {
        let s = (...r) => {
            (this.off(e, s), t(...r));
        ...
        return (
            s &&
                (s.length >= this._maxListeners &&
                    console.warn(
                        `MaxListenersExceededWarning: Possible event memory leak detected. ${s.length + 1} ${e} listeners added. Use setMaxListeners() to increase limit.`
                    ),
                s.push(t)),
            this
        );
...
var l = class extends n {
    ...
    constructor(e) {
        (super({ logger: e == null ? void 0 : e.logger }),
            (this._hooks = /* @__PURE__ */ new Map()),
            (e == null ? void 0 : e.throwHookErrors) !== void 0 &&
                (this._throwHookErrors = e.throwHookErrors));
    }
    get hooks() {
        return this._hooks;
    }
        ...
        if (s)
            for (let r of s)
                try {
                    ...
                    let o = `${e}: ${i.message}`;
                    if (
                        (this.emit("error", new Error(o)),
                        this._logger && this._logger.error(o),
                        this._throwHookErrors)
                    )
                        throw new Error(o);
                }
    }
...
var FileEntryCache = class {
    ...
    removeEntry(filePath, options) {
        if (this.isRelativePath(filePath)) {
            filePath = this.getAbsolutePath(filePath, {
                currentWorkingDirectory: options == null ? void 0 : options.currentWorkingDirectory,
            });
    ...
    getFileDescriptor(filePath, options) {
        ...
        const useModifiedTimeValue =
            (options == null ? void 0 : options.useModifiedTime) ?? this._useModifiedTime;
        try {
...
var FormatResultsCache = class {
    ...
    existsAvailableFormatResultsCache(filePath, options) {
        var _a;
        const fileDescriptor = __privateMethod(
            this,
            _FormatResultsCache_instances,
            getFileDescriptor_fn
        ).call(this, filePath);
...
function getOptions(argv2, detailedOptions) {
    return Object.fromEntries(
        detailedOptions
            ...
            .map(({ forwardToApi, name }) => [forwardToApi, argv2[name]])
    );
}
...
function createApiDetailedOptionMap(detailedOptions) {
    return Object.fromEntries(
        detailedOptions
            .filter((option) => option.forwardToApi && option.forwardToApi !== option.name)
...
async function format3(context, input, opt) {
    ...
    if (context.argv.debugPrintComments) {
        return {
            formatted: await prettier.format(
                JSON.stringify((await prettier.formatWithCursor(input, opt)).comments || []),
                { parser: "json" }
            ),
        };
    }
    ...
    if (context.argv.debugCheck) {
        ...
        if (pp !== pppp) {
            ...
            const past = stringify5((await prettier.__debug.parse(pp, opt, { massage: true })).ast);
            if (ast !== past) {
...
${error2.message}`
    ...
    if (context.argv.check) {
        if (numberOfFilesWithError > 0) {
            const files =
                numberOfFilesWithError === 1 ? "the above file" : `${numberOfFilesWithError} files`;
            context.logger.log(`Error occurred when checking code style in ${files}.`);
...
var isStderrColorSupported =
    !(Boolean(env2.NO_COLOR) || argv.includes("--no-color")) &&
    (Boolean(env2.FORCE_COLOR) ||
        argv.includes("--color") ||
        process.platform === "win32" ||
        (process.stderr.isTTY && env2.TERM !== "dumb") ||
        Boolean(env2.CI));
...
async function printSupportInfo() {
    ...
    const result = await format4((0, import_fast_json_stable_stringify3.default)(supportInfo), {
        parser: "json",
    });
    printToScreen(result.trim());
}
...
function createChoiceUsages(choices, margin, indentation) {
    ...
    return activeChoices.map((choice) =>
        indent(createOptionUsageRow(choice.value, choice.description, threshold), indentation)
    );
}
...
function createPluginDefaults(pluginDefaults) {
    ...
    const defaults = Object.entries(pluginDefaults)
        .sort(([pluginNameA], [pluginNameB]) => pluginNameA.localeCompare(pluginNameB))
        .map(([plugin, value]) => `* ${plugin}: ${createDefaultValueDisplay(value)}`)
        .join("\n");
    return `
Plugin defaults:
${defaults}`;
...
async function run(rawArguments = process.argv.slice(2)) {
    ...
    try {
        const { logLevel } = parseArgvWithoutPlugins(rawArguments, logger, "log-level");
...
async function main(context) {
    ...
    const useStdin =
        !hasFilePatterns && (!mockable_default.isStreamTTY(process.stdin) || context.argv.filepath);
    if (useStdin) {
        if (context.argv.cache) {
