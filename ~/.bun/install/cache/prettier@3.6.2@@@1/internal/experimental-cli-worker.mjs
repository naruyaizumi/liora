...
async function writeFileAsync(filePath, data, options = DEFAULT_WRITE_OPTIONS) {
    ...
    try {
        ...
        fd = await dist_default.retry.open(timeout)(
            tempPath,
            "w",
        ...
        if (isString(data)) {
            await dist_default.retry.write(timeout)(
                fd,
                data,
                0,
        ...
        if (fd) await dist_default.attempt.close(fd);
...
var memoize = (fn, resolver) => {
    const memoized = function (...args) {
        ...
        if (cached !== void 0 || cache2.has(key)) return cached;
    ...
    return (state) => {
        ...
        if (indexEnd > indexStart) {
            if (!isUndefined3(handler) && !state.options.silent) {
                ...
                const output = isFunction3(handler)
...
var RangeNumeric = match(/(\d+)\.\.(\d+)/, (_, $1, $2) =>
    makeRangePaddedInt(+$1, +$2, Math.min($1.length, $2.length)).join("|")
);
var RangeAlphaLower = match(/([a-z]+)\.\.([a-z]+)/, (_, $1, $2) =>
    makeRangeAlpha($1, $2).join("|")
);
var RangeAlphaUpper = match(/([A-Z]+)\.\.([A-Z]+)/, (_, $1, $2) =>
...
async function check(
    ...
    const fileContentFormatted = await format2(
        filePath,
        fileContent,
        formatOptions,
        contextOptions,
...
export { check, checkWithPath, format2 as format, formatWithPath, write, writeWithPath };
