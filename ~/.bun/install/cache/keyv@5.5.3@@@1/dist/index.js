        ...
        if (eventHandlers) {
            for (const handler of eventHandlers) {
                try {
                    ...
                    this.emit(
                        "error",
                        new Error(`Error in hook handler for event "${event}": ${error.message}`)
                    );
                }
            }
        }
...
var iterableAdapters = ["sqlite", "postgres", "mysql", "mongo", "redis", "valkey", "etcd"];
var Keyv = class extends event_manager_default {
    ...
    _checkIterableAdapter() {
        return (
            iterableAdapters.includes(this._store.opts.dialect) ||
            iterableAdapters.some((element) => this._store.opts.url.includes(element))
        );
    }
    _getKeyPrefix(key) {
    ...
    async get(key, options) {
        ...
        const deserializedData =
            typeof rawData === "string" || this.opts.compression
                ? await this.deserializeData(rawData)
                : rawData;
        ...
        if (store.getMany === void 0) {
            const promises = keyPrefixed.map(async (key) => {
                ...
                const deserializedRow =
                    typeof rawData2 === "string" || this.opts.compression
                        ? await this.deserializeData(rawData2)
                        : rawData2;
    ...
    async getRaw(key) {
        ...
        const deserializedData =
            typeof rawData === "string" || this.opts.compression
                ...
                : rawData;
        if (
            deserializedData !== void 0 &&
            deserializedData.expires !== void 0 &&
            deserializedData.expires !== null && // biome-ignore lint/style/noNonNullAssertion: need to fix
...
export { Keyv, KeyvHooks, index_default as default };
