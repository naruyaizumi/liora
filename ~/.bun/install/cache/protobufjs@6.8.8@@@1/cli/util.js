...
exports.requireAll = function requireAll(dirname) {
    ...
    files.forEach(function (file) {
        ...
        if (extname === ".js") all[basename] = require(path.join(dirname, file));
    });
    return all;
};
...
exports.traverseResolved = function traverseResolved(current, fn) {
    fn(current);
    if (current.resolvedType) traverseResolved(current.resolvedType, fn);
    if (current.resolvedKeyType) traverseResolved(current.resolvedKeyType, fn);
    if (current.resolvedRequestType) traverseResolved(current.resolvedRequestType, fn);
    if (current.resolvedResponseType) traverseResolved(current.resolvedResponseType, fn);
};
...
exports.inspect = function inspect(object, indent) {
    ...
    if (object instanceof protobuf.Field) {
        if (object.extend !== undefined) sb.push(indent + chalk.gray("extend: ") + object.extend);
        if (object.partOf) sb.push(indent + chalk.gray("oneof : ") + object.oneof);
    }
    sb.push("");
...
function modInstall(install) {
    child_process.execSync(
        "npm --silent install " + (typeof install === "string" ? install : install.join(" ")),
        {
            cwd: __dirname,
            stdio: "ignore",
        }
...
exports.wrap = function (OUTPUT, options) {
    ...
    wrap = wrap.replace(/( *)\$OUTPUT;/, function ($0, $1) {
        return $1.length ? OUTPUT.replace(/^/gm, $1) : OUTPUT;
    });
    if (options.lint !== "") wrap = "/*" + options.lint + "*/\n" + wrap;
