...
function buildRoot(root) {
    ...
    do {
        ...
        if (
            nested.length === 1 &&
            nested[0] instanceof Namespace &&
            !(nested[0] instanceof Type || nested[0] instanceof Service)
        ) {
            ptr = nested[0];
            if (ptr !== root) pkg.push(ptr.name);
        } else repeat = false;
    } while (repeat);
...
function build(object) {
    if (object instanceof Enum) buildEnum(object);
    else if (object instanceof Type) buildType(object);
    else if (object instanceof Field) buildField(object);
    else if (object instanceof OneOf) buildOneOf(object);
    else if (object instanceof Service) buildService(object);
    else if (object instanceof Method) buildMethod(object);
    else buildNamespace(object);
}

function buildNamespace(namespace) {
    // just a namespace, not a type etc.
    push("");
...
function buildRanges(keyword, ranges) {
    if (ranges && ranges.length) {
        ...
        ranges.forEach(function (range) {
            if (typeof range === "string") parts.push('"' + escape(range) + '"');
            else if (range[0] === range[1]) parts.push(range[0]);
            else parts.push(range[0] + " to " + (range[1] === 0x1fffffff ? "max" : range[1]));
        });
        push("");
...
function buildType(type) {
    if (type.group) return; // built with the sister-field
    push("");
...
function buildField(field, passExtend) {
    if (field.partOf || field.declaringField || (field.extend !== undefined && !passExtend)) return;
    if (first) {
        first = false;
        push("");
    }
...
function buildFieldOptions(field) {
    var keys;
    if (!field.options || !(keys = Object.keys(field.options)).length) return null;
    var sb = [];
    keys.forEach(function (key) {
        ...
        switch (key) {
            case "packed":
                ...
                if (wireType === undefined || (syntax === 3) === val) return;
                break;
            case "default":
                if (syntax === 3) return;
                // skip default (resolved) default values
                if (
                    (field.long && !util.longNeq(field.defaultValue, types.defaults[field.type])) ||
                    (!field.long && field.defaultValue === types.defaults[field.type])
                )
                    return;
                // enum defaults specified as strings are type references and not enclosed in quotes
                if (field.resolvedType instanceof Enum) break;
...
function consolidateExtends(nested) {
    ...
    nested = nested.filter(function (obj) {
        if (!(obj instanceof Field) || obj.extend === undefined) return true;
