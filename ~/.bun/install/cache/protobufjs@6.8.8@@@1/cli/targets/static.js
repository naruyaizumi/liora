...
function static_target(root, options, callback) {
    ...
    try {
        var aliases = [];
        if (config.decode) aliases.push("Reader");
        if (config.encode) aliases.push("Writer");
        aliases.push("util");
        if (aliases.length) {
...
function exportName(object, asInterface) {
    if (asInterface) {
        if (object.__interfaceName) return object.__interfaceName;
    } else if (object.__exportName) return object.__exportName;
    var parts = object.fullName.substring(1).split("."),
        i = 0;
    while (i < parts.length) parts[i] = escapeName(parts[i++]);
    if (asInterface) parts[i - 1] = "I" + parts[i - 1];
...
function aOrAn(name) {
    return (
        ((/^[hH](?:ou|on|ei)/.test(name) || /^[aeiouAEIOU][a-z]/.test(name)) && !/^us/i.test(name)
            ? "an "
            : "a ") + name
    );
}
...
function buildNamespace(ref, ns) {
    ...
    if (ns.name !== "") {
        ...
        else push(escapeName(ref) + "." + escapeName(ns.name) + " = (function() {");
        ++indent;
    }

    if (ns instanceof Type) {
        buildType(undefined, ns);
    } else if (ns instanceof Service) buildService(undefined, ns);
    else if (ns.name !== "") {
        push("");
        pushComment([
            ns.comment || "Namespace " + ns.name + ".",
            ns.parent instanceof protobuf.Root
                ? "@exports " + escapeName(ns.name)
                : "@memberof " + exportName(ns.parent),
            "@namespace",
        ]);
    ...
    ns.nestedArray.forEach(function (nested) {
        if (nested instanceof Enum) buildEnum(ns.name, nested);
        else if (nested instanceof Namespace) buildNamespace(ns.name, nested);
    });
    if (ns.name !== "") {
        push("");
...
function buildFunction(type, functionName, gen, scope) {
    var code = gen.toString(functionName).replace(/((?!\.)types\[\d+])(\.values)/g, "$1"); // enums: use types[N] instead of reflected types[N].values
    ...
    if (config.beautify) code = beautifyCode(code);
    ...
    if (hasScope)
        ...
        Object.keys(scope).forEach(function (key) {
            if (!new RegExp("\\b(" + key + ")\\b", "g").test(code)) delete scope[key];
        });
    ...
    if (isCtor)
        ...
        push(
            escapeName(type.name) +
                ...
                " = (function(" +
                Object.keys(scope).map(escapeName).join(", ") +
                ") { return " +
                lines[0]
        );
...
function toJsType(field) {
    ...
    switch (field.type) {
        ...
        default:
            ...
            else type = "*"; // should not happen
            break;
    }
    if (field.map) return "Object.<string," + type + ">";
    if (field.repeated) return "Array.<" + type + ">";
    return type;
}
...
function buildType(ref, type) {
    if (config.comments) {
        var typeDef = [
            ...
            type.parent instanceof protobuf.Root
                ? "@exports " + escapeName("I" + type.name)
                : "@memberof " + exportName(type.parent),
    ...
    type.fieldsArray.forEach(function (field) {
        ...
        if (config.comments) {
            ...
            if (
                ...
                !field.map &&
                !field.repeated &&
                field.resolvedType instanceof Type
            )
        ...
        if (field.repeated)
            ...
            push(
                escapeName(type.name) +
                    ...
                    prop +
                    " = " +
                    JSON.stringify(field.typeDefault) +
                    ";"
            );
    });
    ...
    type.oneofsArray.forEach(function (oneof) {
        if (firstOneOf) {
            firstOneOf = false;
            push("");
            if (config.comments) push("// OneOf field names bound to virtual getters and setters");
        ...
        pushComment([
            ...
            "@member {" +
                oneof.oneof.map(JSON.stringify).join("|") +
                "|undefined} " +
    ...
    if (config.encode) {
        ...
        pushComment([
            ...
            "@param {$protobuf.Writer} [" +
                (config.beautify ? "writer" : "w") +
                "] Writer to encode to",
        ...
        if (config.delimited) {
            ...
            push(
                escapeName(type.name) +
                    ".encodeDelimited = function encodeDelimited(message, writer) {"
            );
            ++indent;
    ...
    if (config.convert) {
        ...
        pushComment([
            ...
            "@param {" +
                ...
                " " +
                type.name,
            "@param {$protobuf.IConversionOptions} [" +
                (config.beautify ? "options" : "o") +
                "] Conversion options",
...
function buildService(ref, service) {
    ...
    pushComment([
        ...
        service.parent instanceof protobuf.Root
            ? "@exports " + escapeName(service.name)
    ...
    push(
        "function " + escapeName(service.name) + "(rpcImpl, requestDelimited, responseDelimited) {"
    );
    ++indent;
    ...
    if (config.create) {
        ...
        push(
            escapeName(service.name) +
                ".create = function create(rpcImpl, requestDelimited, responseDelimited) {"
        );
        ++indent;
...
function buildEnum(ref, enm) {
    ...
    var comment = [
        ...
        enm.parent instanceof protobuf.Root
            ? "@exports " + escapeName(enm.name)
            : "@name " + exportName(enm),
    ...
    Object.keys(enm.values).forEach(function (key) {
        ...
        comment.push(
            (config.forceEnumString ? "@property {string} " : "@property {number} ") +
                key +
                "=" +
                val +
                " " +
                (enm.comments[key] || key + " value")
        );
    });
    pushComment(comment);
    ...
    Object.keys(enm.values).forEach(function (key) {
        ...
        if (aliased.indexOf(valueId) > -1)
            ...
            push(
                "values[valuesById[" + valueId + "] = " + JSON.stringify(key) + "] = " + val + ";"
            );
