...
exports.publish = function publish(taffy, opts) {
    ...
    // remove private
    if (!options.private) taffy({ access: "private" }).remove();

    // setup output
    out = options.destination ? fs.createWriteStream(options.destination) : process.stdout;
...
var keepTags = ["param", "returns", "throws", "see"];
...
function parseComment(comment) {
    ...
    for (var i = 0; i < lines.length; ++i) {
        ...
        if (lines[i].length || text.length) text.push(lines[i]);
    }
    return {
        text: desc || text,
        tags: tags || [],
...
function writeComment(comment, otherwiseNewline) {
    if (!comment || options.comments === false) {
        if (otherwiseNewline) writeln();
        return;
    }
    if (typeof comment !== "object") comment = parseComment(comment);
    comment.tags = comment.tags.filter(function (tag) {
        return (
            keepTags.indexOf(tag.name) > -1 &&
            (tag.name !== "returns" || tag.text[0] !== "{undefined}")
        );
    });
    writeln();
    ...
    comment.tags.forEach(function (tag) {
        ...
        if (tag.text.length) {
            tag.text.forEach(function (line, i) {
                if (i > 0) write(" * ");
                else if (tag.name !== "throws") line = line.replace(/^\{[^\s]*} ?/, "");
...
function getTypeOf(element) {
    if (element.tsType) return element.tsType.replace(/\r?\n|\r/g, "\n");
    var name = "any";
    var type = element.type;
    if (type && type.names && type.names.length) {
        if (type.names.length === 1) name = element.type.names[0].trim();
        else name = "(" + element.type.names.join("|") + ")";
    } else return name;
...
function begin(element, is_interface) {
    if (!seen[element.longname]) {
        if (isClass(element)) {
            ...
            var classdesc = comment.tags.find(function (tag) {
                return tag.name === "classdesc";
            });
            if (classdesc) {
    ...
    if (element.scope !== "global" || options.module) return;
    write("export ");
}
...
function writeFunctionSignature(element, isConstructor, isTypeDef) {
    ...
    if (element.params)
        element.params.forEach(function (param) {
            ...
            else params[path[0]].type = "{ [k: string]: any }";
        });
    ...
    paramNames.forEach(function (name, i) {
        ...
        if (i < paramNames.length - 1) write(", ");
    });
    ...
    if (!isConstructor) {
        ...
        if (
            element.returns &&
            element.returns.length &&
            (typeName = getTypeOf(element.returns[0])) !== "undefined"
        )
            write(typeName);
        else write("void");
    }
...
function writeInterfaceBody(element) {
    writeln("{");
    ++indent;
    if (element.tsType) writeln(element.tsType.replace(/\r?\n|\r/g, "\n"));
...
function writeProperty(property, declare) {
    ...
    if (declare) write("let ");
    write(property.name);
    if (property.optional) write("?");
...
function handleElement(element, parent) {
    if (element.scope === "inner") return false;

    if (
        element.optional !== true &&
        element.type &&
        element.type.names &&
        element.type.names.length
    ) {
    ...
    if (seen[element.longname]) return true;
    if (isClassLike(element)) handleClass(element, parent);
    else
        switch (element.kind) {
            case "module":
            case "namespace":
...
function handleNamespace(element /*, parent*/) {
    ...
    children.forEach(function (child) {
        if (child.scope === "inner" || seen[child.longname]) return;
        if (first) {
            begin(element);
            writeln("namespace ", element.name, " {");
            ++indent;
...
function handleClass(element, parent) {
    ...
    if (is_interface) write("interface ");
    else {
    ...
    if (element.augments) {
        ...
        if (augments.length) write("extends ", augments[0], " ");
    }

    // implemented interfaces
    var impls = [];
    if (element.implements) Array.prototype.push.apply(impls, element.implements);
    if (element.mixes) Array.prototype.push.apply(impls, element.mixes);
    ...
    // constructor
    if (!is_interface && !element.virtual) handleFunction(element, parent, true);
    ...
    if (incompatible.length) {
        writeln();
        if (element.scope === "global" && !options.module) write("export ");
        writeln("namespace ", element.name, " {");
        ++indent;
    ...
    if (element.isEnum) {
        ...
        if (stringEnum) {
            ...
            element.properties.forEach(function (property, i) {
                ...
                if (i < element.properties.length - 1) writeln(",");
                else writeln();
            });
            --indent;
            writeln("}");
        ...
        } else write(element.kind === "constant" ? "const " : "let ");

        write(element.name);
        if (element.optional) write("?");
        write(": ");

        if (
            element.type &&
            element.type.names &&
            /^Object\b/i.test(element.type.names[0]) &&
            element.properties
        ) {
            writeln("{");
            ++indent;
            element.properties.forEach(function (property, i) {
                writeln(
                    ...
                    ": ",
                    getTypeOf(property),
                    i < element.properties.length - 1 ? "," : ""
                );
            });
            --indent;
            writeln("};");
        } else writeln(getTypeOf(element), ";");
    }
}
...
function handleFunction(element, parent, isConstructor) {
    ...
    writeln(";");
    if (!insideClass) handleNamespace(element);
}

// handles a type definition (not a real type)
function handleTypeDef(element, parent) {
    if (isInterface(element)) {
        if (isClassLike(parent)) queuedInterfaces.push(element);
        else {
            begin(element);
            writeInterface(element);
        }
        ...
        if (element.templates && element.templates.length)
            write("<", element.templates.join(", "), ">");
        write(" = ");
        if (element.tsType) write(element.tsType.replace(/\r?\n|\r/g, "\n"));
        else {
            var type = getTypeOf(element);
            if (
                element.type &&
                element.type.names.length === 1 &&
                element.type.names[0] === "function"
            )
                writeFunctionSignature(element, false, true);
            else if (type === "object") {
                if (element.properties && element.properties.length) writeInterfaceBody(element);
                else write("{}");
            } else write(type);
        }
        writeln(";");
