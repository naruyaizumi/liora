...
exports.main = function main(args, callback) {
    ...
    Object.keys(argv).forEach(function (key) {
        ...
        if (camelKey !== key) argv[camelKey] = argv[key];
    });
        ...
        if (callback)
            ...
            process.stderr.write(
                [
                    ...
                    "",
                    "usage: " +
                        chalk.bold.green("pbjs") +
                        " [options] file1.proto file2.json ..." +
                        chalk.gray("  (or pipe)  ") +
                        "other | " +
                        chalk.bold.green("pbjs") +
                        " [options] -",
    ...
    if (typeof argv["strict-long"] === "boolean") argv["force-long"] = argv["strict-long"];
    ...
    if (!target) target = require(path.resolve(process.cwd(), argv.target));
    ...
    root.resolvePath = function pbjsResolvePath(origin, target) {
        ...
        if (!normOrigin) mainFiles.push(normTarget);
        ...
        if (idx > -1) {
            ...
            if (altname in protobuf.common) resolved = altname;
        }
    ...
    function markReferenced(tobj) {
        ...
        if (tobj.extensionField) tobj.extensionField.parent.referenced = true;
    }
    ...
    function sparsify(root) {
        ...
        util.traverse(root, function (obj) {
            ...
            if (mainFiles.indexOf(obj.filename) > -1) util.traverseResolved(obj, markReferenced);
        });
        ...
        util.traverse(root, function (obj) {
            ...
            if (obj instanceof protobuf.Namespace) {
                ...
                util.traverse(obj, function (iobj) {
                    if (iobj.referenced) hasReferenced = true;
                });
                if (hasReferenced) {
                    // replace with plain namespace if a namespace subclass
                ...
                } // remove completely if nothing inside is referenced
                else parent.remove(obj);

                // remove everything else unreferenced
            } else if (!(obj instanceof protobuf.Namespace)) parent.remove(obj);
        });
    ...
    function callTarget() {
        target(root, argv, function targetCallback(err, output) {
            ...
            try {
                if (argv.out) fs.writeFileSync(argv.out, output, { encoding: "utf8" });
                else if (!callback) process.stdout.write(output, "utf8");
                return callback ? callback(null, output) : undefined;
            } catch (err) {
                if (callback) return callback(err);
                throw err;
            }
