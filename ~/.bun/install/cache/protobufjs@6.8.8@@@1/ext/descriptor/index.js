...
module.exports =
    exports =
    $protobuf.descriptor =
        $protobuf.Root.fromJSON(require("../../google/protobuf/descriptor.json")).lookup(
            ".google.protobuf"
        );
...
Root.fromDescriptor = function fromDescriptor(descriptor) {
    ...
    if (descriptor.file) {
        ...
        for (var j = 0, i; j < descriptor.file.length; ++j) {
            filePackage = root;
            if (
                (fileDescriptor = descriptor.file[j])["package"] &&
                fileDescriptor["package"].length
            )
            ...
            if (fileDescriptor.messageType)
                for (i = 0; i < fileDescriptor.messageType.length; ++i)
                    filePackage.add(
                        Type.fromDescriptor(fileDescriptor.messageType[i], fileDescriptor.syntax)
                    );
...
Type.fromDescriptor = function fromDescriptor(descriptor, syntax) {
    ...
    var type = new Type(
            descriptor.name.length ? descriptor.name : "Type" + unnamedMessageIndex++,
            fromDescriptorOptions(descriptor.options, exports.MessageOptions)
        ),
        i;
    ...
    /* Extension ranges */ if (descriptor.extensionRange && descriptor.extensionRange.length) {
        ...
        for (i = 0; i < descriptor.extensionRange.length; ++i)
            type.extensions.push([
                descriptor.extensionRange[i].start,
                descriptor.extensionRange[i].end,
            ]);
    }
    /* Reserved... */ if (
        (descriptor.reservedRange && descriptor.reservedRange.length) ||
...
Type.prototype.toDescriptor = function toDescriptor(syntax) {
    ...
    /* Extension ranges */ if (this.extensions)
        for (i = 0; i < this.extensions.length; ++i)
            descriptor.extensionRange.push(
...
Field.prototype.toDescriptor = function toDescriptor(syntax) {
    ...
    if (this.map) {
        ...
        switch ((descriptor.type = toDescriptorType(this.type, this.resolve().resolvedType))) {
            ...
            case 11: // type
            case 14: // enum
                descriptor.typeName = this.resolvedType
    ...
    if (syntax === "proto3") {
        ...
        if (!this.packed)
            (descriptor.options || (descriptor.options = exports.FieldOptions.create())).packed =
                false;
    } else if (this.packed)
        // defaults to packed=false
...
Service.fromDescriptor = function fromDescriptor(descriptor) {
    ...
    var service = new Service(
        descriptor.name && descriptor.name.length
            ? descriptor.name
            : "Service" + unnamedServiceIndex++,
        fromDescriptorOptions(descriptor.options, exports.ServiceOptions)
    );
