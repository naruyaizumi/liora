...
gitSemverTags(function (err, tags) {
    ...
    if (argv.tag) {
        ...
        if (idx < 0) throw Error("no such tag: " + argv.tag);
        from = tags[idx + 1];
        tag = to = tags[idx];
    } else tag = pkg.version;
    ...
    commits.on("data", function (chunk) {
        ...
        message.split(";").forEach(function (message) {
            if (/^(Merge pull request |Post-merge)/.test(message)) return;
            ...
            if (!category) {
                if (breakingFallback.test(message)) category = "Breaking";
                else category = "Other";
            }
            var nl = message.indexOf("\n");
            if (nl > -1) message = message.substring(0, nl).trim();
            if (!hash || message.length < 12) return;
    ...
    commits.on("end", function () {
        ...
        Object.keys(categories).forEach(function (category) {
            ...
            messages.forEach(function (message) {
                ...
                output.push(
                    "[:hash:](" + repo + "/commit/" + message.hash + ") " + text + "<br />\n"
                );
            });
        });
        var current;
        ...
        if (re.test(current)) {
            ...
            if (pos > -1) current = current.substring(pos).trim();
            else current = "";
        }
        var contents = output.join("") + "\n" + current;
        if (argv.write) fs.writeFileSync(basedir + "/CHANGELOG.md", contents, "utf8");
