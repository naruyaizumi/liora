...
(function () {
    ...
    ((ref$ = require("prelude-ls")),
        ...
        (keys = ref$.keys),
        (Obj = ref$.Obj),
        (camelize = ref$.camelize));
    deepIs = require("deep-is");
    ((ref$ = require("./util")),
        (closestString = ref$.closestString),
        (nameToRaw = ref$.nameToRaw),
        (dasherize = ref$.dasherize),
        (naturalJoin = ref$.naturalJoin));
    ((ref$ = require("./help")),
        (generateHelp = ref$.generateHelp),
        (generateHelpForOption = ref$.generateHelpForOption));
    ((ref$ = require("type-check")),
        (parsedTypeCheck = ref$.parsedTypeCheck),
        (parseType = ref$.parseType));
    ...
    main = function (libOptions) {
            ...
            for (i$ = 0, len$ = options.length; i$ < len$; ++i$) {
                ...
                if (option.heading == null) {
                    ...
                    if (option.parsedType == null) {
                        ...
                        try {
                            type =
                                (that = libOptions.typeAliases[option.type]) != null
                                    ? that
                                    : option.type;
                    ...
                    if ((that = option.dependsOn)) {
                        if (that.length) {
                            ((ref$ = [].concat(option.dependsOn)),
                                (rawDependsType = ref$[0]),
                                (dependsOpts = slice$.call(ref$, 1)));
                            dependsType = rawDependsType.toLowerCase();
                            if (dependsOpts.length) {
                                if (dependsType === "and" || dependsType === "or") {
                                    option.dependsOn = [dependsType].concat(
                                        arrayFrom$(dependsOpts)
                                    );
                                } else {
                                    throw new Error(
                                        "Option '" +
                                            name +
                                            "': If you have more than one dependency, you must specify either 'and' or 'or'"
                                    );
                                }
                            } else {
                    ...
                    if (option.alias || option.aliases) {
                        ...
                        ((ref$ = partition(fn$, option.aliases)),
                            (shortNames = ref$[0]),
                            (longNames = ref$[1]));
                    ...
                    if (
                        (!option.aliases || option.shortNames.length === 0) &&
                        option.type === "Boolean" &&
                        option["default"] === "true"
                    ) {
            ...
            if (opt == null) {
                ...
                throw new Error(
                    "Invalid option '" +
                        ...
                        "'" +
                        (possiblyMeant
                            ? " - perhaps you meant '" + nameToRaw(possiblyMeant) + "'?"
                            : ".")
                );
            }
            return opt;
        ...
        parse = function (input, arg$) {
            ...
            setValue = function (name, value) {
                ...
                if (opt.boolean) {
                    ...
                    try {
                        ...
                        if (
                            cra != null &&
                            cra[0] &&
                            cra[1].oneValuePerFlag &&
                            opt.parsedType.length === 1 &&
                            opt.parsedType[0].structure === "array"
                        ) {
                ...
                if (obj[name] != null) {
                    if (
                        opt.concatRepeatedArrays != null &&
                        opt.concatRepeatedArrays[0] &&
                        currentType === "Array"
                    ) {
            ...
            mutuallyExclusiveError = function (first, second) {
                throw new Error(
                    "The options " +
                        ...
                        " and " +
                        nameToRaw(second) +
                        " are mutually exclusive - you cannot use them at the same time."
                );
            };
            ...
            checkDependency = function (option) {
                ...
                for (i$ = 0, len$ = targetOptionNames.length; i$ < len$; ++i$) {
                    ...
                    if (targetOption && checkDependency(targetOption)) {
                        ...
                        throw new Error(
                            "The option '" + option.option + "' did not have its dependencies met."
                        );
                    }
                }
                if (type === "and") {
                    ...
                    throw new Error(
                        "The option '" + option.option + "' did not meet any of its dependencies."
                    );
                }
            };
            ...
            checkProp = function () {
                if (prop) {
                    throw new Error(
                        "Value for '" + prop + "' of type '" + getOption(prop).type + "' required."
                    );
                }
            };
                    ...
                    if ((that = arg.match(/^(--?)([a-zA-Z][-a-zA-Z0-9]*)(=)?(.*)?$/))) {
                        ...
                        if (short) {
                            ...
                            for (j$ = 0, len1$ = flags.length; j$ < len1$; ++j$) {
                                ...
                                if (restPositional) {
                                    ...
                                    throw new Error(
                                        "Can't set argument '" +
                                            flag +
                                            "' when not last flag in a group of short flags."
                                    );
                                }
                            }
                        } else {
                            ...
                            if (opt.boolean) {
                                ...
                                if (negated) {
                                    throw new Error(
                                        "Only use 'no-' prefix for Boolean options, not with '" +
                                            noedName +
                                            "'."
                                    );
                                }
