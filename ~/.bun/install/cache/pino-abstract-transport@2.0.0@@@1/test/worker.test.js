...
function workerTest(filename, description = "") {
    test(`does not wait for pino to send config by default${description}`, function ({
        same,
        plan,
    }) {
        plan(4);
    ...
    test(`does not wait for pino to send config if transport is not expecting it${description}`, function ({
        same,
        plan,
    }) {
        plan(4);
    ...
    test(`waits for the pino config when pino intends to send it and the transport requests it${description}`, function ({
        same,
        plan,
    }) {
        plan(4);
    ...
    test(`continues to listen if it receives a message that is not PINO_CONFIG${description}`, function ({
        same,
        plan,
    }) {
        plan(4);
        ...
        stream.emit("message", {
            code: "NOT_PINO_CONFIG",
            config: { levels: "foo", messageKey: "bar", errorKey: "baz" },
        });
        stream.emit("message", { code: "PINO_CONFIG", config: pinoConfig });
        stream.write(lines);
        stream.end();
    ...
    test(`waits for the pino config even if it is sent after write${description}`, function ({
        same,
        plan,
    }) {
        plan(4);
