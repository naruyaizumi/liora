...
const baileysLogger = () => {
  ...
  const formatValue = (value) => {
    ...
    if (typeof value === "object") {
      return Bun.inspect(value, { colors: false, depth: 2 });
    }
    return String(value);
  };
  ...
  return {
    level: "silent",
    fatal: (...args) => {
      if (shouldLog("fatal")) console.error(formatLog("fatal", ...args));
    },
    error: (...args) => {
    ...
    debug: (...args) => {
      if (shouldLog("debug")) console.debug(formatLog("debug", ...args));
    },
    trace: (...args) => {
      if (shouldLog("trace")) console.trace(formatLog("trace", ...args));
    },
    child: () => baileysLogger(),
  };
};
  ...
  try {
    ...
    if (sqlite) {
      try {
        ...
        global.sqlite.close();
        global.logger.debug("SQLite closed");
      } catch (e) {
