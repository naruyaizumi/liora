...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        ...
        const wasRedirected = response.redirected || finalUrl !==
            originalUrl;
        
        if (!response.ok) {
            const errorText = await Bun.readableStreamToText(response
                .body).catch(() => "");
...
${errorText.substring(0, 200)}${errorText.length > 200 ? "..." : ""}\`\`\``;
        ...
        const uint8Array = await Bun.readableStreamToBytes(response
            .body);
        ...
        const transferRate = formatBytes(buffer.length / (totalTime /
            1000)) + "/s";
...
JSON Keys: ${Object.keys(jsonContent).length > 5 ? 
