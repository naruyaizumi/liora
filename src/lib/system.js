...
export async function getRedisInfo() {
    try {
        const version = await $`redis-server --version | awk '{print $3}' | cut -d= -f2`.text();
...
export async function getSoftwareVersions() {
    ...
    return {
        node: nodeVersion.trim().replace('v', '').replace('Not installed', 'Not installed'),
        bun: bunVersion.trim().replace('Not installed', 'Not installed'),
        npm: await $`npm --version 2>/dev/null || echo ""`.text().then(t => t.trim())
    };
}

export async function getCPUInfo() {
    try {
        const cpuInfo = await $`cat /proc/cpuinfo 2>/dev/null || echo ""`
            .text();
        const loadAvg =
        ...
        const loadPercent = load => cores > 0 ? ((load / cores) * 100)
            .toFixed(2) : "0.00";
        ...
        try {
            ...
            if (global._prevCPU) {
                ...
                usage = totalDelta > 0 ? (((totalDelta - idleDelta) * 100) /
                    totalDelta).toFixed(2) : "0.00";
            }
...
export async function getMemoryInfo() {
    try {
        ...
        try {
            ...
            for (const line of lines) {
                if (line.startsWith("nr_active_file")) active = parseInt(
                    line.split(" ")[1]) * 4096;
                if (line.startsWith("nr_inactive_file")) inactive =
                    parseInt(line.split(" ")[1]) * 4096;
                if (line.startsWith("nr_dirty")) dirty = parseInt(line
                    .split(" ")[1]) * 4096;
                if (line.startsWith("nr_writeback")) writeback = parseInt(
                    line.split(" ")[1]) * 4096;
            }
        } catch {
            //
...
export async function getDiskInfo() {
    try {
        const dfOutput =
            await $`df -B1 --output=source,fstype,size,used,avail,pcent,target,itotal,iused,iavail,ipcent 2>/dev/null || df -k`
            .text();
        ...
        for (const line of lines) {
            ...
            if (parts.length >= 6) {
                const size = parseInt(parts[2]) * (parts[2].includes("T") ?
                    1024 * 1024 * 1024 * 1024 :
                    parts[2].includes("G") ? 1024 * 1024 * 1024 :
                    parts[2].includes("M") ? 1024 * 1024 :
                    parts[2].includes("K") ? 1024 : 1);
                const used = parseInt(parts[3]) * (parts[3].includes("T") ?
                    1024 * 1024 * 1024 * 1024 :
                    parts[3].includes("G") ? 1024 * 1024 * 1024 :
                    parts[3].includes("M") ? 1024 * 1024 :
                    parts[3].includes("K") ? 1024 : 1);
                const avail = parseInt(parts[4]) * (parts[4].includes("T") ?
                    1024 * 1024 * 1024 * 1024 :
                    parts[4].includes("G") ? 1024 * 1024 * 1024 :
                    parts[4].includes("M") ? 1024 * 1024 :
                    parts[4].includes("K") ? 1024 : 1);
        ...
        let ioStats = { readBytes: 0, writeBytes: 0, readOps: 0,
            writeOps: 0 };
        try {
            const diskstats = await $`cat /proc/diskstats 2>/dev/null`
            .text();
...
export async function getNetworkInfo() {
    try {
        ...
        let connections = 0;
        try {
            const connOutput = await $`ss -tun state connected | wc -l`
                .text();
        ...
        try {
            const resolv =
                await $`cat /etc/resolv.conf 2>/dev/null | grep nameserver`
                .text();
...
export async function getProcessInfo() {
    try {
        ...
        return {
            ...
            load5: loads[1] || 0,
            load15: loads[2] || 0,
            threads: await $`ps -eL --no-headers | wc -l`.text().then(t =>
                parseInt(t.trim())),
            contextSwitches: await $`cat /proc/stat | grep ctxt`.text()
...
export async function getContainerInfo() {
    try {
        const inContainer =
            await $`cat /proc/1/cgroup 2>/dev/null | grep -q docker || echo $?`
            .text();
        const isDocker = inContainer.trim() === "0";
        
        const isLxc =
            await $`cat /proc/1/environ 2>/dev/null | tr '\\0' '\\n' | grep -q container=lxc || echo $?`
            .text();
        ...
        if (isDocker) {
            ...
            containerId =
                ...
                .text().then(t => t.trim().slice(0, 12));
        } else if (isLxc === "0") {
            containerType = "LXC";
        }
        
        const hasKube = await $`env | grep -q KUBERNETES_SERVICE || echo $?`
            .text();
        if (hasKube === "0") {
    ...
    try {
        ...
        if (parts.length >= 12) {
            ...
            return {
                ...
                cpu: {
                    us: cpuIndex < parts.length ? parseInt(parts[
                        cpuIndex]) || 0 : 0,
                    sy: cpuIndex + 1 < parts.length ? parseInt(parts[
                        cpuIndex + 1]) || 0 : 0,
                    id: cpuIndex + 2 < parts.length ? parseInt(parts[
                        cpuIndex + 2]) || 0 : 100,
                    wa: cpuIndex + 3 < parts.length ? parseInt(parts[
                        cpuIndex + 3]) || 0 : 0,
                    st: cpuIndex + 4 < parts.length ? parseInt(parts[
                        cpuIndex + 4]) || 0 : 0
                }
            };
        }
...
export async function getServiceInfo() {
    try {
        ...
        for (const service of importantServices) {
            try {
                const status =
                    await $`systemctl is-active ${service} 2>/dev/null || echo "inactive"`
                    .text();
                if (status.trim() === "active") {
                    const info =
                        await $`systemctl status ${service} --no-pager 2>/dev/null || echo ""`
                        .text();
                    if (info) {
                        ...
                        const activeLine = lines.find(l => l.includes(
                            "Active:"));
                        const memoryLine = lines.find(l => l.includes(
                            "Memory:"));
                        const cpuLine = lines.find(l => l.includes("CPU:"));
                        
                        if (nameLine) {
                            const serviceName = nameLine.split(
                                ".service - ")[0].trim();
                            const description = nameLine.split(
                                ".service - ")[1]?.trim() || "";
                            ...
                            detailedServices.push({
                                ...
                                description: description.substring(
                                    0, 50),
                                status: "active",
                                active: activeLine?.split(
                                    "Active: ")[1]?.split(";")[
                                    0]?.trim() || "",
                                memory: memoryLine?.split(
                                        "Memory: ")[1]?.trim() ||
                                    "",
                                cpu: cpuLine?.split("CPU: ")[1]
                                    ?.trim() || ""
                            });
                        }
                    }
