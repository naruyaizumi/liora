        ...
        */
        sendAlbum: {
            async value(jid, content, options = {}) {
                ...
                if (
                    ...
                    throw new Error(
                        "Album content with items array is required"
                    );
                }
                
                const items = content.album;
                
                const expectedImageCount = items.filter((item) =>
                    item?.image).length;
                const expectedVideoCount = items.filter((item) =>
                    item?.video).length;
                ...
                for (let i = 0; i < items.length; i++) {
                    ...
                    if (contentItem.image) {
                        ...
                        } else if (typeof contentItem.image ===
                            "object" && contentItem.image.url) {
                            mediaInput.image = {
                        ...
                        if (contentItem.caption) {
                            mediaInput.caption = contentItem
                                .caption;
                        }
                        
                        mediaMsg = await generateWAMessage(album.key
                            .remoteJid, mediaInput, {
                                upload: this.waUploadToServer,
                        ...
                        } else if (typeof contentItem.video ===
                            "object" && contentItem.video.url) {
                            mediaInput.video = {
                        ...
                        if (contentItem.mimetype) {
                            mediaInput.mimetype = contentItem
                                .mimetype;
                        }
                    ...
                    await this.relayMessage(mediaMsg.key.remoteJid,
                        mediaMsg.message, {
        ...
        sendGroupStatus: {
            async value(jid, content = {}, options = {}) {
                ...
                const { backgroundColor, ...contentWithoutBg } =
                content;
        ...
        sendInviteGroup: {
            async value(jid, content, options = {}) {
                ...
                if (!content?.groupInvite) {
                    throw new Error(
                        "Group invite content is required");
                }
                ...
                if (!groupInvite.jid || !groupInvite.code) {
                    throw new Error(
                        "Group JID and invite code are required"
                    );
                }
                
                let inviteExpiration;
                if (groupInvite.expiration) {
                    const expirationNum = parseInt(groupInvite
                        .expiration);
                    inviteExpiration = Math.floor(Date.now() /
                        1000) + expirationNum;
                } else {
                    inviteExpiration = Math.floor(Date.now() /
                        1000) + 3 * 24 * 60 * 60;
                ...
                if (groupInvite.jpegThumbnail && Buffer.isBuffer(
                    ...
                    if (typeof groupInvite.thumbnail === "string") {
                        jpegThumbnail = Buffer.from(groupInvite
                            .thumbnail, "base64");
                    } else if (groupInvite.thumbnail.url) {
                        try {
                            const response = await fetch(groupInvite
                                .thumbnail.url);
                            ...
                            global.logger.error(
                                "Failed to fetch thumbnail from URL:",
                                error);
                        }
                    } else if (Buffer.isBuffer(groupInvite
                            .thumbnail)) {
        ...
        sendInviteAdmin: {
            async value(jid, content, options = {}) {
                ...
                if (!content?.adminInvite) {
                    throw new Error(
                        "Admin invite content is required");
                }
                ...
                if (adminInvite.expiration) {
                    const expirationNum = parseInt(adminInvite
                        .expiration);
                    inviteExpiration = Math.floor(Date.now() /
                        1000) + expirationNum;
                } else {
                ...
                if (adminInvite.jpegThumbnail && Buffer.isBuffer(
                    ...
                    if (typeof adminInvite.thumbnail === "string") {
                        jpegThumbnail = Buffer.from(adminInvite
                            .thumbnail, "base64");
                    } else if (adminInvite.thumbnail.url) {
                        try {
                            const response = await fetch(adminInvite
                                .thumbnail.url);
                            const arrayBuffer = await response
                                .arrayBuffer();
                            jpegThumbnail = Buffer.from(
                    ...
                    } else if (Buffer.isBuffer(adminInvite
                            .thumbnail)) {
                ...
                if (adminInvite.mentions && adminInvite.mentions
                    .length > 0) {
        ...
        pollResult: {
            async value(jid, content, options = {}) {
                ...
                if (!content?.pollResult) {
                    throw new Error(
                        "Poll result content is required");
                }
                ...
                if (!Array.isArray(pollResult.values)) {
                    throw new Error(
                        "Poll result values must be an array");
                }
                
                for (const [index, value] of pollResult.values
                    .entries()) {
                ...
                const pollResultSnapshotMessage = {
                    ...
                    pollVotes: pollResult.values.map(([
                ...
                let contextInfo = {};
                if (pollResult.mentions && pollResult.mentions
                    .length > 0) {
        ...
        sendPayment: {
            async value(jid, amount, currency = "IDR", note =
                ...
                const requestPaymentMessage = {
                    ...
                    background: {
                        placeholderArgb: options.image
                            ?.placeholderArgb || 4278190080,
                        textArgb: options.image?.textArgb ||
                            4294967295,
                        subtextArgb: options.image
                ...
                return await this.relayMessage(message.key
                    .remoteJid, message.message, {
                        messageId: message.key.id,
                    });
            },
            enumerable: true,
        ...
        sendOrder: {
            async value(jid, orderData, options = {}) {
                ...
                if (orderData.thumbnail) {
                    ...
                    } else if (typeof orderData.thumbnail ===
                        "string") {
                        try {
                            if (orderData.thumbnail.startsWith(
                                    "http")) {
                                ...
                                thumbnail = Buffer.from(orderData
                                    .thumbnail, "base64");
                            }
                        } catch (e) {
                ...
                const orderMessage = proto.Message.OrderMessage
                    .fromObject({
                        ...
                        status: orderData.status || proto
                            .Message.OrderMessage.OrderStatus
                            .INQUIRY,
                        surface: orderData.surface || proto
                            .Message.OrderMessage.OrderSurface
                            .CATALOG,
                        message: orderData.message || "",
                        orderTitle: orderData.orderTitle ||
                            "Order",
                        sellerJid: orderData.sellerJid || this
                            .user.id,
                        token: orderData.token || "",
                        totalAmount1000: orderData
                            .totalAmount1000 || 0,
                        totalCurrencyCode: orderData
        ...
        sendPayInfo: {
            async value(jid, content = {}, options = {}) {
                ...
                if (!Array.isArray(interactiveButtons) ||
                    ...
                    throw new Error(
                        "interactiveButtons must be a non-empty array"
                    );
                }
                ...
                for (let i = 0; i < interactiveButtons
                    .length; i++) {
                    const btn = interactiveButtons[i];
                    
                    if (!btn || typeof btn !== "object") {
                        throw new Error(
                            `interactiveButton[${i}] must be an object`
                        );
                    }
                    ...
                    throw new Error(
                        `interactiveButton[${i}] has invalid shape`
                    );
                }
            ...
            async value(jid, content = {}, options = {}) {
                ...
                if (image) {
                    ...
                    if (preparedMedia.documentMessage) {
                        if (fileName) preparedMedia.documentMessage
                            .fileName = fileName;
                        if (fileLength !== null) preparedMedia
                            .documentMessage.fileLength = fileLength
                            .toString();
                        if (pageCount !== null) preparedMedia
                            .documentMessage.pageCount = pageCount;
                        if (mimetype) preparedMedia.documentMessage
                            .mimetype = mimetype;
                    }
                    ...
                    if (product.productImage) {
                        ...
                        if (Buffer.isBuffer(product.productImage)) {
                            ...
                            "object" && product.productImage.url) {
                            mediaInput.image = {
                                url: product
                                    .productImage.url
                            };
                ...
                if (contextInfo && typeof contextInfo ===
                    "object") {
                    messageContent.contextInfo = { ...contextInfo };
                }
                else if (externalAdReply &&
                    typeof externalAdReply === "object") {
                    messageContent.contextInfo = {
                        externalAdReply: {
                            title: externalAdReply.title || "",
                            body: externalAdReply.body || "",
                            mediaType: externalAdReply
                                .mediaType || 1,
                            sourceUrl: externalAdReply
                                .sourceUrl || externalAdReply
                                .url || "",
                            thumbnailUrl: externalAdReply
                                .thumbnailUrl || externalAdReply
                                .thumbnail || "",
        ...
        reply: {
            async value(jid, text = "", quoted, options = {}) {
                ...
                const ephemeral =
                    chat?.metadata?.ephemeralDuration || chat
                ...
                const thumb = thumbs[Math.floor(Math.random() *
                    thumbs.length)];
                ...
                if (global.db?.data?.settings?.[conn.user?.lid]
                    ?.adReply) {
                    messageContent.contextInfo = {
                        externalAdReply: {
                ...
                if (isGroup) {
                    return conn.sendMessage(jid, messageContent,
                        baseOptions);
                }
        ...
        downloadM: {
            async value(m, type) {
                if (!m || !(m.url || m.directPath)) return Buffer
                    .alloc(0);
                
                const stream = await downloadContentFromMessage(m,
                    type);
                const chunks = [];
        ...
        getName: {
            value: async function(jid = "", withoutContact = false) {
                ...
                withoutContact = conn.withoutContact ||
                    withoutContact;
                ...
                const name = v.name || v.vname || v.notify || v
                    .verifiedName || v.subject;
        ...
        processMessageStubType: {
            async value(m) {
                ...
                const author = conn.decodeJid(
                    m.key?.participant || m.participant || m.key
                    ?.remoteJid || ""
                );
            ...
            async value() {
                try {
                    const allGroups = await conn
                        .groupFetchAllParticipating().catch(() =>
                            ({}));
                    
                    if (!allGroups || typeof allGroups !==
                        "object") {
                        return conn.chats || {};
                    }
                    
                    const existing = conn.chats || (conn.chats =
                        Object.create(null));
                    const now = _now();
                    const activeGroups = new Set();
                    
                    for (const [gid, meta] of Object.entries(
                            allGroups)) {
                        if (!_isGroupJid(gid)) continue;
                        
                        activeGroups.add(gid);
                        const chat = existing[gid] || (existing[
                            gid] = { id: gid });
                        
                        chat.subject = meta.subject || chat
                            .subject || "";
                        chat.metadata = meta;
                        chat.isChats = true;
                        chat.lastSync = now;
                    ...
                    for (const jid in existing) {
                        if (_isGroupJid(jid) && !activeGroups.has(
                                jid)) {
                ...
                for (const message of messages) {
                    ...
                    try {
                        if (
                            ...
                            conn.processMessageStubType(message)
                                .catch((e) => global.logger.error(
                                    e));
                        }
                        ...
                        if (!mtype) mtype = mtypeKeys[mtypeKeys
                            .length - 1];
                        ...
                        if (!chats) {
                            ...
                            if (isGroup && !conn._groupMetaCache
                                .get(chat)) {
                        ...
                        if (ctx?.quotedMessage && ctx.stanzaId) {
                            const qChat = conn.decodeJid(ctx
                                .remoteJid || ctx.participant ||
                                chat);
                            ...
                            if (qChat && !_isStatusJid(qChat)) {
                                ...
                                if (!qm.messages[ctx.stanzaId]) {
                                    qm.messages[ctx.stanzaId] =
                                        quotedMsg;
                                    conn._msgIndex.set(ctx.stanzaId,
                                        quotedMsg);
                                }
                                const msgKeys = Object.keys(qm
                                    .messages);
                                if (msgKeys.length > 40) {
                                    for (let i = 0; i < msgKeys
                                        .length - 30; i++) {
                                        delete qm.messages[msgKeys[
                                            i]];
                                    }
                                }
                            }
                        ...
                        if (isGroup) {
                            ...
                            if (sender && sender !== chat) {
                                ...
                                sChat.name ||= message.pushName ||
                                    sChat.name || "";
                            }
                        } else {
                            sender = message.key?.fromMe && conn
                                .user?.lid ? conn.user.lid : chat;
                            chats.name ||= message.pushName || chats
                                .name || "";
                        }
                        ...
                        if (
                            ...
                            const fromMe =
                                message.key?.fromMe ||
                                (conn.user?.lid && sender &&
                                    areJidsSameUser ?
                                    areJidsSameUser(sender, conn
                                        .user.lid) :
                                    false);
                            ...
                            if (
                                ...
                                chats.messages ||= Object.create(
                                    null);
                                chats.messages[message.key.id] =
                                    message;
                                conn._msgIndex.set(message.key.id,
                                    message);
                                
                                const msgKeys = Object.keys(chats
                                    .messages);
                                if (msgKeys.length > 40) {
                                    for (let i = 0; i < msgKeys
                                        .length - 30; i++) {
