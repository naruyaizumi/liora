...
export default function bind(conn) {
  ...
  conn.ev.on(
    ...
    async function ({ id, participants, action }) {
      redisStore.enqueueEvent(
        "group-participants.update",
  ...
  conn.ev.on("presence.update", async function ({ id, presences }) {
    ...
    try {
      ...
      let chats = (await redisStore.get(`${REDIS_PREFIX}${_sender}`)) || {
        id: sender,
      };
      chats.presences = presence;
      await redisStore.atomicSet(
        `${REDIS_PREFIX}${_sender}`,
        chats,
        "presence",
      );

      if (id.endsWith("@g.us")) {
        let groupChats = (await redisStore.get(`${REDIS_PREFIX}${id}`)) || {
          id,
        };
