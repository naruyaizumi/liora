...
export class RedisStore {
    ...
    async _initRedis() {
        try {
            ...
            global.logger?.error({ error: e.message }, "Redis init failed - using fallback mode");
    ...
    async _processEventQueue() {
        ...
        for (let priority = EVENT_PRIORITY.CORE; priority <= EVENT_PRIORITY.NOISE; priority++) {
    ...
    _startBackpressureMonitor() {
        setInterval(() => {
            const totalQueued = Object.values(this.eventQueue)
