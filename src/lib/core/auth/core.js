...
export class DatabaseCore {
  constructor(options = {}) {
    ...
    const connectionString =
      ...
      "postgres://postgres:postgres@localhost:5432/liora_auth";
    ...
    this.cleanupIntervalMs = Number(
      options.cleanupIntervalMs ?? 30 * 60 * 1000,
    );
  ...
  async get(key) {
    ...
    try {
      const result = await this
        .db`SELECT value FROM baileys_state WHERE key = ${key}`;
  ...
  async _performCleanup() {
    ...
    try {
      const sizeResult = await this
        .db`SELECT pg_database_size(current_database()) as size`;
    ...
    try {
      ...
      if (this.initialized && !this.disposed) {
        try {
          ...
          if (stats) {
            global.logger?.info(
              `Final DB stats: ${stats.entries} entries, ${stats.dbSize}`,
            );
          }
