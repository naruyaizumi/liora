...
function exitHandler(_signal) {
    ...
    for (const [id, handler] of signalHandlers) {
        try {
            ...
            global.logger.error({ err: e.message, handler: id, context: "exitHandler" });
        }
    }
}
...
export function initializeSignalHandlers() {
    ...
    try {
        ...
        global.logger.error({ err: e.message, context: "initializeSignalHandlers" });
    }
}

export function registerSignalHandler(id, handler) {
    if (typeof handler !== "function") {
        global.logger.warn({ id, context: "registerSignalHandler: invalid handler" });
        return false;
    }
