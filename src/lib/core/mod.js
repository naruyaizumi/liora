...
export class mods {
  ...
  async sendAlbum(jid, content, options = {}) {
    ...
    if (!content?.album || !Array.isArray(content.album) || content.album
      .length === 0) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (contentItem.image) {
        ...
        } else if (typeof contentItem.image === "object" && contentItem.image
          .url) {
        ...
        } else if (typeof contentItem.video === "object" && contentItem.video
          .url) {
    ...
    const carouselCards = await Promise.all(
      cards.map(async (card) => {
        ...
        if (mediaContent instanceof Uint8Array) {
          ...
          throw new Error(
            "Media must be Uint8Array, URL string, or { url: string }"
          );
        }
        ...
        if (Array.isArray(card.buttons) && card.buttons.length > 0) {
          cardObj.nativeFlowMessage = {
            buttons: card.buttons.map((btn) => ({
              ...
              buttonParamsJson: btn.buttonParamsJson || JSON
                .stringify(btn),
            })),
          };
        }
  ...
  async sendButton(jid, content = {}, options = {}) {
    ...
    if (!Array.isArray(interactiveButtons) || interactiveButtons.length ===
      0) {
    ...
    if (image) {
      ...
      if (preparedMedia.documentMessage) {
        ...
        if (fileLength !== null) preparedMedia.documentMessage.fileLength =
          fileLength.toString();
        if (pageCount !== null) preparedMedia.documentMessage.pageCount =
          pageCount;
      ...
      if (product.productImage) {
        ...
        } else if (typeof product.productImage === "object" && product
          .productImage.url) {
      ...
      messageContent.header = {
        ...
        productMessage: {
          ...
          businessOwnerJid: businessOwnerJid || product.businessOwnerJid ||
            this.conn.user.id,
        },
      };
    } else if (title) {
    ...
    if (contextInfo && typeof contextInfo === "object") {
      ...
      messageContent.contextInfo = {
        externalAdReply: {
          ...
          thumbnailUrl: externalAdReply.thumbnailUrl || externalAdReply
            .thumbnail || "",
          ...
          ...(externalAdReply.thumbnail && externalAdReply
            .thumbnail instanceof Uint8Array && {
              thumbnail: externalAdReply.thumbnail,
            }),
          ...(externalAdReply.jpegThumbnail && externalAdReply
