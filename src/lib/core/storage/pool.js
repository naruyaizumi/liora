...
export class RedisPool {
    ...
    async init() {
        ...
        const promises = Array.from({ length: this.poolSize }, async (_, index) => {
            try {
                ...
                global.logger.error({ err, clientIndex: index }, "Failed to create Redis client");
                throw err;
            }
        });
    ...
    async _createClient(index) {
        ...
        if (typeof client.on === "function") {
            client._eventHandlers.error = (err) => {
                global.logger.error({ err, clientIndex: index }, "Redis client error");
    ...
    release(client) {
        ...
        if (!client._isHealthy || this.reconnecting.has(client)) {
            global.logger.debug(
                { clientIndex: client._poolIndex },
                "Not releasing unhealthy client to pool"
            );
            return;
        }
    ...
    async execute(fn, retries = 2) {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            ...
            try {
                ...
                const nonRetryableErrors = ["WRONGTYPE", "SYNTAX", "NOPERM", "NOAUTH"];

                if (nonRetryableErrors.some((code) => err.message?.includes(code))) {
                    global.logger.error({ err }, "Non-retryable Redis error");
                    throw err;
                }
...
export class BatchManager {
    ...
    add(operation, key, value, ttl) {
        ...
        const totalItems = Array.from(this.batches.values()).reduce(
    ...
    _scheduleFlush(delay) {
        ...
        this.timer = setTimeout(() => {
            ...
            this.flush().catch((err) => {
                global.logger.error({ err }, "Background flush failed");
            });
        }, delay);
    }
        ...
        try {
            await this.pool.execute(async (client) => {
                ...
                if (operations.length > 0) {
                    ...
                    global.logger.debug({ count: operations.length }, "Flushed Redis operations");
                }
            }, 1);
        } catch (err) {
            ...
            if (this.failedBatches.length < 10) {
                ...
                global.logger.error("Too many failed batches, dropping oldest");
    ...
    async destroy() {
        ...
        try {
            ...
            global.logger.error({ err }, "Error during final batch flush");
        }
    }
}
...
export class RedisLRUCache {
    ...
    clear() {
        ...
        this.cache.clear();
        this.pending.clear();
        global.logger.debug({ cleared: size }, "Cache cleared");
        return size;
    }
    ...
    cleanup() {
        ...
        if (deleted > 0) {
            global.logger.debug({ deleted }, "Cleaned up expired cache entries");
        }
...
export class RedisBlocklistCache {
    ...
    async add(jid) {
        ...
        global.logger.debug({ jid }, "Added to blocklist");
    }
    ...
    async delete(jid) {
        ...
        global.logger.debug({ jid }, "Removed from blocklist");
    }
