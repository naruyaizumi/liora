...
export class StoreManager extends EventEmitter {
    ...
    async init() {
        try {
            ...
            global.logger.error({ err }, "Redis initialization error");
            throw err;
        }
    }
    ...
    async fetchGroupMetadata(conn, id, force = false) {
        ...
        const promise = this.queue.add(async () => {
            try {
                ...
                global.logger.warn({ err, id }, "Failed to fetch group metadata");
        ...
        this.cleanupInterval = setInterval(() => {
            this.queue.add(async () => {
                try {
                    ...
                    global.logger.error({ err }, "Cleanup error");
                    this.stats.errorsCount++;
                }
            });
        }, 300000);
    ...
    async syncToChats(conn) {
        ...
        return await this.queue.add(async () => {
            ...
            if (synced > 0) {
                global.logger.info(`Synced ${synced} stale chats removed`);
            }
