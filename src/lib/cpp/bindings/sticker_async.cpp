...
public:
    StickerAsyncWorker(Napi::Env& env,
...
protected:
    void Execute() override {
        result_ = StickerCore::ProcessSticker(input_.data(), input_.size(), options_);
    }
...
public:
    static Napi::Promise Execute(Napi::Env env,
                                 std::vector<uint8_t>&& input,
                                 StickerCore::StickerOptions&& opts) {
        auto* deferred = new Napi::Promise::Deferred(Napi::Promise::Deferred::New(env));
        auto promise = deferred->Promise();

        Napi::Function callback = Napi::Function::New(env, [](const Napi::CallbackInfo&) {});

        auto tsfn = Napi::ThreadSafeFunction::New(env, callback, "StickerCallback", 0, 1);

        get_sticker_pool().enqueue([input = std::move(input),
                                    opts = std::move(opts),
                                    tsfn,
                                    deferred]() mutable {
            auto result = StickerCore::ProcessSticker(input.data(), input.size(), opts);

            auto* data = new TSFNData{deferred, std::move(result)};

            napi_status status =
                    tsfn.BlockingCall(data, [](Napi::Env env, Napi::Function, TSFNData* data) {
                        Napi::HandleScope scope(env);
...
static Napi::Value StickerAsync(const Napi::CallbackInfo& info) {
    ...
    if (info.Length() < 1 || !info[0].IsBuffer()) {
        ...
        deferred.Reject(Napi::Error::New(env, "First argument must be a Buffer").Value());
    ...
    auto* worker = new StickerAsyncWorker(env, std::move(input), std::move(options));
...
static Napi::Value StickerThreadPool(const Napi::CallbackInfo& info) {
    ...
    return StickerThreadPoolWorker::Execute(env, std::move(input), std::move(options));
}
