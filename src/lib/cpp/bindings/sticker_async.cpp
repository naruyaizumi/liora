...
protected:
    void Execute() override {
        result_ = StickerCore::ProcessSticker(
            input_.data(), 
            input_.size(), 
            options_
        );
...
public:
    static Napi::Promise Execute(
        ...
        get_sticker_pool().enqueue([
            input = std::move(input),
            opts = std::move(opts),
            tsfn,
            deferred
        ]() mutable {
            auto result = StickerCore::ProcessSticker(
                input.data(),
                input.size(),
                opts
            );
            ...
            napi_status status = tsfn.BlockingCall(data, [](Napi::Env env, Napi::Function, TSFNData* data) {
                Napi::HandleScope scope(env);
                ...
                if (data->result.success) {
                    ...
                    data->deferred->Reject(Napi::Error::New(env, data->result.error).Value());
                }
...
static Napi::Value StickerAsync(const Napi::CallbackInfo& info) {
    ...
    options.maxDuration = opt.Has("maxDuration") ? opt.Get("maxDuration").ToNumber().Int32Value() : 15;
