...
function loadAddon() {
    ...
    throw new Error("converter.node not found in Node.js worker thread");
}
...
if (parentPort) {
    ...
    parentPort.on("error", (error) => {
        console.error("[ConverterWorker] Parent port error:", error);
        process.exit(1);
    });

    process.on("uncaughtException", (error) => {
        console.error("[ConverterWorker] Uncaught exception:", error);
        if (parentPort) {
            parentPort.postMessage({
                data: null,
    ...
    process.on("unhandledRejection", (reason) => {
        console.error("[ConverterWorker] Unhandled rejection:", reason);
        if (parentPort) {
            parentPort.postMessage({
                data: null,
