...
static SwrContext* init_resampler(AVCodecContext* dec_ctx, AVCodecContext* enc_ctx) {
...
ConvertResult ConvertAudio(const uint8_t* input_data,
                           size_t input_size,
    ...
    try {
        ...
        io_guard.ctx = avio_alloc_context(
                io_buffer, 65536, 0, &buf_ctx, &read_packet, nullptr, &seek_packet);
        ...
        int audio_stream =
                av_find_best_stream(fmt_guard.ctx, AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0);
        ...
        if (avformat_alloc_output_context2(&out_guard.ctx, nullptr, mux_name.c_str(), nullptr) !=
                    0 ||
            !out_guard.ctx) {
        ...
        int sample_rate = (opts.sampleRate > 0)                  ? opts.sampleRate
                          : (dec_ctx_guard.ctx->sample_rate > 0) ? dec_ctx_guard.ctx->sample_rate
                                                                 : 48000;
        ...
        if (out_codec_id == AV_CODEC_ID_OPUS) {
            av_opt_set(enc_ctx_guard.ctx->priv_data,
                       "application",
                       (opts.ptt ? "voip" : "audio"),
                       0);
            av_opt_set(enc_ctx_guard.ctx->priv_data, "vbr", (opts.vbr ? "on" : "off"), 0);
        }
        ...
        fifo_guard.fifo = av_audio_fifo_alloc(enc_ctx_guard.ctx->sample_fmt,
                                              out_channels,
                                              enc_ctx_guard.ctx->frame_size > 0
        ...
        int64_t pts = 0;
        int frame_size = enc_ctx_guard.ctx->frame_size > 0 ? enc_ctx_guard.ctx->frame_size
        ...
        while (av_read_frame(fmt_guard.ctx, packet) >= 0) {
            if (packet->stream_index == audio_stream) {
                ...
                while (ret >= 0) {
                    ...
                    if (converted_samples > 0) {
                        av_samples_alloc_array_and_samples(&converted_data,
                                                           nullptr,
                                                           out_channels,
                                                           converted_samples,
                        ...
                        converted_samples = swr_convert(
                                swr_guard.ctx, converted_data, converted_samples, nullptr, 0);

                        if (converted_samples > 0) {
                            av_audio_fifo_write(fifo_guard.fifo,
                                                (void**)converted_data,
                                                converted_samples);
                        }
                    ...
                    while (av_audio_fifo_size(fifo_guard.fifo) >= frame_size) {
                        ...
                        if (encode_write_frame(encode_frame_guard.frame,
                                               enc_ctx_guard.ctx,
                                               out_guard.ctx,
                                               out_stream) < 0) {
                            av_frame_unref(encode_frame_guard.frame);
        ...
        while (true) {
            ...
            if (ret >= 0) {
                uint8_t** converted_data = nullptr;
                int converted_samples = swr_convert(swr_guard.ctx,
                                                    nullptr,
                                                    0,
                                                    (const uint8_t**)decode_frame_guard.frame->data,
                                                    decode_frame_guard.frame->nb_samples);
        ...
        while (true) {
            ...
            converted_samples =
                    swr_convert(swr_guard.ctx, converted_data, converted_samples, nullptr, 0);
        ...
        while (av_audio_fifo_size(fifo_guard.fifo) > 0) {
            ...
            if (av_frame_get_buffer(encode_frame_guard.frame, 0) >= 0) {
                if (av_audio_fifo_read(fifo_guard.fifo,
                    ...
                    encode_write_frame(encode_frame_guard.frame,
                                       enc_ctx_guard.ctx,
                                       out_guard.ctx,
                                       out_stream);
                }
