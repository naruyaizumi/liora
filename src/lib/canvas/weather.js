...
function getWeatherBackground(phrase) {
    ...
    } else if (lower.includes("cloud") || lower.includes("overcast") || lower.includes("dreary")) {
        return {
...
export async function canvas(weatherData) {
    ...
    ctx.fillText(`Generated ${dateStr} • Powered by AccuWeather`, BASE_WIDTH - 20, 30);
    ...
    await drawWeatherIcon(ctx, mainCardX + 310, mainCardY + 180, 80, todayForecast.Day.IconPhrase);

    ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
    ctx.font = "20px Cobbler";
    ctx.fillText(todayForecast.Day.IconPhrase, mainCardX + 30, mainCardY + 300);
    ...
    for (let i = 1; i < forecasts.length; i++) {
        ...
        await drawWeatherIcon(ctx, forecastX + 25, y + 75, 50, forecast.Day.IconPhrase);

        ctx.fillStyle = "#FFFFFF";
        ctx.font = "bold 32px Cobbler";
        ctx.textAlign = "right";
        ctx.fillText(
            `${Math.round(parseFloat(forecast.Temperature.Max))}°`,
            forecastX + dayCardW - 25,
            y + 50
        );
        ...
        ctx.font = "18px Cobbler";
        ctx.fillText(
            `${Math.round(parseFloat(forecast.Temperature.Min))}°`,
            forecastX + dayCardW - 25,
            y + 75
        );
    ...
    ctx.textAlign = "center";
    ctx.fillText("Temperature Forecast", padding + chartW / 2, chartY - 10);
