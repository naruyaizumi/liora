export async function fileType(buffer) {
    ...
    return await Promise.resolve().then(() => {
        ...
        if (buffer.length >= 6 && 
            ...
            buffer[2] === 0x41 && buffer[3] === 0x4D && 
            buffer[4] === 0x52 && buffer[5] === 0x0A) {
        ...
        if (buffer.length >= 4 && 
            ...
            if (bufferStr.includes('[Content_Types].xml')) {
                if (bufferStr.includes('word/')) return { mime: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', ext: 'docx' };
                if (bufferStr.includes('ppt/')) return { mime: 'application/vnd.openxmlformats-officedocument.presentationml.presentation', ext: 'pptx' };
                if (bufferStr.includes('xl/')) return { mime: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', ext: 'xlsx' };
            }
        }
        ...
        if (buffer.length >= 7 && 
            ((buffer[0] === 0x52 && buffer[1] === 0x61 && buffer[2] === 0x72 && buffer[3] === 0x21) ||
             (buffer[0] === 0x52 && buffer[1] === 0x45 && buffer[2] === 0x7E && buffer[3] === 0x5E))) {
...
export async function getCategory(type) {
    ...
    return await Promise.resolve().then(() => {
        ...
        if (type.mime.includes('pdf') || type.mime.includes('document') || type.mime.includes('sheet') || type.mime.includes('presentation')) return 'document';
        if (type.mime.includes('zip') || type.mime.includes('rar')) return 'archive';
