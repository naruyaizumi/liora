...
let participants = await conn.groupMetadata(jid).then(res => res.participants.map(p => p.id))
...
console.error(`[ERROR JPMHT ${jid}]:`, err)
failed++
}
}
await m.reply(`🎐 *JPM (hidetag) selesai dikirim ke semua grup!*
🍢 *Berhasil: ${success}*
🍡 *Gagal: ${failed}*
