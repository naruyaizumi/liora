...
let handler = async (m, { conn, usedPrefix, command, args }) => {
  ...
  if (!/^https?:\/\/(www\.)?instagram\.com\//i.test(url)) {
    return m.reply(
      "🍰 *URL tidak valid! Kirimkan link Instagram yang benar, ya.*",
    );
  }
  ...
  try {
    ...
    for (let i = 0; i < album.length; i++) {
      ...
      if (item.type === "image") {
        await conn.sendMessage(
          ...
          {
            image: { url: item.url },
            caption: `*${item.caption} (${i + 1}/${album.length})*`,
          },
          { quoted: m },
        );
    ...
    console.error(err);
    m.reply(
      "🍮 *Terjadi kesalahan saat mengambil data dari Instagram. Coba lagi nanti!*",
    );
  } finally {
