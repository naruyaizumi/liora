...
if (!args[0] || !args[0].startsWith('http')) return m.reply('🍙 *Berikan link lagu Spotify yang valid ya~!*')
await global.loading(m, conn)
try {
let res = await fetch(global.API("btz", "/api/download/spotify", { url: args[0] }, "apikey"))
let json = await res.json()
if (!json.status || !json.result?.data?.url) return m.reply("🍤 *Gagal mengunduh lagu dari Spotify!*")
