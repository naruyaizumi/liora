...
await conn.sendFile(m.chat, video, "threads.mp4", `🍱 *Video Threads berhasil diunduh!*`, m, false, { mimetype: "video/mp4" })
return
}
if (Array.isArray(image_urls) && image_urls.length) {
for (let img of image_urls) {
if (!img) continue
await conn.sendFile(m.chat, img, "threads.jpg", "🍡 *Gambar Threads berhasil diunduh!*", m)
}
return
}
