let handler = async (m, { conn, text }) => {
    try {
        ...
        const decodedJid = await conn.signalRepository.lidMapping
            .getPNForLID(lid);
        if (!decodedJid) return m.reply(
            "Cannot resolve JID for this LID.");
        
        const [exists] = await conn.onWhatsApp(decodedJid);
        if (!exists?.exists) return m.reply(
            "This number is not registered on WhatsApp.");
        
        const pp = await conn.profilePictureUrl(decodedJid, "image")
            .catch(() => null) ||
            "https://qu.ax/jVZhH.jpg";
        
        const statusRes = await conn.fetchStatus(decodedJid).catch(() =>
            null);
        ...
        const bisnis = await conn.getBusinessProfile(decodedJid).catch(
        () => null);
        const businessHours = bisnis?.business_hours?.business_config
            ?.length ?
            bisnis.business_hours.business_config.map(cfg => {
        ...
        const title = bisnis?.description || bisnis?.category ?
            "WhatsApp Business" :
            "WhatsApp User";
...
`;
        ...
        await conn.sendMessage(
            m.chat, { image: { url: pp }, caption, mentions: [
                lid] }, { quoted: m }
        );
