...
let handler = async (m, { conn, usedPrefix, command, args }) => {
  ...
  const level =
    Number.isInteger(levelArg) && levelArg >= 1 && levelArg <= 5
      ? levelArg
      : null;
  ...
  if (!/^image\/(jpe?g|png|webp)$/i.test(mime)) {
    return m.reply(
      `🍪 *Balas/kirim gambar (jpg/png/webp) dengan perintah: ${usedPrefix + command} [1-5]*`,
    );
  }
  ...
  try {
    ...
    if (!input?.length)
      return m.reply("🍮 Terjadi kesalahan: Gagal mengunduh gambar.");

    const img = sharp(input, { failOn: "none" });
    const meta = await img.metadata().catch(() => ({}));
    const format = (meta.format || "").toLowerCase(); // 'jpeg' | 'png' | 'webp'
    ...
    if (format === "jpeg" || format === "jpg") {
      ...
      return m.reply("🍮 *Terjadi kesalahan: Format gambar tidak didukung.*");
    }

    const output = await pipe.toBuffer();
    if (!output?.length)
      return m.reply("🍮 *Terjadi kesalahan: Gagal mengompresi gambar.*");

    const afterBytes = output.length;
    const saved = Math.max(0, beforeBytes - afterBytes);
    const ratio = beforeBytes
      ? ((saved / beforeBytes) * 100).toFixed(1)
      : "0.0";

    const caption = [
      "🍰 *KOMPRES GAMBAR SELESAI*",
      "━━━━━━━━━━━━━━━━━━━",
      `🍧 *Level: ${autoLevel} (Q≈${quality})*`,
      `🧁 *Format: ${format} → ${outFormat}*`,
      `🍓 *Ukuran Sebelum: ${formatBytes(beforeBytes)}*`,
      `🍦 *Ukuran Sesudah: ${formatBytes(afterBytes)}*`,
      `🍮 *Hemat: ${formatBytes(saved)} (${ratio}%)*`,
    ].join("\n");
    ...
    console.error(e);
    await m.reply(`🍮 Terjadi kesalahan: ${e.message || e}`);
  } finally {
    await global.loading(m, conn, true);
  }
};

handler.help = ["compress [level 1-5]"];
handler.tags = ["tools"];
