...
let handler = async (m, { conn, usedPrefix, command, args }) => {
  ...
  if (!/^image\/(jpe?g|png|webp)$/i.test(mime)) {
    return m.reply(`🍪 *Balas/kirim gambar (jpg/png/webp) dengan perintah: ${usedPrefix + command} [1-5]*`)
  }

  await global.loading(m, conn)
  try {
    const input = await q.download()
    if (!input?.length) return m.reply("🍮 Terjadi kesalahan: Gagal mengunduh gambar.")
    ...
    if (!output?.length) return m.reply("🍮 *Terjadi kesalahan: Gagal mengompresi gambar.*")
