let handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        if (!text) return m.reply(`Enter TikTok username to search.\n› Example: ${usedPrefix + command} naruyaizumi`);
        ...
        if (!json.status || !json.result) return m.reply("TikTok account not found or may be private.");

        const { username, description, likes, followers, following, totalPosts, profile } = json.result;
