...
const TECH_TIP_ENDPOINT = '/api/tech/tip'
...
let res = await fetch(`${BASE_URL}${TECH_TIP_ENDPOINT}?page=${page}`)
...
throw new Error('Tidak ada artikel Tech Tip ditemukan')
}
...
▸ *Kategori:* ${article.category || 'Tech Tip'}
${article.price ? `▸ *Harga:* ${article.price}` : ''}
...
`💠 TECH TIP - HALAMAN ${page}\n\n${formattedArticles}\n\nGunakan *${usedPrefix}${command} <page>* untuk halaman lainnya`,
m, {
contextInfo: {
externalAdReply: {
title: `Tech Tip - Page ${page}`,
body: "Panduan dan tips teknologi",
...
console.error('Error fetching Tech Tip:', error)
...
handler.help = ['techtip']
handler.tags = ['info']
handler.command = /^(techtip)$/i
