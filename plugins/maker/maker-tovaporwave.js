...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/tovaporwave?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        if (!img1Url) throw new Error("Failed to process image to vaporwave style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Vaporwave-style transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
...
handler.help = ["tovaporwave"];
handler.tags = ["maker"];
handler.command = /^(tovaporwave)$/i;
