...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/towatercolor?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        if (!img1Url) throw new Error("Failed to process image to watercolor style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Watercolor-style transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
...
handler.help = ["towater"];
handler.tags = ["maker"];
handler.command = /^(towater)$/i;
