...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/tovintage?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        if (!img1Url) throw new Error("Failed to process image to vintage style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Vintage-style transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
...
handler.help = ["tovintage"];
handler.tags = ["maker"];
handler.command = /^(tovintage)$/i;
