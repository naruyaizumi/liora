...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/todisney?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        if (!img1Url) throw new Error("Failed to process image to Disney style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Disney-style transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
...
handler.tags = ["maker"];
handler.command = /^(todisney|disneyfy)$/i;
