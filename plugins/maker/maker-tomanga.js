...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/tomanga?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        if (!img1Url) throw new Error("Failed to process image to manga style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Manga-style transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
