...
let handler = async (m, { conn, usedPrefix, command }) => {
    try {
        ...
        let apiUrl = global.API("btz", "/api/maker/jadicartoon", { url: uploaded }, "apikey");

        let response = await fetch(apiUrl);
        if (!response.ok) throw new Error("Gagal memproses gambar ke kartun!");
        ...
        await conn.sendMessage(
            ...
            {
                image: buffer,
                caption: `🍡 *Berhasil mengubah gambar jadi kartun!*`,
            },
            { quoted: m }
        );
    } catch (e) {
