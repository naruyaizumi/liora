...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/tocartoon?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Cartoon transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
