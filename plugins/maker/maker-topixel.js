...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/topixelated?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        if (!img1Url) throw new Error("Failed to process image to pixelated style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Pixelated-style transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
