...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/tofigure?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        if (!img1Url) throw new Error("Failed to process image to figure style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Figure-style transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
...
handler.tags = ["maker"];
handler.command = /^(tofigure|figurefy)$/i;
