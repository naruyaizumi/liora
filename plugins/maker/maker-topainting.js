...
let handler = async (m, { conn }) => {
    try {
        ...
        const api = `https://api.nekolabs.web.id/tools/convert/tooilpainting?imageUrl=${encodeURIComponent(uploaded)}`;
        ...
        if (!img1Url) throw new Error("Failed to process image to oil painting style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Oil painting-style transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
...
handler.help = ["topainting"];
handler.tags = ["maker"];
handler.command = /^(topainting)$/i;
