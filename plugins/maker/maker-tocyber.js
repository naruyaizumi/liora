...
let handler = async (m, { conn }) => {
    try {
        ...
        if (!img1Url) throw new Error("Failed to process image to cyberpunk style.");

        await conn.sendMessage(
            m.chat,
            { image: { url: img1Url }, caption: "Cyberpunk transformation result." },
            { quoted: m }
        );
    } catch (e) {
        conn.logger.error(e);
