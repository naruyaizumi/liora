...
if (!mime) return m.reply(`🍚 *Reply gambar, gif, atau video dengan perintah ${usedPrefix + command}*`)
...
if ((q.msg || q).seconds > 30) return m.reply('🍧 *Maksimal durasi video adalah 30 detik!*')
let img = await q.download?.()
stiker = await mp4ToWebp(img, { pack: global.config.stickpack, author: global.config.stickauth, crop: false })
await conn.sendFile(m.chat, stiker, 'sticker.webp', '', m)
} else if (args[0] && isUrl(args[0])) {
stiker = await sticker(false, args[0], global.config.stickpack, global.config.stickauth, 20)
await conn.sendFile(m.chat, stiker, 'sticker.webp', '', m)
}
} finally {
