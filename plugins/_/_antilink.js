...
function normalizeUrl(raw) {
    return /^https?:\/\//i.test(raw) || /^ftp:\/\//i.test(raw) ? raw :
        `https://${raw}`
}
...
function cleanCache() {
    if (VALID_CACHE.size > MAX_CACHE_SIZE) {
        ...
        if (VALID_CACHE.size > MAX_CACHE_SIZE) {
            const entries = [...VALID_CACHE.entries()].sort((a, b) => a[1].ts -
                b[1].ts)
...
async function isValidUrlWithNetwork(url) {
    ...
    try {
        ...
        if (head && (head.ok || head.status === 405 || head.status ===
            403)) {
            ok = true
        }
        ...
        if (!ok) {
            ...
            const timeout2 = setTimeout(() => controller2.abort(),
                REQ_TIMEOUT)
    ...
    for (let i = 0; i < Math.min(MAX_CONCURRENT, queue.length); i++) {
        workers.push((async function worker() {
            while (queue.length) {
                ...
                if (!u) continue
                try {
                    const ok = await isValidUrlWithNetwork(
                        u)
                    if (ok) out.push(u)
                } catch {}
            }
...
export async function before(m, { conn, isOwner, isMods, isAdmin,
    isBotAdmin }) {
    try {
        ...
        const candidates = [...new Set(rawMatches
            .map(t => t.trim())
