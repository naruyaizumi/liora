...
if (!args[0]) return m.reply(`*Contoh: ${usedPrefix}${command} btc*`)
...
if (!info) return m.reply(`*Pair ${pairKey.toUpperCase()} tidak ditemukan.*`)
let ticker = summaries.tickers[`${coin}_idr`]
if (!ticker) return m.reply(`*Data ticker ${coin} tidak ditemukan.*`)
let canvas = createCanvas(800, 560)
let ctx = canvas.getContext('2d')
let bg = await loadImage('https://files.catbox.moe/5y6n22.jpg')
...
let dataRight = [
['Fee Maker', `${info.trade_fee_percent_maker}%`],
['Fee Taker', `${info.trade_fee_percent_taker}%`],
['Harga Beli', `Rp ${Number(ticker.buy).toLocaleString('id-ID')}`],
['Harga Jual', `Rp ${Number(ticker.sell).toLocaleString('id-ID')}`],
['Harga Terakhir', `Rp ${Number(ticker.last).toLocaleString('id-ID')}`]
]
...
drawSummaryBox(40, 430, 'Harga 24 Jam Terakhir', prices24h || '-')
drawSummaryBox(420, 430, 'Harga 7 Hari Terakhir', prices7d || '-')
let title = `📊 *Market Info ${info.description}*`
let prediksi = `🍮 *Analisa & Prediksi Pasar*\n\n`
prediksi += `📌 *Saat ini ${info.traded_currency_unit} berada di harga Rp ${Number(ticker.last).toLocaleString('id-ID')}.*\n`
prediksi += `📈 *Tertinggi: Rp ${Number(ticker.high).toLocaleString('id-ID')}*\n📉 *Terendah: Rp ${Number(ticker.low).toLocaleString('id-ID')}*\n\n`
let diff = Number(ticker.last) - Number(prices24h)
if (!isNaN(diff)) {
let persentase = (diff / prices24h * 100).toFixed(2)
let arah = diff > 0 ? 'naik' : 'turun'
let simbol = diff > 0 ? '🍫' : '🍭'
prediksi += `${simbol} *Harga ${arah} sekitar ${Math.abs(persentase)}% dibandingkan 24 jam terakhir.*\n\n`
} else {
prediksi += `🍩 *Perbandingan dengan 24 jam terakhir tidak tersedia.*\n\n`
}
prediksi += `🍡 *Volume: ${Number(ticker.vol_btc).toFixed(2)} ${info.traded_currency_unit}*\n`
prediksi += Number(ticker.vol_btc) > 50
 ? `🍰 *Aktivitas pasar tinggi — cocok untuk trader aktif!*\n`
 : `🧁 *Aktivitas pasar tenang — peluang entry lebih aman.*\n`
await conn.sendFile(m.chat, canvas.toBuffer(), `${info.symbol}.jpg`, `${title}\n\n${prediksi}`, m)
}
