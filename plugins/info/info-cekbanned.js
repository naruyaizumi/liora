...
if (!user.banned) {
return m.reply(`🍩 *${isSender ? 'Kamu' : `@${who.split('@')[0]}`} tidak sedang dibanned kok~*`, false, { mentions: [who] })
}
let now = Date.now()
let status = user.bannedTime == 999 ? '🍯 *Status: Banned Permanen~*' : `🍪 *Sisa Waktu:* ${msToDate(user.bannedTime - now)}`
let caption = `
🍓 *Status Banned*
━━━━━━━━━━━━━━
