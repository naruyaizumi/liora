...
let handler = async (m, { conn, args }) => {
    if (!args[0]) return m.reply('Enter the name of the village/sub-district, for example "Senen"');
    ...
    let [kode] =
        wilayahLengkap.find(([, full]) => {
            return keywords.every((k) => full.includes(k));
        }) || [];
    if (!kode)
        return m.reply(
            'Sorry, location not found. Please try writing the full name, for example "Senen, Central Jakarta, DKI Jakarta"'
        );
    ...
    let caption = `Regional Weather Forecast\n${lokasi.desa}, ${lokasi.kecamatan}, ${lokasi.provinsi}\nSource: BMKG.go.id`;
