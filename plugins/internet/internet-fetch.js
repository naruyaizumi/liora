    ...
    try {
      ...
      req.on("error", () => {
        resolve({
          ...
          res: { status: 0, statusText: "ERROR", headers: { get: () => null, raw: () => ({}) } },
          fallback: true,
        })
      })
      req.on("timeout", () => {
        ...
        resolve({
          buffer: Buffer.alloc(0),
          res: { status: 0, statusText: "TIMEOUT", headers: { get: () => null, raw: () => ({}) } },
          fallback: true,
        })
      })
    } catch {
      resolve({
        buffer: Buffer.alloc(0),
        res: { status: 0, statusText: "INVALID_URL", headers: { get: () => null, raw: () => ({}) } },
        fallback: true,
      })
    }
  })
...
let handler = async (m, { text, conn, usedPrefix, command }) => {
  ...
  while (redirects < 10) {
    const { buffer: buf, res, fallback } = await smartFetch(current, {
      redirect: "manual",
      headers: {
...
🍦 *Size: ${contentLength ? formatSize(contentLength) : formatSize(buffer.length)}*`
  ...
  if (buffer.length < 100 * 1024) {
    ...
    if (textPreview.length > 4000) textPreview = textPreview.slice(0, 4000) + "\n... (truncated)"
