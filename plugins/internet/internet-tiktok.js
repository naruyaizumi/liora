let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text) return m.reply(`Please enter a TikTok keyword!\nExample: ${usedPrefix + command} luxury girl`)
    
    await global.loading(m, conn)
    
    try {
        let res = await fetch(`https://api.elrayyxml.web.id/api/search/tiktok?q=${encodeURIComponent(text)}`)
        let json = await res.json()
        
        if (!json.status || !json.result || json.result.length === 0) throw '*No results found on TikTok!*'
