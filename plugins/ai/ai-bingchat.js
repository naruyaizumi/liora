...
try {
if (!text) return m.reply(`🍩 *Masukkan pertanyaan untuk Bing Chat AI!*\n\n🍰 *Contoh: ${usedPrefix + command} Apa itu relativitas waktu?*`)
await global.loading(m, conn)
let apiUrl = global.API("btz", "/api/search/bing-chat", { text }, "apikey")
let response = await fetch(apiUrl)
if (!response.ok) return m.reply('🍪 *Gagal memproses permintaan ke Bing Chat. Coba beberapa saat lagi.*')
