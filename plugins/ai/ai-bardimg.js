...
if (!text) return m.reply(`🍰 *Masukkan teks untuk analisis gambar!*\n\n🍡 *Contoh: ${usedPrefix + command} Jelaskan gambar ini!*`)
let apiUrl = global.API("btz", "/api/search/bard-img", { url: linkUpload, text }, "apikey")
let response = await fetch(apiUrl)
if (!response.ok) return m.reply('🍱 *Gagal memproses gambar melalui Bard AI. Coba beberapa saat lagi ya~*')
