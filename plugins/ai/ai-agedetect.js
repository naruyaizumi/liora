...
let handler = async (m, { conn, usedPrefix, command }) => {
  try {
    ...
    if (!media)
      return m.reply(
        "🍫 *Gagal mengunduh gambar! Pastikan file tidak rusak ya~*",
      );
    ...
    let apiUrl = global.API(
      "btz",
      "/api/search/agedetect",
      { url: linkUpload },
      "apikey",
    );
    let response = await fetch(apiUrl);
    if (!response.ok)
      return m.reply("🍰 *Terjadi kesalahan saat menganalisis gambar!*");
    let json = await response.json();
    if (!json.result)
      return m.reply("🍮 *Wajah tidak terdeteksi atau terjadi kesalahan!*");
