...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        ...
        if (!response.ok) return m.reply("🍱 *Terjadi kesalahan saat menghubungi Bard AI.*");
        let json = await response.json();
        let resultText = String(json?.result ?? "🍙 *Hasil tidak ditemukan!*");
