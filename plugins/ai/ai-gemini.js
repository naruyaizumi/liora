...
🩷 *Coba kirim pertanyaan atau media sekarang!*`,
        ...
        if (mime.includes("audio")) {
            ...
            if (!linkUpload) return m.reply("🍫 *Gagal mengunggah audio. Coba beberapa saat lagi ya~*");
            ...
            if (!response.ok) return m.reply("🍱 *Gagal memproses audio melalui Gemini. Coba lagi nanti!*");

            let json = await response.json();
            resultText = safeString(json?.result || "🍡 *Gemini tidak bisa mengenali isi audio ini!*");
            prompt = inputText;
            ...
            let linkUpload = await uploader(media).catch(() => null);
            if (!linkUpload) return m.reply("🍫 *Gagal mengunggah gambar. Coba beberapa saat lagi ya~*");

            if (!prompt)
                return m.reply(`🍬 *Masukkan teks untuk analisis gambar!*\n\n🍡 *Contoh: ${usedPrefix + command} Jelaskan gambar ini!*`);

            let apiUrl = global.API("btz", "/api/search/bard-img", { url: linkUpload, text: prompt }, "apikey");
            let response = await fetch(apiUrl);
            if (!response.ok) return m.reply("🍱 *Gagal memproses gambar melalui Gemini. Coba lagi nanti!*");
            ...
            let linkUpload = await uploader(media).catch(() => null);
            if (!linkUpload) return m.reply("🍫 *Gagal mengunggah video. Coba beberapa saat lagi ya~*");

            if (!prompt)
                return m.reply(`🍬 *Masukkan teks untuk analisis video!*\n\n🍡 *Contoh: ${usedPrefix + command} Tolong analisis video ini!*`);

            let apiUrl = global.API("btz", "/api/search/bard-video", { url: linkUpload, text: prompt }, "apikey");
            let response = await fetch(apiUrl);
            if (!response.ok) return m.reply("🍱 *Gagal memproses video melalui Gemini. Coba lagi nanti!*");
            ...
            let response = await fetch(apiUrl);
            if (!response.ok) return m.reply("🍫 *Ups! Gagal mengakses Gemini. Coba lagi nanti ya~*");

            let json = await response.json();
            resultText = safeString(json?.message || "🍪 *Gemini belum bisa jawab sekarang. Coba lagi nanti ya~*");
        }
