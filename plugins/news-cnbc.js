...
try {
let apiUrl = global.API('btz', '/api/news/cnbc', {}, 'apikey')
let response = await fetch(apiUrl)
let json = await response.json()
if (!json?.status || !Array.isArray(json.result))
return m.reply('❌ *Gagal mengambil berita dari CNBC.*')
let data = json.result.slice(0, 10)
let caption = `📰 *Rangkuman Berita CNBC Hari Ini:*\n\n` +
data.map((v, i) => `
*${i + 1}. ${v.berita.trim()}*
📖 *${v.berita_url}*
...
console.error(e)
m.reply('❌ *Gagal memuat berita dari CNBC.*')
}
}
