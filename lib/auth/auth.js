...
export function PostgresAuth(_databaseUrl, _options = {}) {
    ...
    function estimatePayloadSize(data) {
        try {
            ...
            logger.warn({ err: e.message }, "Failed to estimate payload size");
            return 0;
        }
    }
    ...
    function chunkData(flatData, maxSize = MAX_BATCH_SIZE) {
        ...
        for (const key of keys) {
            ...
            if ((currentSize >= maxSize || currentPayloadSize + itemSize > MAX_PAYLOAD_SIZE) && currentSize > 0) {
    ...
    function ensureCredsStructure(credsObj) {
        ...
        return {
            ...
            preKeys: Array.isArray(credsObj.preKeys) 
                ? credsObj.preKeys 
                : (Array.isArray(baseCreds.preKeys) ? baseCreds.preKeys : []),
        };
    }
    ...
    async function initCreds() {
        ...
        try {
            ...
            if (!isConnected) {
                logger.warn("API not accessible, using default credentials");
            ...
            if (response.ok) {
                ...
                if (data.value) {
                    try {
                        ...
                        creds = ensureCredsStructure(initAuthCreds());
                        logger.warn("Failed to parse creds, initializing new creds");
                    }
                } else {
                    logger.info("No existing creds in DB, using new credentials");
                    creds = ensureCredsStructure(initAuthCreds());
                }
            } else {
                logger.warn(
    ...
    async function saveCreds() {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to save credentials: ${response.status} ${response.statusText}`);
            }
            return true;
        } catch (e) {
            logger.error(
    ...
    const keys = {
            ...
            try {
                ...
                if (uncachedIds.length > 0) {
                    ...
                    for (const idChunk of idChunks) {
                        ...
                        if (!response.ok) {
                            throw new Error(`Failed to get keys: ${response.status} ${response.statusText}`);
                        }
                    ...
                    for (let i = 0; i < dataChunks.length; i++) {
                        ...
                        try {
                            ...
                            if (!response.ok) {
                                ...
                                throw new Error(`Failed to set keys (chunk ${i + 1}/${dataChunks.length}): ${response.status} ${errorText}`);
                            }
                        } catch (e) {
                            logger.error(
                                { 
                ...
                if (keysToDelete.length > 0) {
                    ...
                    for (let i = 0; i < deleteChunks.length; i++) {
                        ...
                        try {
                            ...
                            if (!response.ok) {
                                throw new Error(`Failed to delete keys (chunk ${i + 1}/${deleteChunks.length}): ${response.status} ${response.statusText}`);
                            }
                        } catch (e) {
                            logger.error(
                                { 
        ...
        async clear() {
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Failed to clear keys: ${response.status} ${response.statusText}`);
                }
                
                cache.clear();
            } catch (e) {
                logger.error({ err: e.message, stack: e.stack }, "keys.clear error");
            }
        },
    };
    ...
    async function getStats() {
        try {
            ...
            logger.error({ err: e.message, stack: e.stack }, "getStats error");
            return null;
        }
    }
