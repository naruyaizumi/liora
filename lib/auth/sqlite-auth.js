...
export function SQLiteAuth(_dbPath, options = {}) {
        ...
        return credsMutex.runExclusive(() => {
            ...
            try {
                ...
                if (row?.value) {
                    ...
                    if (!creds || typeof creds !== "object" || !creds.noiseKey || !creds.signedIdentityKey) {
                ...
                logger.error({ err: e, context: "initializeCreds" }, "Failed to initialize credentials");
                creds = initAuthCreds();
                credsInitialized = true;
                return creds;
            }
    ...
    async function commitWithRetry(mutations) {
        ...
        logger.debug({ mutationCount }, "Committing transaction");
        ...
        for (let attempt = 0; attempt < txOptions.maxCommitRetries; attempt++) {
            try {
                ...
                const commitTx = db.db.transaction(() => {
                    ...
                    for (const type in mutations) {
                        ...
                        for (const id in bucket) {
                            ...
                            if (!validateKey(k)) {
                                logger.debug({ key: k }, "Skipping invalid key");
                                continue;
                            }
                ...
                if (retriesLeft === 0) {
                    ...
                    throw new Error(`Transaction failed after ${attempt + 1} attempts: ${error.message}`);
                }
    ...
    async function transaction(work, key = "default") {
        ...
        return getTxMutex(key).runExclusive(async () => {
            ...
            logger.debug({ key }, "Starting transaction");
            ...
            try {
                ...
                logger.debug({ 
                    dbQueries: ctx.dbQueries,
                    operations: commitResult.operations,
                    duration,
                    key,
