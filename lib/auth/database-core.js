...
class AuthDatabase {
    ...
    _initDatabase(options = {}) {
        ...
        for (const [key, value] of Object.entries(pragmas)) {
            try {
                ...
                logger.error({ pragma: key, err: e.message }, "Pragma failed");
            }
        }
    ...
    set(key, value) {
        ...
        if (this.writeBuffer.size >= this.maxBufferSize) {
            this.flush().catch((e) => {
                this.stats.errors++;
                logger.error({ err: e }, "Force flush failed");
            });
        } else if (this.autoFlushEnabled) {
            this._scheduleFlush();
        }
            ...
            for (const stmt of statements) {
                if (stmt && typeof stmt.finalize === "function") {
                    try {
                        ...
                        logger.error({ err: e }, "Statement finalize failed");
                    }
                }
            }
