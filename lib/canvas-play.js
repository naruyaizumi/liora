...
export async function youtubeCanvas(coverUrl, title, artist) {
    ...
    const containerX = 1200 - containerWidth - 60;
    const containerY = (630 - containerHeight) / 2;
    ...
    try {
        const logo = await loadImage('https://upload.wikimedia.org/wikipedia/commons/thumb/0/08/YouTube_light_logo_%282024%29.svg/1280px-YouTube_light_logo_%282024%29.svg.png');
        ctx.save();
        ctx.globalAlpha = 0.9;
    ...
    const accentGradient = ctx.createLinearGradient(accentX, accentY, accentX, accentY + accentHeight);
    ...
    for (let i = 0; i < 5; i++) {
        ctx.beginPath();
        ctx.arc(containerX + containerWidth / 2 - (2 * dotSpacing) + (i * dotSpacing), dotY, dotRadius, 0, Math.PI * 2);
        ctx.fill();
    }
