...
static Napi::Value AddExif(const Napi::CallbackInfo &info) {
  ...
  std::string pack_name =
      meta.Has("packName") ? meta.Get("packName").ToString().Utf8Value() : "";
  ...
  if (result.empty()) {
    Napi::Error::New(env, "Failed to attach EXIF to WebP")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
...
static Napi::Value MakeSticker(const Napi::CallbackInfo &info) {
  ...
  if (info.Length() < 1 || !info[0].IsBuffer()) {
    Napi::TypeError::New(env, "sticker(buffer, options?)")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
  ...
  auto result =
      StickerCore::ProcessSticker(buffer.Data(), buffer.Length(), options);
