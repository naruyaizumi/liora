...
static Napi::Value AddExif(const Napi::CallbackInfo& info) {
    ...
    if (info.Length() < 2 || !info[0].IsBuffer() || !info[1].IsObject()) {
        Napi::TypeError::New(env, "addExif(buffer, {packName, authorName, emojis})").ThrowAsJavaScriptException();
        return env.Null();
    }
    ...
    std::string pack_name = meta.Has("packName") ? 
        meta.Get("packName").ToString().Utf8Value() : "";
    std::string author_name = meta.Has("authorName") ? 
        meta.Get("authorName").ToString().Utf8Value() : "";
...
static Napi::Value EncodeRGBA(const Napi::CallbackInfo& info) {
    ...
    if (info.Length() < 3 || !info[0].IsBuffer() || !info[1].IsNumber() || !info[2].IsNumber()) {
        Napi::TypeError::New(env, "encodeRGBA(buffer, width, height, options?)").ThrowAsJavaScriptException();
        return env.Null();
    }
    ...
    Napi::Object opt = (info.Length() >= 4 && info[3].IsObject()) ? 
        info[3].As<Napi::Object>() : Napi::Object::New(env);
    ...
    if (buffer.Length() < expected_size) {
        Napi::Error::New(env, "Buffer too small for specified dimensions").ThrowAsJavaScriptException();
        return env.Null();
    }
