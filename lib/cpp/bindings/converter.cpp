...
static Napi::Value Convert(const Napi::CallbackInfo& info) {
    ...
    options.format = opt.Has("format") ? 
        opt.Get("format").ToString().Utf8Value() : "opus";
    options.sampleRate = opt.Has("sampleRate") ? 
        opt.Get("sampleRate").ToNumber().Int32Value() : 48000;
    options.channels = opt.Has("channels") ? 
        opt.Get("channels").ToNumber().Int32Value() : 2;
    options.ptt = opt.Has("ptt") ? 
        opt.Get("ptt").ToBoolean() : false;
    options.vbr = opt.Has("vbr") ? 
        opt.Get("vbr").ToBoolean() : true;
    ...
    if (opt.Has("bitrate")) {
        ...
        if (br.IsNumber()) {
            ...
            options.bitrate = ConverterCore::ParseBitrate(
                br.ToString().Utf8Value(), 
                64000
            );
        }
    }
