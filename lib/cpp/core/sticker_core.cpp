...
bool IsWebPFormat(const uint8_t* data, size_t size) noexcept {
    if (!data || size < 12)
        return false;
    return std::memcmp(data, "RIFF", 4) == 0 && std::memcmp(data + 8, "WEBP", 4) == 0;
}

std::vector<uint8_t> BuildWhatsAppExif(const std::string& packName,
                                       const std::string& authorName,
                                       const std::vector<std::string>& emojis) noexcept {
    try {
        std::ostringstream json;
        json << "{" << "\"sticker-pack-id\":\"" << random_hex(16) << "\","
             << "\"sticker-pack-name\":\"" << packName << "\"," << "\"sticker-pack-publisher\":\""
             << authorName << "\"," << "\"emojis\":[";
        ...
        static const uint8_t tiff_header[] = {0x49, 0x49, 0x2A, 0x00, 0x08, 0x00, 0x00, 0x00,
...
std::vector<uint8_t> AttachExifToWebP(const std::vector<uint8_t>& webp_data,
...
static std::vector<uint8_t> resize_rgba_512(const uint8_t* rgba,
                                            int width,
                                            int height,
                                            bool crop) noexcept {
    ...
    try {
        ...
        SwsContext* sws = sws_getContext(src_w,
                                         src_h,
                                         AV_PIX_FMT_RGBA,
                                         TARGET_SIZE,
                                         TARGET_SIZE,
...
static std::vector<uint8_t> encode_webp_static(const uint8_t* rgba_512, int quality) noexcept {
    try {
        WebPConfig config;
        if (!WebPConfigPreset(&config, WEBP_PRESET_PICTURE, static_cast<float>(quality))) {
            return {};
        }
...
static std::vector<uint8_t> decode_first_frame_rgba(const uint8_t* input_data,
    ...
    AVIOContext* avio_ctx = avio_alloc_context(
            avio_buffer, AVIO_BUFFER_SIZE, 0, &buffer_ctx, read_packet, nullptr, seek_packet);
            ...
            if (avcodec_send_packet(codec_ctx, &packet) >= 0) {
                if (avcodec_receive_frame(codec_ctx, frame) >= 0) {
                    ...
                    if (sws_ctx) {
                        ...
                        uint8_t* dst_data[4] = {rgba_data.data(), nullptr, nullptr, nullptr};
...
StickerResult ProcessSticker(const uint8_t* input_data,
                             size_t input_size,
    ...
    try {
        ...
        if (IsWebPFormat(input_data, input_size)) {
            ...
            auto rgba = decode_first_frame_rgba(input_data, input_size, &width, &height);
