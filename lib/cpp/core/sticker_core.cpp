...
std::vector<uint8_t> BuildWhatsAppExif(
...
std::vector<uint8_t> AttachExifToWebP(
    const std::vector<uint8_t>& webp_data,
    const std::vector<uint8_t>& exif_data
) noexcept {
    if (webp_data.empty() || exif_data.empty()) return {};
...
static std::vector<uint8_t> resize_rgba_512(
    const uint8_t* rgba,
    int width,
    int height,
    bool crop
...
static std::vector<uint8_t> encode_webp_static(
    const uint8_t* rgba_512,
    int quality
) noexcept {
    try {
        WebPConfig config;
...
static std::vector<uint8_t> decode_first_frame_rgba(
    ...
    AVIOContext* avio_ctx = avio_alloc_context(
        avio_buffer, AVIO_BUFFER_SIZE, 0, &buffer_ctx,
...
std::optional<std::vector<uint8_t>> EncodeRGBA(
    ...
    if (width != 512 || height != 512) {
        ...
        if (rgba_512.empty()) return std::nullopt;
...
StickerResult ProcessSticker(
    const uint8_t* input_data,
    size_t input_size,
    const StickerOptions& options
) noexcept {
