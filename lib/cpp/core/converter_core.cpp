...
static AVSampleFormat pick_sample_fmt(const AVCodec *codec) {
  ...
  for (const AVSampleFormat *p = codec->sample_fmts; *p != AV_SAMPLE_FMT_NONE;
       ++p) {
...
static SwrContext *init_resampler(AVCodecContext *dec_ctx,
...
ConvertResult ConvertAudio(const uint8_t *input_data, size_t input_size,
  ...
  try {
    ...
    if (!io_buffer)
      return ConvertResult("Memory allocation failed");

    AVIOCtxGuard io_guard;
    io_guard.ctx = avio_alloc_context(io_buffer, 65536, 0, &buf_ctx,
    ...
    int audio_stream = av_find_best_stream(fmt_guard.ctx, AVMEDIA_TYPE_AUDIO,
                                           -1, -1, nullptr, 0);
    ...
    if (avcodec_parameters_to_context(dec_ctx_guard.ctx, in_stream->codecpar) !=
        0) {
    ...
    if (avformat_alloc_output_context2(&out_guard.ctx, nullptr,
                                       mux_name.c_str(), nullptr) != 0 ||
        !out_guard.ctx) {
    ...
    int sample_rate = (opts.sampleRate > 0) ? opts.sampleRate
                      : (dec_ctx_guard.ctx->sample_rate > 0)
                          ? dec_ctx_guard.ctx->sample_rate
                          : 48000;
    ...
    if (out_codec_id == AV_CODEC_ID_OPUS) {
      ...
      av_opt_set(enc_ctx_guard.ctx->priv_data, "vbr", (opts.vbr ? "on" : "off"),
                 0);
    }
    ...
    fifo_guard.fifo = av_audio_fifo_alloc(
        enc_ctx_guard.ctx->sample_fmt, out_channels,
        enc_ctx_guard.ctx->frame_size > 0 ? enc_ctx_guard.ctx->frame_size
                                          : 1024);
    ...
    while (av_read_frame(fmt_guard.ctx, packet) >= 0) {
      if (packet->stream_index == audio_stream) {
        ...
        while (ret >= 0) {
          ret = avcodec_receive_frame(dec_ctx_guard.ctx,
                                      decode_frame_guard.frame);
          if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
            break;
          if (ret < 0) {
            av_packet_free(&packet);
          ...
          int converted_samples =
              swr_convert(swr_guard.ctx, nullptr, 0,
          ...
          if (converted_samples > 0) {
            av_samples_alloc_array_and_samples(
                &converted_data, nullptr, out_channels, converted_samples,
                enc_ctx_guard.ctx->sample_fmt, 0);

            converted_samples = swr_convert(swr_guard.ctx, converted_data,
                                            converted_samples, nullptr, 0);

            if (converted_samples > 0) {
              av_audio_fifo_write(fifo_guard.fifo, (void **)converted_data,
                                  converted_samples);
            }
          ...
          while (av_audio_fifo_size(fifo_guard.fifo) >= frame_size) {
            ...
            encode_frame_guard.frame->sample_rate =
                enc_ctx_guard.ctx->sample_rate;

#if LIBAVUTIL_VERSION_MAJOR >= 57
            av_channel_layout_copy(&encode_frame_guard.frame->ch_layout,
                                   &enc_ctx_guard.ctx->ch_layout);
#else
            encode_frame_guard.frame->channel_layout =
                enc_ctx_guard.ctx->channel_layout;
    ...
    while (true) {
      int ret =
          avcodec_receive_frame(dec_ctx_guard.ctx, decode_frame_guard.frame);
      ...
      if (ret >= 0) {
        ...
        if (converted_samples > 0) {
          av_samples_alloc_array_and_samples(&converted_data, nullptr,
                                             out_channels, converted_samples,
                                             enc_ctx_guard.ctx->sample_fmt, 0);
    ...
    while (true) {
      ...
      av_samples_alloc_array_and_samples(&converted_data, nullptr, out_channels,
                                         converted_samples,
                                         enc_ctx_guard.ctx->sample_fmt, 0);
    ...
    while (av_audio_fifo_size(fifo_guard.fifo) > 0) {
      int samples_to_encode =
          std::min(av_audio_fifo_size(fifo_guard.fifo), frame_size);
