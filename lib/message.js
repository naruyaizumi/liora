...
processMessageStubType: {
  async value(m) {
    ...
    const name = Object.entries(WAMessageStubType)
      .find(([_, v]) => v === m.messageStubType)?.[0] || "UNKNOWN";
    const author = conn.decodeJid(m.key?.participant || m.participant || m.key?.remoteJid || "");
