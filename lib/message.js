...
logger: {
  get() {
    const log = (level, args) => {
      switch(level) {
        case 'info': loggerInstance.info(...args); break;
        case 'warn': loggerInstance.warn(...args); break;
        case 'error': loggerInstance.error(...args); break;
        case 'debug': loggerInstance.debug(...args); break;
        case 'trace': loggerInstance.trace?.(...args); break;
        default: loggerInstance.info(...args);
      }
    };
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        download: {
            value() {
                const t = this.mediaType;
                if (!t) return Promise.resolve(Buffer.alloc(0));
                return this.conn?.downloadM?.(this.mediaMessage[t], t.replace(/message/i, ""));
            },
            enumerable: true,
            configurable: true,
        },
