...
export function naruyaizumi(connectionOptions, options = {}) {
  ...
  async value(content, jids = []) {
    ...
    for (const id of jids) {
      if (isJidGroup(id)) {
        try {
          const metadata = await this.groupMetadata(id).catch(() => null)
    ...
    const backgroundColor = !isMedia ? content.backgroundColor || getRandomHexColor() : undefined
    const ptt = isAudio ? (typeof content.ptt === "boolean" ? content.ptt : true) : undefined

    const eph =
      this.chats[STORIES_JID]?.metadata?.ephemeralDuration ||
      this.chats[STORIES_JID]?.ephemeralDuration ||
      0
    ...
    for (const id of jids) {
      try {
        ...
        console.error(`Failed to send mention to ${id}: ${err.message}`)
      }
    }
