...
const _hidden = (target, key, value) =>
  Object.defineProperty(target, key, { value, enumerable: false, configurable: false, writable: true });
  ...
  Object.defineProperties(conn, {
    ...
    getFile: {
      async value(PATH, saveToFile = false) {
        ...
        const type =
          (await fileTypeFromBuffer(data).catch(_no)) || { mime: "application/octet-stream", ext: "bin" };
    ...
    sendFile: {
      async value(jid, path, filename = "", text = "", quoted, ptt = false, options = {}) {
        ...
        if (res?.status && res.status !== 200) {
          let err;
          try { err = JSON.parse(file.toString()); } catch { err = { status: res.status }; }
          throw err;
        }
        const opt =
        ...
        if (/webp/.test(type.mime) || (/image/.test(type.mime) && options.asSticker)) {
          ...
          file = finalBuffer.length > 5 * 1024 * 1024 ? Readable.from(finalBuffer) : finalBuffer;
          pathFile = null;
          mtype = "audio";
        ...
        for (const o of ["asSticker", "asLocation", "asVideo", "asDocument", "asImage"]) delete options[o];
        const message = {
          ...options,
          caption,
          ptt,
    ...
    reply: {
      async value(jid, text = "", quoted, options = {}) {
        ...
        if (global.db?.data?.settings?.[conn.user.jid]?.adReply) {
          ...
          return Buffer.isBuffer(text)
            ? conn.sendFile(jid, text, "file", "", quoted, false, options)
            : conn.sendMessage(jid, { text, ...options }, { quoted, ...options, ephemeralExpiration: ephemeral });
        }
      },
      enumerable: true,
    },
      ...
      async value() {
        try {
          ...
          for (const [gid, meta] of Object.entries(allGroups)) {
            ...
            if (chat.subject !== meta.subject) chat.subject = meta.subject || chat.subject || "";
            chat.metadata = meta;
            chat.isChats = true;
            chat.lastSync = now;
          }
      ...
      async value(m) {
        ...
        for (const message of arr) {
          ...
          try {
            ...
            if (mtype === "senderKeyDistributionMessage" || mtype === "messageContextInfo") {
              mtype = (mtypeKeys[1] && mtypeKeys[1] !== "messageContextInfo" && mtypeKeys[1]) || mtypeKeys[mtypeKeys.length - 1];
            }
            const chat = conn.decodeJid(
              message.key?.remoteJid || msgObj?.senderKeyDistributionMessage?.groupId || ""
            );
            ...
            if (ctx?.quotedMessage) {
              ...
              if (qChat && !_isStatusJid(qChat)) {
                const quotedClone = {
                  ...
                  message: qMsg,
                  ...(qChat.endsWith("@g.us") ? { participant: conn.decodeJid(ctx.participant) } : {}),
                };
            ...
            if (isGroup) {
              if (!chats.subject || !chats.metadata) {
                conn.groupMetadata(chat).then((md) => {
                  if (md) {
            ...
            if (mtype === "senderKeyDistributionMessage" || mtype === "messageContextInfo") continue;
            chats.isChats = true;
            chats.messages ||= Object.create(null);
            const fromMe = message.key?.fromMe || areJidsSameUser?.(sender || chat, conn.user?.id);
            if (mtype !== "protocolMessage" && !fromMe && message.message && message.messageStubType !== WAMessageStubType.CIPHERTEXT) {
...
export function serialize() {
  ...
  const firstMeaningfulType = (msg) => {
    ...
    if (!["senderKeyDistributionMessage", "messageContextInfo"].includes(keys[0])) return keys[0];
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    mtype: {
      get() {
        return memo(this, "mtype", () => (this.message ? firstMeaningfulType(this.message) : ""));
      },
      enumerable: true,
    },
    msg: {
    ...
    text: {
      get() {
        ...
        const alt =
          ...
          (primary?.paramsJson ? (() => { try { return JSON.parse(primary.paramsJson)?.id; } catch { return undefined; }})() : undefined) ||
          "";
