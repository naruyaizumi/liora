...
if (mode === 'search') {
let html = await get(`https://otakudesu.cloud/?s=${encodeURIComponent(query)}&post_type=anime`)
...
let genres = []
$(el).find('.set:contains("Genres") a').each((i, g) => genres.push($(g).text()))
let status = $(el).find('.set:contains("Status")').text().split(':')[1]?.trim()
let rating = $(el).find('.set:contains("Rating")').text().split(':')[1]?.trim() || 'N/A'
...
anonym: $('#venkonten .infozin > p:nth-child(1) > span').text().replace('Judul: ', ''),
japanese: $('#venkonten .infozin > p:nth-child(2) > span').text().replace('Japanese: ', '')
},
thumbnail: $('.fotoanime > img').attr('src'),
score: $('#venkonten .infozin > p:nth-child(3) > span').text().replace('Skor: ', ''),
producer: $('#venkonten .infozin > p:nth-child(4) > span').text().replace('Produser: ', ''),
...
total_eps: $('#venkonten .infozin > p:nth-child(7) > span').text().replace('Total Episode: ', ''),
duration: $('#venkonten .infozin > p:nth-child(8) > span').text().replace('Durasi: ', ''),
release: $('#venkonten .infozin > p:nth-child(9) > span').text().replace('Tanggal Rilis: ', ''),
studio: $('#venkonten .infozin > p:nth-child(10) > span').text().replace('Studio: ', ''),
