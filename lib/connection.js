...
async function connectionUpdateHandler(update) {
  ...
  switch (connection) {
    case "connecting":
      console.log(chalk.gray("[liora] Connecting..."))
      break
    case "open":
      console.log(chalk.green("[liora] Connected to WhatsApp."))
      break
    case "close":
      console.log(chalk.red("[liora] Connection closed — reconnecting..."))
      break
  }
    ...
    if (statusCode !== DisconnectReason.loggedOut) {
      setTimeout(async () => {
        try {
          ...
          console.log(chalk.yellow("[liora] Session reloaded after disconnect"))
        } catch (err) {
...
async function initReload(conn, pluginFolder, getAllPlugins) {
  ...
  async function loadPlugins() {
    ...
    conn.logger.info(`[liora] Plugins loaded: ${success} OK, ${failed} failed.`)
  }
  ...
  global.reload = async (_ev, filename) => {
    ...
    try {
      ...
      delete global.plugins[filename]
      conn.logger.info(`[liora] Plugin removed: ${filename}`)
      return
    }
    ...
    try {
      ...
      global.plugins[filename] = module.default || module
      conn.logger.info(chalk.green(`[liora] Reloaded: ${filename}`))
    } catch (err) {
      conn.logger.warn(chalk.red(`[liora] Reload error in '${filename}': ${err.message}`))
    }
  }
  ...
  schedule("pluginWatcherDebounce", async () => {
    ...
    if (Date.now() - lastChange >= debounce) {
      pending = false
      await global.reload(null, lastFile).catch((e) =>
        conn.logger.warn(`[liora] Debounced reload failed: ${e.message}`)
      )
    }
  }, { intervalSeconds: 1 })
