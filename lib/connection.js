...
import { schedule, fetch } from "liora-lib";
import chalk from "chalk";
import { checkGempa, clearTmp } from "./schedule.js";

async function getBaileysVersion() {
    try {
        const r = await fetch("https://raw.githubusercontent.com/Itsukichann/Baileys/master/lib/Defaults/baileys-version.json");
        return (await r.json())?.version || [2, 3000, 1026924051];
    } catch {
...
async function connectionUpdateHandler(update) {
    ...
    const dcReason = (() => {
        ...
        const raw =
            ...
            err?.code ??
            err?.errno ??
            (typeof err?.message === "string" && err.message.match(/\b\d{3,4}\b/)?.[0]) ??
            0;
    ...
    const tryRecover = () => {
        ...
        if (hardStop) {
            ...
            stopKeepAlive();
            conn.logger.error(`Auto-reconnect disabled for reason: ${dcReason}. Manual action required.`);
            return;
        }
        ...
        global.__reconnect.timer = setTimeout(async () => {
            ...
            try {
                ...
                conn.logger.info(`Reloaded session (attempt ${global.__reconnect.attempts}, reason: ${dcReason})`);
            } catch (e) {
                conn.logger.error("ReloadHandler failed:", e);
                global.__reconnect.attempts += 1;
            } finally {
                global.__reconnect.inflight = false;
            }
        }, delay);
        conn.logger.warn(`Scheduling reconnect in ${Math.ceil(delay/1000)}s (reason: ${dcReason})`);
    };
        ...
        async () => {
            ...
            if (Date.now() - lastChange >= debounce) {
                ...
                await global
                    ...
                    .catch((e) => conn.logger.warn(`Debounced reload failed: ${e.message}`));
            }
        },
        { intervalSeconds: 1 }
    );
...
export { getBaileysVersion, connectionUpdateHandler, initReload, initCron };
