function toMs(str) {
let match = str.match(/^(\d+)(s|m|h|d|w)$/i)
...
async function pagi(conn, x, data) {
    ...
    return await conn.adReply(x.room, pagii(x), 'W E R E W O L F', '', await resize(thumb1, 300, 175), false)
}
...
async function voting(conn, x, data) {
    ...
    return await conn.adReply(x.room, textnya, 'W E R E W O L F', '', await resize(thumb2, 300, 175), false)
}
...
async function malam(conn, x, data) {
    ...
    if (hasil_vote === 0) {
        ...
        return conn.adReply(x.room, textnya, 'W E R E W O L F', '', await resize(thumb3, 300, 175), false).then(() => {
            changeDay(x.room, data)
        ...
        if (hasil_vote.role === "werewolf") {
            ...
            return await conn.adReply(x.room, textnya, 'W E R E W O L F', '', await resize(thumb4, 300, 175), false).then(() => {
                changeDay(x.room, data)
                voteDone(x.room, data)
                resetVote(x.room, data)
                clearAllVote(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await conn.reply(x.player[i].id, textnya, false, { mentions: membernya })
                }
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "sorcerer" || x.player[i].role === "werewolf") {
                textnya += `${x.player[i].number}) @${x.player[i].id.replace("@s.whatsapp.net", "")}\n     *Role* : ${x.player[i].role}\n\n`
                ment.push(x.player[i].id)
            }
        }
        return await conn.adReply(sesinya, textnya, 'W E R E W O L F', '', await resize(thumb5, 300, 175), false).then(() => {
            delete data[x.room]
        })
        ...
        return await conn.adReply(sesinya, textnya, 'W E R E W O L F', '', await resize(thumb6, 300, 175), false).then(() => {
            delete data[x.room]
        })
    }
}
