...
class HotStore {
  ...
  _warmupCache() {
    try {
      ...
      for (const chat of chats) {
        try {
          const messages = this.cold.getMessages(
            chat.id,
      ...
      this.writeQueue.add(() => {
        try {
          if (typeof this.cold.cleanupOldMessages === 'function') {
            return this.cold.cleanupOldMessages(jid, this.config
              .maxMessagesPerChat)
          }
        } catch (e) {
          logger.error(e.message)
  ...
  async _flushPendingWrites() {
    ...
    if (pendingChatsSnapshot.size === 0 && pendingMessagesSnapshot.size ===
      0) {
      return
    }
  ...
  getStats() {
    try {
      const chatCacheStats = typeof this.chatCache.getStats === 'function' ?
        this.chatCache.getStats() : { keys: this.chatCache.keys().length }
      
      const metadataCacheStats = typeof this.metadataCache.getStats ===
        'function' ?
        this.metadataCache.getStats() : { keys: this.metadataCache.keys()
            .length }
