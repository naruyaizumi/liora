...
function safeTimestamp(timestamp) {
  ...
  if (typeof timestamp === 'number' && !isNaN(timestamp) && isFinite(
      timestamp)) {
...
`)
      ...
      this.stmts.getStat = this.db.prepare(
    ...
    try {
      return await this.mutex.runExclusive(() => {
        try {
          ...
          const lastMessageTime = safeTimestamp(chatData
            .lastMessageTime) || 0
          ...
          if (typeof chatData.unreadCount === 'number') {
            this.stmts.updateUnreadCount.run(chatData.unreadCount,
              chatData.id)
          }
        } catch (e) {
          logger.error(e.message)
    ...
    try {
      return await this.mutex.runExclusive(() => {
        try {
          ...
          this.stmts.upsertMessage.run(msgId, chatId, timestamp, fromMe,
            data)
          ...
          const transaction = this.db.transaction((msgs) => {
            ...
            for (const msg of msgs) {
              ...
              try {
                ...
                if (data) {
                  this.stmts.upsertMessage.run(msgId, chatId,
                    timestamp, fromMe, data)
                  
                  const currentMax = chatTimestamps.get(chatId) || 0
                  chatTimestamps.set(chatId, Math.max(currentMax,
                    timestamp))
                }
              } catch (e) {
    ...
    try {
      return await this.mutex.runExclusive(() => {
        try {
          ...
          const result = this.stmts.deleteOldMessages.run(chatId, chatId,
            keepCount)
          if (result.changes > 0) {
            // hmm...
          }
  ...
  _finalizeStatements() {
    ...
    for (const key of stmtKeys) {
      try {
        if (this.stmts[key] && typeof this.stmts[key].finalize ===
          'function') {
  ...
  async close() {
    return this.mutex.runExclusive(async () => {
      ...
      try {
        ...
        while (this.operationCount > 0 && (Date.now() - startTime) <
          maxWait) {
        ...
        if (this.operationCount > 0) {
          logger.warn({ pending: this.operationCount },
