...
async function initReload(conn, pluginFolder, getAllPlugins) {
        ...
        try {
            ...
            for (const filepath of files) {
                ...
                try {
                    ...
                    failed++;
                    conn.logger.warn(
                        `Failed to load plugin '${filename}': ${error.message}`
                    );
                }
            }
            
            conn.logger.info(
                `Plugins loaded: ${success} OK, ${failed} failed.`
            );
        } catch (e) {
    ...
    try {
        ...
        conn.logger.info(`Watching plugin folder: ${pluginFolder}`);
    } catch (e) {
        conn.logger.error(e.message);
    }
    ...
    return () => {
        ...
        conn.logger.info('Plugin watcher cleanup completed');
    };
}
