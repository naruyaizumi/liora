    ...
    Object.defineProperties(conn, {
        ...
        sendAlbum: {
            async value(jid, items = [], options = {}) {
                try {
                    ...
                    conn.logger.error("sendAlbum error:", error);
                    throw error;
                }
            },
            enumerable: true,
            ...
            async value(jid, text = "", quoted, options = {}) {
                ...
                if (global.db?.data?.settings?.[conn.user?.jid]?.adReply) {
                    return conn.sendMessage(
                        ...
                        { quoted, ephemeralExpiration: ephemeral }
                    );
                } else {
                    return conn.sendMessage(
                        jid,
        ...
        parseMention: {
            value(text = "") {
                ...
                for (const m of matches) {
                    const num = m[1];
                    if (num && num !== "0" && /^[0-9]{5,16}$/.test(num)) {
        ...
        getName: {
            value: async function (jid = "", withoutContact = false) {
                ...
                const self = conn.user?.id && areJidsSameUser 
        ...
        processMessageStubType: {
            async value(m) {
                ...
                conn.ev.emit("protocol.event", [{ id: chat, event: name, author, params }]);
            },
            enumerable: true,
        },
        insertAllGroup: {
                ...
                for (const message of arr) {
                    ...
                    try {
                        ...
                        if (ctx?.quotedMessage) {
                            ...
                            if (qChat && !_isStatusJid(qChat)) {
                                const quotedClone = {
                                    key: {
                                        remoteJid: qChat,
                                        fromMe: conn.user?.jid && areJidsSameUser 
                                ...
                                const qm = (conn.chats[qChat] ||= { id: qChat, isChats: !_isGroupJid(qChat) });
                        ...
                        if (isGroup) {
                            ...
                            sender = message.key?.fromMe && conn.user?.id 
                                ? conn.user.id 
                                : chat;
                        ...
                        const fromMe =
                            message.key?.fromMe || 
                            (conn.user?.id && sender && areJidsSameUser 
            ...
            ? {
                  chatRead: {
                      value(jid, participant, messageID) {
                          const participantJid = participant || conn.user?.jid;
                          if (!participantJid) {
                              conn.logger.warn("No participant or user JID available");
                              return Promise.resolve();
                          }
                          return conn.sendReadReceipt(jid, participantJid, [messageID]);
                      },
                      enumerable: true,
                  },
              }
                              ...
                              content: [
                                  {
                                      tag: "status",
                                      attrs: {},
                                      content: Buffer.from(String(status || ""), "utf-8"),
                                  },
                              ],
                          });
                      },
