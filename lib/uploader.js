...
async function uploader8(buffer, duration = "12M") {
    if (!buffer) throw new Error("Buffer tidak boleh kosong");
    const type = await fileTypeFromBuffer(buffer);
    if (!type) throw new Error("Format file tidak dikenali");
    ...
    try {
        const blob = new Blob([buffer], { type: type.mime });
        const form = new FormData();
        form.append("elrayyxml", blob, `upload.${type.ext}`);
        form.append("duration", duration);

        const res = await fetch("https://s.elrayyxml.my.id/upload", {
            method: "POST",
            body: form,
        });
        ...
        const json = await res.json().catch(() => null);
        if (!json?.url) {
            throw new Error("Upload gagal atau response tidak valid dari ElrayyXML");
        }

        return json.url.trim();
    } catch (err) {
        console.error("Uploader8 (ElrayyXML) error:", err.message || err);
        return null;
    }
}
...
async function uploader9(buffer) {
    ...
    try {
        ...
        const res = await fetch("https://cnd.abella.icu/upload", {
            method: "POST",
            headers: {
                ...DEFAULT_HEADERS,
        ...
        const json = await res.json().catch(() => null);
        if (!json) {
            throw new Error("Upload gagal atau response tidak valid dari Abella");
        }

        return json.url?.trim?.() || json;
    } catch (err) {
        console.error("Uploader9 (Abella) error:", err.message || err);
        return null;
    }
}
...
async function uploader10(buffer) {
    ...
    try {
        ...
        const json = await res.json().catch(() => null);
        if (!json?.files?.[0]?.url) {
            throw new Error("Upload gagal atau response tidak valid dari YPNK");
        }

        return `https://cdn.ypnk.biz.id${json.files[0].url}`.trim();
    } catch (err) {
        console.error("Uploader10 (YPNK) error:", err.message || err);
        return null;
    }
}

async function uploader11(buffer) {
    if (!buffer) throw new Error("Buffer tidak boleh kosong");
