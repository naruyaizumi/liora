...
async function uploader2(buffer) {
    ...
    try {
        ...
        if (!json?.files?.[0]?.url)
            throw new Error("Upload failed or invalid response from Uguu.");
...
async function uploader4(buffer) {
    ...
    try {
        ...
        const json = await res.json().catch(() => null);
        if (!json?.direct_url)
            throw new Error("Upload failed or invalid response from Put.icu.");
...
async function uploader5(buffer) {
    ...
    try {
        ...
        const json = await res.json().catch(() => null);
        if (!json?.data?.url)
            throw new Error("Upload failed or invalid response from Tmpfiles.");
