...
GlobalFonts.registerFromPath(join(process.cwd(), "lib", "Cobbler-SemiBold.ttf"), "Cobbler-Bold");
...
export async function canvas(videos, query) {
    ...
    const eyeIcon = await loadIcon("https://cdn-icons-png.flaticon.com/512/709/709612.png");
    const chartIcon = await loadIcon("https://cdn-icons-png.flaticon.com/512/3135/3135757.png");
    const videoIcon = await loadIcon("https://cdn-icons-png.flaticon.com/512/2991/2991147.png");
    const clockIcon = await loadIcon("https://cdn-icons-png.flaticon.com/512/2088/2088617.png");
    ...
    const totalViews = videos.slice(0, 6).reduce((sum, v) => sum + (v.views || 0), 0);
    const avgViews = Math.floor(totalViews / 6);
    const totalDuration = videos
        .slice(0, 6)
        .reduce((sum, v) => sum + parseDuration(v.timestamp), 0);
    ...
    const statsCards = [
        ...
        { title: "TOTAL VIEWS", value: formatNumber(totalViews), icon: eyeIcon, color: "#007AFF" },
        { title: "AVG VIEWS", value: formatNumber(avgViews), icon: videoIcon, color: "#34C759" },
        {
            title: "AVG DURATION",
    ...
    for (let i = 0; i < displayVideos.length && i < 6; i++) {
        ...
        if (thumbnails[i]) {
            ...
            if (video.timestamp) {
                ...
                ctx.textAlign = "center";
                ctx.fillText(video.timestamp, badgeX + badgeW / 2, badgeY + badgeH / 2 + 4);
            }
        }
    ...
    ctx.fillText(`Top 10 videos progression for "${query}"`, padding + 30, chartsY + 85);

    drawBarChart(ctx, padding, chartsY + 100, chartCardW, chartCardH - 100, videos);
