...
function drawCard(ctx, x, y, w, h, r = 20, isDark = false) {
    ...
    grad.addColorStop(0, isDark ? "rgba(255, 255, 255, 0.08)" :
...
function drawProgressCircle(ctx, cx, cy, radius, percent, color, isDark =
    false) {
    const lineWidth = 12;
    
    ctx.save();
    ctx.strokeStyle = isDark ? "rgba(255, 255, 255, 0.1)" :
        "rgba(0, 0, 0, 0.06)";
...
export async function canvas(systemData) {
    ...
    ctx.textAlign = "right";
    ctx.fillText(dateStr, W - P, P + 2);
    
    ctx.fillStyle = c.secondary;
    ctx.font = "bold 22px Cobbler";
    ctx.fillText(timeStr, W - P, P + 28);
    ...
    const mainCardW = (W - P * 2 - G * 2) / 3;
    const mainCardH = 220;
    
    const cpuPct = Math.max(0, Math.min(parseFloat(systemData.cpu.load1Pct),
        100));
    const ramPct = Math.max(0, systemData.ram.ramTotal > 0 ? (systemData.ram
        .ramUsed / systemData.ram.ramTotal) * 100 : 0);
    const diskPct = Math.max(0, systemData.disk.total > 0 ? (systemData.disk
        .used / systemData.disk.total) * 100 : 0);
    ...
    ctx.font = "bold 18px Cobbler";
    ctx.textAlign = "right";
    ctx.fillText(systemData.kernel.hostname.split('.')[0], P + infoCardW -
        28, infoY);
    ...
    ctx.font = "bold 18px Cobbler";
    ctx.textAlign = "right";
    ctx.fillText(systemData.uptimeSys, P + infoCardW - 28, infoY +
        infoLineH);
    ...
    ctx.font = "bold 18px Cobbler";
    ctx.textAlign = "right";
    ctx.fillText(systemData.proc.nodeVersion, P + infoCardW - 28, infoY +
        infoLineH * 2);
    ...
    ctx.font = "bold 18px Cobbler";
    ctx.textAlign = "right";
    ctx.fillText(formatSize(systemData.network.rx), P + infoCardW * 2 + G -
        28, netY);
    ...
    ctx.font = "bold 18px Cobbler";
    ctx.textAlign = "right";
    ctx.fillText(formatSize(systemData.network.tx), P + infoCardW * 2 + G -
        28, netY + infoLineH);
    
    ctx.fillStyle = c.secondary;
    ctx.font = "14px Cobbler";
    ctx.textAlign = "left";
    ctx.fillText("Total Packets:", P + infoCardW + G + 28, netY +
        infoLineH * 2);
    ctx.fillStyle = c.primary;
    ctx.font = "bold 18px Cobbler";
    ctx.textAlign = "right";
    ctx.fillText(
        ...
        P + infoCardW * 2 + G - 28, netY + infoLineH * 2);
    ...
    const chartData = Array.from({ length: 30 }, (_, i) => {
        ...
        if (t <= 0.33) {
            const localT = t / 0.33;
            value = cpuLoad15Pct + (cpuLoad5Pct - cpuLoad15Pct) *
                localT;
        } else if (t <= 0.66) {
            const localT = (t - 0.33) / 0.33;
            value = cpuLoad5Pct + (cpuLoad1Pct - cpuLoad5Pct) *
                localT;
        } else {
            const localT = (t - 0.66) / 0.34;
            const variation = Math.sin(localT * Math.PI * 3) * 2 + (
                Math.random() - 0.5) * 1.5;
    ...
    drawSmoothChart(ctx, P + 28, chartAreaY, chartW - 56, chartAreaH,
        chartData, c.accent, true);
    ...
    const healthPct = Math.round(100 - ((cpuPct + ramPct + diskPct) / 300 *
        100));
    
    const healthCX = P + chartW + G + chartW / 2;
    const healthCY = Y + chartH / 2 + 5;
    drawProgressCircle(ctx, healthCX, healthCY, 75, healthPct, c.success,
        true);
    
    ctx.fillStyle = c.white;
    ctx.font = "bold 56px Cobbler";
    ctx.textAlign = "center";
    ctx.fillText(`${healthPct}%`, healthCX, healthCY +
        18);
    
    ctx.fillStyle = "rgba(255, 255, 255, 0.7)";
    ctx.font = "14px Cobbler";
    ctx.fillText("Overall Health", healthCX, healthCY +
        44);
    
    const heapPct = systemData.heap.heapTotal > 0 ? (systemData.heap
        .heapUsed / systemData.heap.heapTotal) * 100 : 0;
    ...
    const items = [
        ...
        { label: "System Uptime:", value: systemData.uptimeSys },
        {
            label: "Heap Memory:",
    ...
    items.forEach((item, index) => {
        ...
        ctx.font = "bold 18px Cobbler";
        ctx.fillText(item.value, P + chartW + G + 28, y +
            labelOffset);
    });
