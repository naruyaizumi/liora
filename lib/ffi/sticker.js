...
function encodeWebP(rgbaBuffer, width, height, quality = 80) {
  ...
  if (rgbaBuffer.length < expectedSize) {
    throw new Error(`Buffer too small: expected at least ${expectedSize} bytes, got ${rgbaBuffer.length}`);
  }
...
async function attachExifCLI(webpBuffer, exifBuffer) {
  ...
  try {
    ...
    try {
      execSync(
        `webpmux -set exif "${exifFile}" "${inputFile}" -o "${outputFile}"`,
        {
          stdio: "ignore",
...
async function attachExif(webpBuffer, exifBuffer) {
  ...
  try {
    const setResult = webpmux.symbols.WebPMuxSetChunk(
      mux,
      "EXIF",
      ptr(exifData),
      1
    ...
    const assembleResult = webpmux.symbols.WebPMuxAssemble(
      mux,
      ptr(outputData)
    );
