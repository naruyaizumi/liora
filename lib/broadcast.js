    ...
    for (const id of groups) {
        try {
            ...
            if (type === "imageMessage" && buffer) {
                await conn.sendMessage(id, { image: buffer, caption: teks || content.caption || "" });
            } else if (type === "videoMessage" && buffer) {
                await conn.sendMessage(id, { video: buffer, caption: teks || content.caption || "" });
            } else if (type === "audioMessage" && buffer) {
                const isPTT = content.ptt === true;
                await conn.sendMessage(id, {
                    audio: buffer,
                    ptt: isPTT,
                    mimetype: content.mimetype || (isPTT ? "audio/ogg; codecs=opus" : "audio/mpeg"),
                });
            } else {
                const extra = {};
                if (ht) {
                ...
                const finalText = teks || content.text || content.caption || "📢 Broadcast tanpa teks";
