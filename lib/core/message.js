...
export function serialize() {
    ...
    const safeGet = (o, k) => (o && Object.prototype.hasOwnProperty.call(o, k) ?
        o[k] : undefined);
    const firstMeaningfulType = (msg) => {
        const keys = fastKeys(msg);
        if (!keys.length) return "";
        if (!["senderKeyDistributionMessage", "messageContextInfo"]
            .includes(keys[0]))
            return keys[0];
        if (keys.length >= 3 && keys[1] !== "messageContextInfo")
        return keys[1];
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        mtype: {
            get() {
                return memo(this, "mtype", () =>
                    this.message ? firstMeaningfulType(this
                        .message) : ""
                );
            },
            enumerable: true,
        ...
        mediaMessage: {
            get() {
                return memo(this, "mediaMessage", () => {
                    if (!this.message) return null;
                    const env = getMediaEnvelope(this
                        .message, this.msg);
                    if (!env) return null;
                    const t = fastKeys(env)[0];
                    return MediaType.includes(t) ? env :
                        null;
                });
            },
            enumerable: true,
        ...
        quoted: {
            get() {
                return memo(this, "quoted", () => {
                    ...
                    if (!baseMsg || !ctx || !quoted)
                    return null;
                    const type = fastKeys(quoted)[0];
                    if (!type) return null;
                    const rawNode = quoted[type];
                    const textNode = typeof rawNode ===
                        "string" ? rawNode : rawNode?.text;
                    const base = typeof rawNode ===
                        "string" ? { text: rawNode } :
                        rawNode || {};
            ...
            get() {
                ...
                if (msg?.nativeFlowResponseMessage) {
                    ...
                    if (flowMsg.paramsJson) {
                        try {
                            const parsed = JSON.parse(flowMsg
                                .paramsJson);
                            if (parsed?.id) return String(parsed
                            .id);
                        } catch (err) {
                            console.debug(
                                "Failed to parse nativeFlowResponseMessage:",
                                err);
                        }
                    }
                }
                
                // Check for hydrated template
                const hydratedText = msg?.hydratedTemplate
                    ?.hydratedContentText || "";
        ...
        download: {
            value() {
                ...
                return this.conn
                    .downloadM(this.mediaMessage[t], t.replace(
                        /message/i, ""))
                    .catch((err) => {
        ...
        getQuotedObj: {
            value() {
                ...
                try {
                    ...
                    console.debug("Failed to get quoted object:",
                    error);
                    return null;
                }
            },
