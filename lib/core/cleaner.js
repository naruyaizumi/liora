...
export class CleanupManager {
    ...
    async cleanup() {
        logger.debug(
            `Cleanup manager: ${this.intervals.size} intervals, ${this.timeouts.size} timeouts, ${this.cleanupFC.size} functions, ${this.eventHandlers.size} event types`
        );
        ...
        logger.debug("Cleanup manager: All cleared");
    }
}
...
export async function shutdown(signal, cleanupManager) {
    ...
    const emergency = setTimeout(() => {
        logger.error("\n[EMERGENCY] Shutdown taking too long (>8s), FORCE EXITING!");
        process.exit(1);
    }, 8000);
    ...
    try {
        ...
        logger.info("Cleanup manager finished");
