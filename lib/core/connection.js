...
/**
 * Handles connection state changes and implements reconnection logic with exponential backoff.
 * Monitors WhatsApp connection events and automatically attempts to recover from disconnections.
...
async function DisconnectReason(update) {
    ...
    const backoff = (baseMs, factor = 1.8, maxMs = 60_000) => {
        ...
        const raw = Math.min(maxMs, Math.round(baseMs * Math.pow(factor,
            n)));
        
        // Add random jitter (20-50% of delay) to avoid synchronized reconnections
        const jitter = raw * (0.2 + Math.random() * 0.3);
        return Math.max(500, raw + Math.round((Math.random() < 0.5 ? -
            1 : 1) * jitter));
    };
    
    /**
     * Parse and categorize the disconnection reason from error codes.
     * Maps WebSocket, HTTP, and system error codes to human-readable categories.
     */
    const dcReason = (() => {
        ...
        const raw =
            ...
            (typeof e?.message === "string" && e.message.match(
                /\b\d{3,4}\b/)?.[0]) ??
            0;
        ...
        switch (code) {
            ...
            default: {
                ...
                if (msg.includes("replaced") && msg.includes(
                        "session"))
            ...
            return "connection_reset";
                ...
                if (msg.includes("unauthoriz") || msg.includes(
                        "forbidden")) return "forbidden";
                if (msg.includes("unpaired"))
    ...
    /**
     * Start periodic keep-alive heartbeat to maintain connection health.
     * Updates timestamp every 45 seconds to prevent idle timeout.
     */
    ...
    return; // Already running
        ...
        }, 45_000); // 45 second interval
    };
    
    /**
     * Stop keep-alive heartbeat timer.
     * Called when connection is closed or entering cooldown.
     */
    const stopKeepAlive = () => {
        if (global.__reconnect.keepAliveTimer) {
    ...
    /**
     * Attempt to recover from disconnection with intelligent retry logic.
     * Implements exponential backoff, cooldowns, and reason-specific delays.
     */
    const tryRecover = () => {
        ...
        let baseDelay = 1_000; // Default 1 second
        let hardStop = false; // Flag for non-recoverable errors
        ...
        if (global.__reconnect.attempts >= 6) {
            global.__reconnect.cooldownUntil = Date.now() + 5 *
            60_000; // 5 minutes
            global.__reconnect.attempts = 0;
            conn.logger.warn(
                "Too many consecutive failures; entering 5m cooldown."
                );
            return;
        }
    ...
    if (lastDisconnect?.error) {
        if (["logged_out", "device_unpaired", "pairing_required"].includes(
                dcReason)) {
            conn.logger.error(
                `Session requires manual fix (${dcReason}). No auto-reconnect.`
                );
        } else {
            tryRecover();
        }
