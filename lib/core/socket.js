    ...
    Object.defineProperties(conn, {
        ...
        getFile: {
            async value(PATH, saveToFile = false) {
                ...
                if (!Buffer.isBuffer(data)) throw new TypeError(
        ...
        sendFile: {
            async value(
                ...
                const fileName =
                    filename ||
                    `file-${hash.toString(16).substring(0, 16)}.${type.ext}`;
        ...
        sendInviteGroup: {
            async value(jid, participant, inviteCode, inviteExpiration,
                groupName = "Unknown Subject", caption =
                "Invitation to join my WhatsApp group",
                jpegThumbnail = null, options = {}) {
                if (!this.user?.id) {
                    throw new Error("User not authenticated");
                }
                ...
                const msg = proto.Message.create({
                    groupInviteMessage: {
                        inviteCode,
                        inviteExpiration: parseInt(
                                inviteExpiration) || Date
                            .now() + (3 * 24 * 60 * 60 *
                                1000),
                        groupJid: jid,
                        groupName,
                        jpegThumbnail: Buffer.isBuffer(
                                jpegThumbnail) ?
                            jpegThumbnail : null,
                        caption,
                        contextInfo: {
                ...
                return await this.relayMessage(participant, message
                    .message, {
        ...
        sendStickerPack: {
            async value(jid, packData, options = {}) {
                if (!this.user?.id) throw new Error(
                    "User not authenticated");
                
                const { name, publisher, description, cover,
                    stickers, packId } = packData.stickerPack;
                
                if (!stickers || stickers.length === 0) {
                    throw new Error("Stickers array is required");
                }
                
                if (stickers.length < 3) {
                    throw new Error(
                        "Sticker pack requires at least 3 stickers"
                        );
                }
                
                const { zip } = await import("fflate");
                const { sticker: createSticker } = await import(
                    "#add-on");
                
                const stickerData = {};
                const stickerPromises = stickers.map(async (s,
                i) => {
                    let buffer;
                    
                    if (Buffer.isBuffer(s.sticker)) {
                        buffer = s.sticker;
                    } else if (typeof s.sticker ===
                        "object" && s.sticker.url) {
                        const res = await fetch(s
                            .sticker.url);
                        buffer = Buffer.from(await res
                            .arrayBuffer());
                    } else if (typeof s.sticker ===
                        "string") {
                        if (/^https?:\/\//.test(s
                        ...
                        throw new Error(
                            `Invalid sticker format at index ${i}`
                            );
                    }
                ...
                if (Buffer.isBuffer(cover)) {
                    ...
                    if (/^https?:\/\//.test(cover)) {
                        ...
                        coverBuffer = Buffer.from(await res
                            .arrayBuffer());
                    } else {
                        const file = Bun.file(cover);
                        coverBuffer = Buffer.from(await file
                            .arrayBuffer());
                    }
                }
                ...
                try {
                    ...
                    const coverUpload =
                await prepareWAMessageMedia({ image: coverBuffer }, { upload: this
                            .waUploadToServer });
                    
                    const coverImage = coverUpload.imageMessage;
                    const imageDataHash = Buffer.from(await crypto
                            .subtle.digest("SHA-256", coverBuffer))
                        .toString('base64');
                    
                    const packUpload =
                await prepareWAMessageMedia({ document: zipBuffer,
                        fileName: "stickerpack.zip" },
                    {
                        upload: this.waUploadToServer,
                        mediaTypeOverride: "document"
                    });
                    
                    const fileSha256 = Buffer.from(await crypto
                        .subtle.digest("SHA-256", zipBuffer));
                    const fileEncSha256 = fileSha256;
                    const mediaKey = crypto.getRandomValues(
                        new Uint8Array(32));
                    
                    const stickerPackMessage = proto.Message
                        .StickerPackMessage.fromObject({
                            name: name || "Sticker Pack",
                            publisher: publisher || "",
                            stickerPackId,
                            packDescription: description || "",
                            stickerPackOrigin: proto.Message
                                .StickerPackMessage
                            ...
                            directPath: packUpload
                                .documentMessage?.directPath ||
                                "/",
                            fileLength: zipBuffer.length,
                            mediaKeyTimestamp: Math.floor(Date
                                .now() / 1000),
                            trayIconFileName: `${stickerPackId}.png`,
                            imageDataHash,
                            thumbnailDirectPath: coverImage
                                .directPath,
                            thumbnailFileSha256: coverImage
                                .fileSha256,
                            thumbnailFileEncSha256: coverImage
                                .fileEncSha256,
                            thumbnailHeight: 96,
                            thumbnailWidth: 96,
                            contextInfo: {
                ...
                .create({ stickerPackMessage });
                    ...
                    this.logger?.error({ err: e.message, stack: e
                                .stack },
                        "sendStickerPack failed");
                    throw e;
                }
            },
            enumerable: true,
