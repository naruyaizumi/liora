...
*/
            ...
            async value(jid, items = [], options = {}) {
                try {
                    ...
                    const messageContent = {
                        ...
                        albumMessage: {
                            expectedImageCount: items.filter((
                                a) => a?.image).length,
                            expectedVideoCount: items.filter((
                                a) => a?.video).length,
                        },
                    };
                    ...
                    await this.relayMessage(album.key.remoteJid,
                        album.message, {
                    ...
                    for (const content of items) {
                        ...
                        await this.relayMessage(mediaMsg.key
                            .remoteJid, mediaMsg.message, {
        ...
        parseMention: {
            value(text = "") {
                if (!_isStr(text)) return [];
                const out = [];
                const matches = text.matchAll(_MENTION_RE);
                for (const m of matches) {
                    const num = m[1];
                    if (num && num !== "0" && /^[0-9]{5,16}$/.test(
                            num)) {
        ...
        getName: {
            value: async function(jid = "", withoutContact = false) {
                ...
                const self =
                    conn.user?.id && areJidsSameUser ?
                    areJidsSameUser(jid, conn.user.id) : false;
            ...
            value(messageID) {
                ...
                for (const chatData of Object.values(conn.chats ||
                    ...
                    if (messages[messageID]) {
                        conn._msgIndex.set(messageID, messages[
                            messageID]);
            ...
            async value() {
                try {
                    ...
                    for (const jid in existing) {
                        if (_isGroupJid(jid) && !allGroups[jid] &&
                            existing[jid].lastSync !== now) {
                            delete existing[jid];
                        }
                    }
                    ...
                    try {
                        ...
                        if (isGroup) {
                            ...
                            sender = message.key?.fromMe && conn
                                .user?.id ? conn.user.id : chat;
                            
                            if (!chats.name) {
                                chats.name = message.pushName ||
                                    chats.name || "";
                            }
                        }
                        ...
                        const fromMe =
                            ...
                            (conn.user?.id && sender &&
                                areJidsSameUser ?
                                areJidsSameUser(sender, conn.user
                                    .id) :
                                false);
        ...
        {
            chatRead: {
                value(jid, participant, messageID) {
                    const participantJid = participant || conn.user
                        ?.jid;
                    if (!participantJid) {
                        conn.logger.warn(
                            "No participant or user JID available"
                        );
                        return Promise.resolve();
                    }
                    return conn.sendReadReceipt(jid, participantJid,
                        [messageID]);
                },
                enumerable: true,
            },
    ...
    if (conn.user?.id) {
        conn.user.jid = conn.decodeJid(conn.user.id);
    }
