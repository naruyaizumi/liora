        ...
        */
        sendAlbum: {
            async value(jid, content, options = {}) {
                ...
                if (!content?.album || !Array.isArray(content
                        .album) || content.album.length === 0) {
                    throw new Error(
                ...
                const expectedImageCount = items.filter(item => item
                    ?.image).length;
                const expectedVideoCount = items.filter(item => item
                    ?.video).length;
