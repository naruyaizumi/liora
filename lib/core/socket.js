...
export function naruyaizumi(connectionOptions, options = {}) {
    ...
    Object.defineProperties(conn, {
        ...
        sendFile: {
            async value(jid, path, filename = "", text = "", quoted,
                ptt = false, options = {}) {
                const ephemeral =
                ...
                if (/webp/.test(type.mime) || (/image/.test(type
                        .mime) && options.asSticker)) {
                    mtype = "sticker";
                } else if (/image/.test(type.mime) || (/webp/.test(
                        type.mime) && options.asImage)) {
                    mtype = "image";
                    ...
                    if (ptt) {
                        return await this.sendPTT(jid, finalBuffer,
                            options, quoted);
                    }
                ...
                for (const o of ["asSticker", "asLocation",
                        "asVideo", "asDocument", "asImage"
                    ])
                    delete options[o];
        ...
        sendPTT: {
            async value(jid, audioBuffer, options = {}, quoted) {
                if (!options.waveform) {
                    options.waveform = await this.generateWaveform(
                        audioBuffer);
                }
                ...
                try {
                    const { default: decode } = await import(
                        "audio-decode");
                    const decodedAudio = await decode(
                        audioBuffer.buffer.slice(
                            audioBuffer.byteOffset,
                            audioBuffer.byteOffset + audioBuffer
                            .byteLength
                        )
                    );
                    duration = Math.floor(decodedAudio.duration);
                } catch (err) {
                    this.logger?.debug?.(
                        "Duration calculation failed:", err
                        .message);
                    duration = Math.floor(audioBuffer.length /
                    8000);
                }
                
                const media =
            await prepareWAMessageMedia({ audio: audioBuffer }, { upload: this
                            .waUploadToServer }
                );
                ...
                .message, { messageId: msg.key.id });
            },
            enumerable: true,
        },
        generateWaveform: {
            async value(audioBuffer) {
                try {
                    const { default: decode } = await import(
                        "audio-decode");
                    ...
                    const samples = 64;
                    const blockSize = Math.floor(rawData.length /
                        samples);
                    const peaks = [];
                    ...
                    const normalized = peaks.map(rms => {
                    ...
                    return Buffer.from(new Uint8Array(normalized));
                } catch (err) {
                    this.logger?.debug?.(
                        "Waveform generation failed, using fallback:",
                        err.message
                    );
                    
                    const fallbackWaveform = new Uint8Array(64);
                    for (let i = 0; i < 64; i++) {
                        fallbackWaveform[i] = Math.floor(Math
                            .random() * 40 + 30);
                    }
                    return Buffer.from(fallbackWaveform);
                }
            },
            enumerable: true,
        },
