    ...
    Object.defineProperties(conn, {
        ...
        sendStickerPack: {
            async value(jid, packData, options = {}) {
                ...
                const { name, publisher, description, cover, stickers, packId } =
                    packData.stickerPack;
                ...
                if (stickers.length < 3) {
                    throw new Error("Sticker pack requires at least 3 stickers");
                }

                const { zip } = await import("fflate");
                const { sticker: createSticker } = await import("#add-on");
                ...
                const stickerPromises = stickers.map(async (s, i) => {
                    ...
                    if (Buffer.isBuffer(s.sticker)) {
                        buffer = s.sticker;
                    } else if (typeof s.sticker === "object" && s.sticker.url) {
                        const res = await fetch(s.sticker.url);
                        buffer = Buffer.from(await res.arrayBuffer());
                    } else if (typeof s.sticker === "string") {
                        if (/^https?:\/\//.test(s.sticker)) {
                            const res = await fetch(s.sticker);
                            buffer = Buffer.from(await res.arrayBuffer());
                        } else {
                            const file = Bun.file(s.sticker);
                            buffer = Buffer.from(await file.arrayBuffer());
                        }
                    } else {
                        throw new Error(`Invalid sticker format at index ${i}`);
                    }
                    ...
                    const stickerOpts = {
                        ...
                        fps: 30,
                        maxDuration: 10,
                        packName: name || global.config?.stickpack || "",
                        authorName: publisher || global.config?.stickauth || "",
                        emojis: s.emojis || [],
                    };

                    const stickerBuffer = await createSticker(buffer, stickerOpts);

                    const hash = Buffer.from(
                        await crypto.subtle.digest("SHA-256", stickerBuffer)
                    ).toString("base64url");
                    const fileName = `${hash}.webp`;

                    stickerData[fileName] = [new Uint8Array(stickerBuffer), { level: 0 }];
                ...
                if (Buffer.isBuffer(cover)) {
                    ...
                    if (/^https?:\/\//.test(cover)) {
                        ...
                        coverBuffer = Buffer.from(await file.arrayBuffer());
                    }
                }
                ...
                try {
                    ...
                    const imageDataHash = Buffer.from(
                        await crypto.subtle.digest("SHA-256", coverBuffer)
                    ).toString("base64");

                    const packUpload = await prepareWAMessageMedia(
                        { document: zipBuffer, fileName: "stickerpack.zip" },
                        {
                            upload: this.waUploadToServer,
                    ...
                    const fileSha256 = Buffer.from(
                        await crypto.subtle.digest("SHA-256", zipBuffer)
                    );
                    ...
                    const stickerPackMessage = proto.Message.StickerPackMessage.fromObject({
                        ...
                        directPath: packUpload.documentMessage?.directPath || "/",
                    ...
                    const msg = proto.Message.create({ stickerPackMessage });
