    ...
    Object.defineProperties(conn, {
        ...
        sendFile: {
            async value(jid, path, filename = "", text = "", quoted, ptt = false, options = {}) {
                ...
                if (/webp/.test(type.mime) || (/image/.test(type.mime) && options.asSticker)) {
                    ...
                    if (ptt) {
                        return await this.sendPTT(jid, finalBuffer, options, quoted);
                    }
        ...
        sendPTT: {
            async value(jid, audioBuffer, options = {}, quoted) {
                if (!options.waveform) {
                    options.waveform = await this.generateWaveform(audioBuffer);
                }
                ...
                try {
                    ...
                    this.logger?.debug?.("Duration calculation failed:", err.message);
                ...
                return await this.relayMessage(jid, msg.message, { messageId: msg.key.id });
            },
            enumerable: true,
        },
        generateWaveform: {
            async value(audioBuffer) {
                try {
                    ...
                    const normalized = peaks.map((rms) => {
                        ...
                        return Math.min(100, Math.max(5, Math.floor(compressed * 100)));
                    });
                    ...
                    for (let i = 0; i < 64; i++) {
                        fallbackWaveform[i] = Math.floor(Math.random() * 40 + 30);
                    }
