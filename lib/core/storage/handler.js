    ...
    const handlers = {
                ...
                for (const chatData of chatsArray) {
                    ...
                    if (isGroup(id) && !manager.getGroupMeta(id)) {
                        manager.fetchGroupMetadata(conn, id).catch(err => {
                            logger.debug({ err, id }, "Failed to fetch group metadata on chat upsert");
                        });
                    }
                }
        ...
        "group-participants.update": async ({ id, participants, action }) => {
            try {
                ...
                logger.debug({ id, participants: participants?.length, action }, "Group participants updated");
            } catch (err) {
        ...
        "presence.update": async ({ id, presences }) => {
            try {
                ...
                logger.trace({ id, count: Object.keys(presences || {}).length }, "Presence updated");
            } catch (err) {
