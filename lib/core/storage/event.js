        ...
        "contacts.upsert": async (contacts) => {
            ...
            await Promise.all(
                contactArray.map(async (contact) => {
                    ...
                    if (isGroup(id)) {
                        const newSubject = contact.subject || contact.name || existing.subject || "";
                        if (newSubject) update.subject = newSubject;
                    } else {
                        const newName =
                            contact.notify || contact.name || existing.name || existing.notify || "";
...
export default function bind(conn) {
    ...
    if (!("loadMessage" in conn)) conn.loadMessage = async (jid, id) => (await manager.getMessage(id)) || null;
