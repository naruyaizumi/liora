...
export class StoreManager extends EventEmitter {
            ...
            mutex.runExclusive(async () => {
                try {
                    ...
                    if (metadata) {
                        ...
                        logger.debug(
                            { id, participants: metadata.participants?.length },
                            "Group metadata fetched"
                        );
                    }
    ...
    _logMetrics() {
        const hitRates = {
            ...
            group:
                this.metrics.hits.group / (this.metrics.hits.group + this.metrics.misses.group) ||
                0,
            message:
                this.metrics.hits.message /
                    (this.metrics.hits.message + this.metrics.misses.message) || 0,
            contact:
                this.metrics.hits.contact /
                    (this.metrics.hits.contact + this.metrics.misses.contact) || 0,
        };
