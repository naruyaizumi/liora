...
export default function bind(conn, options = {}) {
    ...
    conn.searchChats = (query) => {
        ...
        for (const [id, chat] of manager.chatCache.entries()) {
            ...
            if (name.includes(lowerQuery) || contactName.includes(lowerQuery) || id.includes(lowerQuery)) {
                results.push({
                    ...chat,
                    contact,
                });
    ...
    conn.getStoreStats = () => {
        ...
        return {
            ...
            health: {
                ...
                cacheUtilization: {
                    chats: (metrics.caches.chats.size / metrics.caches.chats.max * 100).toFixed(2) + "%",
                    groups: (metrics.caches.groups.size / metrics.caches.groups.max * 100).toFixed(2) + "%",
                    messages: (metrics.caches.messages.size / metrics.caches.messages.max * 100).toFixed(2) + "%",
                    contacts: (metrics.caches.contacts.size / metrics.caches.contacts.max * 100).toFixed(2) + "%",
                },
                hitRates: {
                    chat: (metrics.hits.chat / (metrics.hits.chat + metrics.misses.chat) * 100).toFixed(2) + "%",
                    group: (metrics.hits.group / (metrics.hits.group + metrics.misses.group) * 100).toFixed(2) + "%",
                    message: (metrics.hits.message / (metrics.hits.message + metrics.misses.message) * 100).toFixed(2) + "%",
                    contact: (metrics.hits.contact / (metrics.hits.contact + metrics.misses.contact) * 100).toFixed(2) + "%",
                },
            },
        };
    };
