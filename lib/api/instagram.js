...
export async function instagram(url) {
    ...
    for (const endpoint of endpoints) {
        ...
        if (json.result?.media && typeof json.result.media === "string" && json.result.isVideo === true) {
            return {
                success: true,
                type: "video",
        ...
        if (json.result?.media && Array.isArray(json.result.media) && json.result.isVideo === false) {
        ...
        if (Array.isArray(raw)) {
            const formatZenz = raw.every(
                (item) =>
                    typeof item === "object" &&
                    ("videoUrl" in item || "imageUrl" in item)
            );
