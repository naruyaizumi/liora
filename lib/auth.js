...
export function SQLiteKeyStore(dbPath = DEFAULT_DB, options = {}) {
        ...
        try {
            if (cache.size >= cacheMax && !cache.has(k)) {
                ...
                if (removed < evictCount) {
                    ...
                    for (let i = 0; i < evictCount - removed; i++) {
    ...
    function deepClone(obj) {
        ...
        try {
            ...
            logger.error({ err: e, context: "deepClone" }, "Clone failed");
            return CLONE_FAILED;
        }
    }
        ...
        return await txKeyMutex.runExclusive(async () => {
            ...
            if (!isOutermost) {
                try {
                    ...
                    if (savedCache === CLONE_FAILED || savedMutations === CLONE_FAILED) {
                        throw new Error("Failed to clone transaction state");
                    }
                } catch (e) {
                    logger.error({ err: e, context: "transaction-backup" }, "Transaction backup failed");
                    transactionsInProgress -= 1;
                    throw e;
                }
            } else {
            ...
            try {
                    ...
                    await writeMutex.runExclusive(async () => {
                        ...
                        if (upserts.length || deletes.length) {
                            try {
                                ...
                                logger.error({ err: e, context: "transaction-commit" }, "Transaction commit failed");
                                throw e;
                            }
                        }
                    });
                ...
                logger.error({ err: e, context: "transaction-work" }, "Transaction work failed");
    ...
    async function _getMany(type, ids) {
        ...
        for (const id of missing) {
            ...
            try {
                ...
                logger.error({ err: e, type, id, context: "_getMany" }, "Failed to get value");
            }
        }
        return out;
    }
                ...
                try {
                    ...
                    if (optimize) {
                        try {
                            ...
                            logger.warn({ err: e, context: "dispose-optimize" }, "Optimization failed");
                        }
                    }
                    if (ownDb && db.open) {
                        db.close();
                    ...
                    logger.error({ err: e, context: "dispose" }, "Disposal failed");
                    throw e;
                }
            })();
    ...
    const instanceCleanup = () => {
        ...
        if (!disposed && db.open) {
            ...
            try {
                ...
                logger.error({ err: e, context: "instanceCleanup" }, "Cleanup failed");
            }
        }
    };
