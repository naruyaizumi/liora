...
export function SQLiteKeyStore(dbPath = DEFAULT_DB, options = {}) {
    ...
    async function transaction(work, key = "default") {
        ...
        return await txKeyMutex.runExclusive(async () => {
            ...
            if (isOutermost) {
                ...
                savedCache = txnCache ? JSON.parse(JSON.stringify(txnCache)) : null;
                savedMutations = txnMutations ? JSON.parse(JSON.stringify(txnMutations)) : null;
            }
            ...
            try {
                ...
                logger.error(e);
                if (!isOutermost && savedCache && savedMutations) {
                    txnCache = savedCache;
    ...
    let signalsAttached = false;
    if (ownDb && (options.attachSignals ?? true) && !signalsAttached) {
        signalsAttached = true;
