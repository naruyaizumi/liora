...
const HISTORY_FILE = path.join(process.cwd(), "tmp", "metrics.json");
...
export async function getPostgreSQLInfo() {
    try {
        ...
        if (status === "Running") {
            try {
                const connResult = await $`sudo -u postgres psql -t -c "SELECT count(*) FROM pg_stat_activity;" 2>/dev/null`.text();
            ...
            try {
                const sizeResult = await $`sudo -u postgres psql -t -c "SELECT pg_size_pretty(sum(pg_database_size(datname))::bigint) FROM pg_database;" 2>/dev/null`.text();
    ...
    try {
        ...
        if (status === "Running") {
            try {
                ...
                if (ping.trim() === "PONG") {
                    try {
                        const memResult = await $`redis-cli info memory 2>/dev/null | grep '^used_memory_human:' | cut -d: -f2`.text();
...
export function getWarnings(cpu, ram, disk, inodes) {
    const warnings = [];
...
export async function saveMetricsHistory(cpu, ram, disk, heap) {
    try {
        const tmpDir = path.dirname(HISTORY_FILE);
        if (!fs.existsSync(tmpDir)) {
            fs.mkdirSync(tmpDir, { recursive: true });
        }

        let history = { cpu: [], memory: [], disk: [], heap: [] };
        ...
        history.cpu.push({ time: timestamp, value: parseFloat(cpu.load1Pct) });
        history.memory.push({ time: timestamp, value: ram.ramTotal > 0 ? (ram.ramUsed / ram.ramTotal) * 100 : 0 });
        history.disk.push({ time: timestamp, value: disk.total > 0 ? (disk.used / disk.total) * 100 : 0 });
        history.heap.push({ time: timestamp, value: heap.heapTotal > 0 ? (heap.heapUsed / heap.heapTotal) * 100 : 0 });

        history.cpu = history.cpu.slice(-maxPoints);
        history.memory = history.memory.slice(-maxPoints);
        history.disk = history.disk.slice(-maxPoints);
        history.heap = history.heap.slice(-maxPoints);
