...
export async function getCPUInfo() {
    try {
        ...
        for (const line of lines) {
            if (line.startsWith("model name")) model = line.split(":")[1]
                .trim();
            if (line.startsWith("processor")) cores++;
            if (line.startsWith("cpu MHz") && speed === 0) speed =
...
export async function getDiskUsage() {
    try {
        const { stdout } = await execAsync(
            "df -k / --output=size,used,avail -B1 | tail -1");
...
export async function getNetworkStats() {
    try {
        ...
        return { rx: totalRx, tx: totalTx, rxPackets: totalRxPackets,
...
export function getWarnings(cpu, ram, disk) {
    ...
    const ramUsagePct = ram.ramTotal > 0 ? (ram.ramUsed / ram.ramTotal) * 100 :
        0;
