  ...
  async value(
    ...
    if (
      ...
      try {
        ...
        waveform = Array.from({ length: 40 }, (_, i) => Math.abs(channel[i * step] || 0))
      } catch {/*  ignore  */}
...
sendAlbum: {
  async value(jid, items = [], options = {}) {
    ...
    await this.relayMessage(album.key.remoteJid, album.message, { messageId: album.key.id });

    for (const content of items) {
      const msg = await this.prepareMessageFromContent(album.key.remoteJid, content, {
        upload: this.waUploadToServer,
        ephemeralExpiration: options?.quoted?.expiration ?? 0,
      });
  ...
  enumerable: true,
    ...
    cMod: {
      value(jid, message, text = "", sender = conn.user.jid, options = {}) {
        ...
        if (
          copy.key.remoteJid.endsWith("@s.whatsapp.net") ||
          copy.key.remoteJid.endsWith("@lid")
        )
    ...
    insertAllGroup: {
      async value() {
        const groups =
          (await conn.groupFetchAllParticipating().catch((_) => null)) || {}; // eslint-disable-line no-unused-vars
    ...
    text: {
      get() {
        ...
        return (
          ...
          (text?.paramsJson ? JSON.parse(text.paramsJson)?.id : undefined) ||
          ""
        );
      },
      set(str) {
