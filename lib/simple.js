                              ...
                              jid,
                              {
                                  ...
                                  contextInfo: {
                                      externalAdReply: {
                                          ...
                                          thumbnailUrl: "https://qu.ax/yqEpZ.jpg",
                                          mediaType: 1,
...
parseMention: {
        value(text = "") {
            return [...text.matchAll(/@([0-9]{5,20}|0)/g)].map((v) => {
                let jid = v[1] + "@s.whatsapp.net"
                for (const chat of Object.values(conn.chats || {})) {
                    const participants = chat?.metadata?.participants
                    if (!participants) continue
                    const p = participants.find(u => u.lid === jid || u.id === jid)
                    if (p?.id) return p.id
                }
                return jid
            })
...
quoted: {
    get() {
        ...
        return Object.defineProperties(
            JSON.parse(JSON.stringify(typeof q === "string" ? { text: q } : q)),
            {
                ...
                chat: {
                    get() {
                        return (contextInfo.remoteJid || self.chat || "").decodeJid();
                    },
                    enumerable: true,
                },
                isBaileys: {
                ...
                sender: {
                    get() {
                        let jid = contextInfo.participant || this.chat || "";
                        if (jid.endsWith("@lid")) {
                            try {
                                const participants = self.conn?.chats[self.chat]?.metadata?.participants || [];
                                const p = participants.find(u => u.lid === jid);
                                if (p?.id) jid = p.id;
                            } catch {
                                // ignore
                ...
                mentionedJid: {
                    get() {
                        let jids = q.contextInfo?.mentionedJid || self.getQuotedObj()?.mentionedJid || [];
                        return jids.map(jid => {
                            if (jid.endsWith("@lid")) {
                                try {
                                    const participants = self.conn?.chats[self.chat]?.metadata?.participants || [];
    ...
    enumerable: true,
        ...
        mentionedJid: {
    get() {
        const raw = this.msg?.contextInfo?.mentionedJid || []
