...
import { makeWASocket, proto, areJidsSameUser, WAMessageStubType, extractMessageContent, jidDecode, downloadContentFromMessage, generateWAMessage, generateForwardMessageContent, generateWAMessageFromContent } from 'baileys'
...
filename = path.join(global.__dirname(import.meta.url), '../tmp/' + (+new Date) + '.' + type.ext)
...
let ephemeral = this.chats[jid]?.metadata?.ephemeralDuration || this.chats[jid]?.ephemeralDuration || false
let caption = text
...
let m = null
try {
m = await this.sendMessage(jid, message, { ...opt, ...options, ephemeralExpiration: ephemeral })
} catch (e) {
console.error(e)
}
if (!m) {
...
metadata = await conn.groupMetadata(chat).catch(_ => ({})) || {} // eslint-disable-line no-unused-vars
...
return typeof this._text === 'string' ? this._text : (typeof text === 'string' ? text : (text?.selectedDisplayText || text?.hydratedTemplate?.hydratedContentText || JSON.parse(text?.paramsJson)?.id || text)) || ''
},
set(str) {
this._text = str
},
