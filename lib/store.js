...
const FLUSH_MS = 40;
const FLUSH_MAX = 256;
const MSG_TTL_MS = Number(process.env.LIORA_MSG_TTL_MS || 0); // 0=off
...
function assignIf(obj, key, val) { if (val == null) return; if (obj[key] !== val) obj[key] = val; }
function ensureChat(conn, jid) { if (!conn.chats) conn.chats = Object.create(null); return (conn.chats[jid] ||= { id: jid }); }

function maybeTrimMessages(chat) {
  const msgs = chat.messages; if (!msgs) return;
  const keys = Object.keys(msgs); const n = keys.length;
...
function openDB() {
  const filename = "file:liora-store?mode=memory&cache=shared";
  const db = new Database(filename, { timeout: 5000, fileMustExist: false });
  ...
  const ps = {
    ...
    metaSet : db.prepare("INSERT INTO meta(key,val) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET val=excluded.val"),
...
async function getGroupMetadataCached(conn, jid) {
  ...
  if (cache.inflight.has(jid)) {
    try { return await cache.inflight.get(jid); } catch {/* ignore */}
  }
  const p = (async () => {
    try {
      ...
      if (md && Array.isArray(md.participants)) {
        for (const p of md.participants) {
          if (p.id && p.lid) {
            try { conn.lidCache.set(p.id, p.lid); conn.lidCache.set(p.lid, p.id); } catch {/* ignore */}
            try { conn.lidMappingStore?.put?.(p.id, p.lid); } catch {/* ignore */}
          }
        }
      }
...
function buildHandlers(conn, dirty) {
  ...
  h.updateContacts = function (payload) {
    ...
    for (const contact of list) {
      const id = contact?.id; if (isStatusJid(id)) continue;
      const chat = ensureChat(conn, id);
      if (isGroupJid(id)) assignIf(chat, "subject", contact.subject || contact.name || chat.subject || "");
      else assignIf(chat, "name", contact.notify  || contact.name || chat.name || chat.notify || "");
      for (const [k, v] of Object.entries(contact)) { if (k === "id") continue; if (v != null) chat[k] = v; }
      try {
        ps.contactUp.run(id, JSON.stringify({ id, name: chat.name || null, imgUrl: chat.imgUrl || null }));
      } catch {/* ignore */}

      maybeTrimMessages(chat);
      markContact(id); markChat(id);
    }
  };
  ...
  h.onChatsSet = async function ({ chats }) {
    for (let { id, name, readOnly } of chats || []) {
      ...
      if (name) { if (isGroupJid(id)) assignIf(chat, "subject", name); else assignIf(chat, "name", name); }

      if (isGroupJid(id)) {
        const md = await getGroupMetadataCached(conn, id);
        if (md) { assignIf(chat, "subject", chat.subject || md.subject); chat.metadata = md; markGroup(id); }
      }
  ...
  h.onGroupsUpdate = async function (updates) {
    for (const upd of updates || []) {
      const id = upd?.id; if (isStatusJid(id) || !isGroupJid(id)) continue;
      const chat = ensureChat(conn, id);
      chat.isChats = true;
      const md = await getGroupMetadataCached(conn, id);
      if (md) {
  ...
  h.onChatsUpsert = function (chatsUpsert) {
    const id = chatsUpsert?.id; if (isStatusJid(id)) return;
  ...
  h.onPresenceUpdate = function ({ id, presences }) {
    const sender = (presences && Object.keys(presences)[0]) || id; if (!sender) return;
...
export function bind(conn) {
  ...
  ensureGroupCache(conn);
  if (!conn.lidCache) Object.defineProperty(conn, "lidCache", { value: new Map(), enumerable: false });
  const lidCache = conn.lidCache;
  const lidMappingStore = {
    put(pn, lid) {
      if (!pn || !lid) return false;
      try { lidCache.set(pn, lid); lidCache.set(lid, pn); ps.lidUp.run(pn, lid, Date.now()); return true; }
      catch { return false; }
    },
    async getLIDForPN(pnOrJid) {
      ...
      if (!lid) { try { lid = ps.lidByPN.get(key)?.lid || null; } catch {/* ignore */} }
      if (!lid && typeof conn.onWhatsApp === "function") {
        try {
          const r = await conn.onWhatsApp([key]).catch(() => null);
          const x = Array.isArray(r) ? r[0] : null;
          if (x?.exists && x?.lid) { lid = x.lid; try { lidMappingStore.put(key, lid); } catch {/* ignore */} }
        } catch {/* ignore */}
      }
      return lid || null;
    },
    async getPNForLID(lid) {
      ...
      if (!pn) {
        try { pn = ps.pnByLID.get(lid)?.pn || null; } catch {/* ignore */}
        if (pn) { try { lidCache.set(lid, pn); lidCache.set(pn, lid); } catch {/* ignore */} }
      }
      return pn || null;
    },
    cache: lidCache,
  ...
  const markChat = (jid) => { dirtyChats.add(jid); schedule(); };
  const markGroup = (jid) => { dirtyGroups.add(jid); schedule(); };
  const markContact= (jid) => { dirtyContacts.add(jid); schedule(); };
  const markMsg = (jid, id, ts, json) => { dirtyMsgs.push({ jid, id, ts, json }); if (dirtyMsgs.length > FLUSH_MAX) flush(); };
      ...
      db.transaction(() => {
        if (dirtyChats.size) {
          for (const jid of dirtyChats) {
            const c = conn.chats[jid]; if (!c) continue;
            ps.chatUp.run(jid, JSON.stringify({
              id: c.id,
              name: c.name || null,
        ...
        if (dirtyContacts.size) {
          for (const cid of dirtyContacts) {
            const c = conn.chats[cid]; if (!c) continue;
            ps.contactUp.run(cid, JSON.stringify({ id: cid, name: c.name || null, imgUrl: c.imgUrl || null }));
          }
          dirtyContacts.clear();
        }
        if (dirtyMsgs.length) {
