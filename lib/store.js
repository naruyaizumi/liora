...
function openDB() {
    ...
    if (!db.__liora_init) {
        try { db.pragma("journal_mode = MEMORY"); } catch (e) { logger.debug(e.message); }
        try { db.pragma("synchronous = OFF"); } catch (e) { logger.debug(e.message); }
        try { db.pragma("temp_store = MEMORY"); } catch (e) { logger.debug(e.message); }
        try { db.pragma("locking_mode = NORMAL"); } catch (e) { logger.debug(e.message); }
        try { db.pragma("cache_spill = OFF"); } catch (e) { logger.debug(e.message); }
        try { db.pragma("foreign_keys = OFF"); } catch (e) { logger.debug(e.message); }
        try { db.pragma("cache_size = -65536"); } catch (e) { logger.debug(e.message); }
        try { db.pragma("busy_timeout = 10000"); } catch (e) { logger.debug(e.message); }
      ...
      `);
        ...
        const ps = {
            ...
            msgGetAll: db.prepare("SELECT id,json FROM messages WHERE jid=? ORDER BY ts DESC LIMIT ?"),
    ...
    function flush() {
        ...
        try {
            ...
            if (flushRetries < MAX_FLUSH_RETRIES) {
                ...
                logger.error(`Flush failed after ${MAX_FLUSH_RETRIES} retries, dropping ${chatsToFlush.size} chats, ${msgsToFlush.length} messages`);
                flushRetries = 0;
            }
        } finally {
            flushing = false;
            const hasWork = dirtyChats.size || dirtyGroups.size || dirtyContacts.size || dirtyMsgs.length || dirtyMsgsDel.length;
            if (flushScheduled || hasWork) {
                // Use exponential backoff for retries
                const delay = flushRetries > 0 
