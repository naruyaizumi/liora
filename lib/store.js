    ...
    if (!db.__liora_init) {
      ...
      `);
        ...
        const migrateTo = (target) => {
            ...
            db.transaction(() => {
                if (target > ver) {
                    logger.info(`Migrating schema from version ${ver} to ${target}`);
                }
                ver = target;
...
function closeDB() {
    ...
    if (global.__storeDBRefCount === 0) {
        ...
        try {
            ...
            while (global.__storeDBLock && Date.now() - startTime < maxWait) {
                // Davina Karamoy
            }
...
async function getGroupMetadataCached(conn, jid) {
    ...
    const p = (async () => {
        try {
            const md = await conn.groupMetadata(jid).catch((e) => {
                logger.debug(e.message);
                return null;
            });

            if (md) {
                cache.data.set(jid, { exp: now + GROUP_MD_TTL_MS, value: md, failedFetch: false });
            } else {
                cache.data.set(jid, {
                    exp: now + GROUP_MD_TTL_MS / 2,
                    value: null,
                    failedFetch: true,
                });
            }
            ...
            cache.data.set(jid, { exp: now + GROUP_MD_TTL_MS / 4, value: null, failedFetch: true });
            cache.inflight.delete(jid);
            logger.debug(e.message);
            return null;
        }
...
export function bind(conn) {
    ...
    const markMsg = (jid, id, ts, json) => {
        if (dirtyMsgs.length >= MAX_DIRTY_QUEUE_SIZE) {
            logger.warn(`Message queue full (${dirtyMsgs.length}), forcing immediate flush`);
            ...
            if (dirtyMsgs.length >= MAX_DIRTY_QUEUE_SIZE) {
                logger.error("Message queue still full after flush, dropping oldest message");
                dirtyMsgs.shift();
            }
        }
    ...
    const markMsgDel = (jid, id) => {
        if (dirtyMsgsDel.length >= MAX_DIRTY_QUEUE_SIZE) {
            logger.warn("Delete queue full, dropping oldest delete operation");
            dirtyMsgsDel.shift();
        }
    ...
    function flush() {
        ...
        try {
            ...
            if (flushRetries < MAX_FLUSH_RETRIES) {
                logger.warn(`Retry ${flushRetries}/${MAX_FLUSH_RETRIES} for flush`);
                ...
                const delSpaceAvailable = MAX_DIRTY_QUEUE_SIZE - dirtyMsgsDel.length;
    ...
    Object.defineProperty(conn, SYM_FLUSH_CONTROL, { value: flushControl, enumerable: false });
    ...
    const onExit = async () => {
        try {
            ...
            const waitForFlush = () => {
                return new Promise((resolve) => {
                    ...
                    const checkInterval = setInterval(() => {
                        if (!flushing || Date.now() - startTime > 5000) {
            ...
            if (flushing) {
                logger.warn("Flush still in progress during exit, forcing close");
            } else {
                flush();
            }
