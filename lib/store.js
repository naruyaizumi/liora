...
function openDB() {
    ...
    if (!db.__liora_init) {
        ...
        try { db.pragma("locking_mode = NORMAL"); } catch {} // EXCLUSIVE -> NORMAL (anti lock)
        try { db.pragma("cache_spill = OFF"); } catch {}
...
function buildHandlers(conn, dirty) {
    ...
    h.onGroupParticipantsUpdate = async function ({ id }) {
        ...
        if (md) {
            ...
            for (const p of md.participants || []) {
                if (p.id && p.lid) {
                    try { conn.lidCache.set(p.id, p.lid); } catch {}
                    try { conn.lidCache.set(p.lid, p.id); } catch {}
    ...
    const lidMappingStore = {
        ...
        async getPNForLID(lid) {
            ...
            if (!pn) {
                ...
                if (pn) {
                    try { lidCache.set(lid, pn); } catch {}
                    try { lidCache.set(pn, lid); } catch {}
                }
            }
            return pn || null;
        },
