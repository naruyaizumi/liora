    ...
    try {
      ...
      if (md && Array.isArray(md.participants)) {
        for (const p of md.participants) {
          if (p.id && p.lid) {
            try { conn.lidCache.set(p.id, p.lid); conn.lidCache.set(p.lid, p.id) } catch {}
            try { conn.lidMappingStore?.put?.(p.id, p.lid) } catch {}
          }
        }
      }
  ...
  h.updateContacts = function updateContacts(payload) {
    ...
    for (const contact of list) {
      ...
      if (isGroupJid(id)) {
        ...
        assignIf(chat, "name", contact.notify || contact.name || chat.name || chat.notify || "")
      }
      ...
      try {
        ps.contactUp.run(id, JSON.stringify({
          id, name: chat.name || null, imgUrl: chat.imgUrl || null
        }))
      } catch {}
...
export function bind(conn) {
  ...
  const lidMappingStore = {
    ...
    async getLIDForPN(pnOrJid) {
      ...
      if (!lid) {
        try { lid = ps.lidByPN.get(key)?.lid || null } catch {}
      }
      if (!lid && typeof conn.onWhatsApp === "function") {
        try {
          ...
          if (x?.exists && x?.lid) {
            lid = x.lid
            try { lidMappingStore.put(key, lid) } catch {}
          }
        } catch {}
      }
      return lid || null
    ...
    async getPNForLID(lid) {
      ...
      if (!pn) {
        try { pn = ps.pnByLID.get(lid)?.pn || null } catch {}
        if (pn) {
          try { lidCache.set(lid, pn); lidCache.set(pn, lid) } catch {}
        }
      }
      return pn || null
    },
  ...
  function flush() {
    ...
    try {
      db.transaction(() => {
        ...
        if (dirtyMsgs.length) {
          for (const { jid, id, ts, json } of dirtyMsgs) ps.msgUp.run(jid, id, ts || 0, json)
          dirtyMsgs.length = 0
        }
      })()
  ...
  const timers = {
    checkpoint: setInterval(() => {
      try { db.prepare("PRAGMA wal_checkpoint(FULL);").run() } catch {}
