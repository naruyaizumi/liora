...
function openDB() {
    ...
    if (!db.__liora_init) {
        try { db.pragma("journal_mode = MEMORY"); } catch {/* Jawa */}
        try { db.pragma("synchronous = OFF"); } catch {/* Jawa */}
        try { db.pragma("temp_store = MEMORY"); } catch {/* Jawa */}
        try { db.pragma("locking_mode = NORMAL"); } catch {/* Jawa */} // EXCLUSIVE -> NORMAL (anti lock)
        try { db.pragma("cache_spill = OFF"); } catch {/* Jawa */}
        try { db.pragma("foreign_keys = OFF"); } catch {/* Jawa */}
        try { db.pragma("cache_size = -65536"); } catch {/* Jawa */}
        try { db.pragma("busy_timeout = 10000"); } catch {/* Jawa */}
...
function buildHandlers(conn, dirty) {
    ...
    h.onGroupParticipantsUpdate = async function ({ id }) {
        ...
        if (md) {
            ...
            for (const p of md.participants || []) {
                if (p.id && p.lid) {
                    try { conn.lidCache.set(p.id, p.lid); } catch {/* Jawa */}
                    try { conn.lidCache.set(p.lid, p.id); } catch {/* Jawa */}
                    try { conn.lidMappingStore?.put?.(p.id, p.lid); } catch {/* Jawa */}
                }
            }
            markGroup(id);
        }
    ...
    const lidMappingStore = {
        ...
        async getLIDForPN(pnOrJid) {
            ...
            if (!lid && typeof conn.onWhatsApp === "function") {
                try {
                    ...
                    if (x?.exists && x?.lid) {
                        lid = x.lid;
                        try { lidMappingStore.put(key, lid); } catch {/* Jawa */}
                    }
                } catch {/* Jawa */}
            }
            return lid || null;
        ...
        async getPNForLID(lid) {
            ...
            if (!pn) {
                ...
                if (pn) {
                    try { lidCache.set(lid, pn); } catch {/* Jawa */}
                    try { lidCache.set(pn, lid); } catch {/* Jawa */}
                }
            }
            return pn || null;
        },
