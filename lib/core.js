...
async function checkUpdate() {
  ...
  if (remotePkg.version !== pkg.version) {
    console.log(
      chalk.yellow.bold(
        `🍓 Versi lokal: ${pkg.version} → Versi terbaru: ${remotePkg.version}`,
      ),
    );
    console.log(
      chalk.magenta.bold(
        "🍰 Ada update baru! Mohon tunggu sebentar ya, lagi proses~",
      ),
    );
    return true;
  } else {
...
async function doUpdate() {
  ...
  execSync(
    `curl -L https://github.com/${repo}/archive/refs/heads/main.zip -o tmp/update.zip`,
    {
      stdio: "inherit",
    },
  );
...
`),
  ...
  if (fails.length === 0) {
    ...
    console.error(
      chalk.red.bold(
        `🍂 ${fails.length} dependencies missing (${ok}/${total} OK):`,
      ),
    );
  ...
  if (global.config?.DEVELOPER) {
    console.log(
      chalk.yellow.bold("🛠️ Developer mode aktif → auto-update dilewati"),
    );
    return;
  }
