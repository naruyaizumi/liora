...
export default async function (m, conn = { user: {} }) {
  try {
    ...
    let truncatedMessage =
      messageText.length > 100 ? m.text.substring(0, 100) + "..." : m.text;
    ...
    console.log(chalk.cyan.bold("────────────────────────────────"));
    console.log(
      `${chalk.blue.bold("📨  Pengirim")}: ${chalk.yellow.bold(m.sender)}`,
    );
    console.log(
      `${chalk.blue.bold("🙎  Nama")}: ${chalk.yellow.bold(senderName)}`,
    );
