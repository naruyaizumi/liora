...
async function drawIconCard(ctx, x, y, width, height, iconUrl, label, value, subtext, accentColor, glowColor) {
    ...
    if (data.length > 0) {
        ...
        data.forEach((point, i) => {
            ...
            if (i === 0) {
                ...
                const prevY = chartY + chartHeight - ((prevPoint.value - minValue) / range) * chartHeight;
...
export async function canvas(systemData, options = {}) {
    ...
    const ramPct = systemData.ram.ramTotal > 0 ? 
        (systemData.ram.ramUsed / systemData.ram.ramTotal) * 100 : 0;
    const diskPct = systemData.disk.total > 0 ? 
        (systemData.disk.used / systemData.disk.total) * 100 : 0;
    
    drawCircularProgress(
        ctx, 220, yPos + 90, 80, 
        Math.min(cpuPct, 100), 
        colors.orange, colors.orangeGlow,
        'CPU', 
    ...
    drawCircularProgress(
        ctx, 980, yPos + 90, 80, 
        diskPct, 
        colors.purple, colors.purpleGlow,
        'STORAGE', 
    ...
    await drawIconCard(
        ctx, 40, yPos, cardWidth, cardHeight,
    ...
    await drawIconCard(
        ctx, 420 + 13, yPos, cardWidth, cardHeight,
    ...
    await drawIconCard(
        ctx, 800 + 27, yPos, cardWidth, cardHeight,
        ...
        colors.success, colors.successGlow
    );
    ...
    drawWaveChart(
        ctx, 40, yPos, chartWidth, chartHeight,
        history.cpu, colors.orange, colors.orangeGlow,
        'CPU LOAD', cpuPct
    );
    
    drawWaveChart(
        ctx, 40 + chartWidth + chartSpacing, yPos, chartWidth, chartHeight,
        history.memory, colors.accent, colors.accentGlow,
        'MEMORY USAGE', ramPct
    );
    ...
    drawWaveChart(
        ctx, 40, yPos, chartWidth, chartHeight,
        history.disk, colors.purple, colors.purpleGlow,
        'DISK USAGE', diskPct
    );
    
    const heapPct = systemData.heap.heapTotal > 0 ? 
        (systemData.heap.heapUsed / systemData.heap.heapTotal) * 100 : 0;
    
    drawWaveChart(
        ctx, 40 + chartWidth + chartSpacing, yPos, chartWidth, chartHeight,
        history.heap, colors.warning, colors.warningGlow,
        'HEAP MEMORY', heapPct
    );
    ...
    await drawIconCard(
        ...
        colors.warning, colors.warningGlow
    );
    
    await drawIconCard(
        ctx, 800, yPos, 360, 100,
    ...
    const pgGlow = systemData.postgres.status === 'Running' ? colors.successGlow : colors.dangerGlow;
    ...
    const redisGlow = systemData.redis.status === 'Running' ? colors.successGlow : colors.dangerGlow;
    
    await drawIconCard(
        ctx, 620, yPos, 540, 100,
