...
const isOwner = m.fromMe || isMods || [...global.config.owner.filter(([number, _, isDeveloper]) => number && !isDeveloper).map(([number]) => number.replace(/[^0-9]/g, '') + '@s.whatsapp.net')].includes(m.sender)
...
? [[new RegExp(_prefix.replace(/[|\\{}()[\]^$+*?.]/g, '\\$&')).exec(m.text), new RegExp(_prefix.replace(/[|\\{}()[\]^$+*?.]/g, '\\$&'))]] 
: [[[], new RegExp]]
).find(p => p[1])
...
m.text = ''
if (!m.fromMe && global.db.data.settings[this.user.jid]?.self && !isMods && !isOwner)
return
if (global.db.data.settings[this.user.jid]?.gconly && !m.isGroup && !isMods && !isOwner) {
...
mediaType: 1,
thumbnailUrl: 'https://files.cloudkuimages.guru/images/DqOJlT4iNQJP.jpg',
renderLargerThumbnail: true
}
}
}, { quoted: m })
...
text = text.replace('@subject', await this.getName(id))
...
case 'promote': {
text = chat.sPromote || this.promote || conn.promote || '@user has been promoted as admin!'
text = text.replace('@subject', await this.getName(id))
.replace('@desc', groupMetadata.desc?.toString() || 'unknown')
.replace('@user', userTag)
msgOptions = {
