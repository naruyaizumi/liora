...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    if (
      global.db.data.settings[this.user.jid]?.queque &&
      m.text &&
      !(isMods || isOwner)
    ) {
    ...
    const user =
      (m.isGroup
        ? participants.find(
            (u) =>
              conn.decodeJid(u.id) === senderId ||
              conn.decodeJid(u.phoneNumber) === senderId,
          )
        : {}) || {};
    ...
    const bot =
      (m.isGroup
        ? participants.find(
            (u) =>
              conn.decodeJid(u.id) === botId ||
              conn.decodeJid(u.phoneNumber) === botId,
          )
        : {}) || {};
